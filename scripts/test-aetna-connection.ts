...
const FHIR_BASE = 'https://vteapif1.aetna.com/fhirdirectory/v2';
...
async function main() {
  ...
  try {
    ...
    console.log(`   ✓ Token received (type: ${tokenData.token_type}, expires in: ${tokenData.expires_in}s)`);
    ...
    const bundle = await fhirRes.json() as { resourceType: string; total?: number; entry?: Array<{ resource: { resourceType: string; name?: Array<{ given?: string[]; family?: string }> } }> };
