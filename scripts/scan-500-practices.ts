...
async function updatePracticeCounts(state: string): Promise<void> {
  ...
  try {
    ...
    if (pcOk) {
      ...
      console.log(
        '  [Counts] update_provider_counts RPC failed — create it in Supabase SQL Editor (see scan-500-practices.ts comments)',
      );
    }
    ...
    if (mcOk) {
      ...
      console.log(
        '  [Counts] update_mismatch_counts RPC failed — create it in Supabase SQL Editor (see scan-500-practices.ts comments)',
      );
    }
...
async function exportTargetList(outputFile: string, state?: string) {
  ...
  const tiered = practices.map((p: any) => {
    ...
    if (p.mismatch_count >= 2 && p.provider_count >= 6) {
      tier = 1;
      tierLabel = 'Tier 1 — High signal';
    } else if (p.mismatch_count >= 2 && p.provider_count >= 4) {
      tier = 2;
      tierLabel = 'Tier 2 — Movement signal';
    } else if (p.mismatch_count >= 1 && p.provider_count >= 4) {
      tier = 3;
      tierLabel = 'Tier 3 — Single mismatch';
    } else {
      tier = 4;
      tierLabel = 'Tier 4 — Low signal';
    }
  ...
  // Build CSV
  const headers =
    'tier,tier_label,practice_id,npi,name,url,state,city,providers,mismatches,last_scan';
  const rows = tiered.map(
    (t: any) =>
  ...
  tiered.slice(0, 5).forEach((t: any, i: number) => {
    console.log(
      `    ${i + 1}. ${t.name || t.url} — ${t.mismatches} mismatches, ${t.providers} providers (${t.tierLabel})`,
    );
  });
}
