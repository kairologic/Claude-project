...
async function main() {
  ...
  let query = supabase
    ...
    .select('id, npi, practice_website_id, field_name, old_value, new_value, detection_source, confidence, confidence_score, signal_type, source_url, detected_at')
    .eq('resolved', false)
  ...
  let totalWf = 0, totalTasks = 0, totalAlerts = 0, totalEvents = 0;
  ...
  for (let i = 0; i < newDeltas.length; i += BATCH_SIZE) {
    ...
    const { data: insertedWfs, error: wfError } = await supabase
      .from('workflow_instances')
      .insert(workflowRows)
      .select('id, practice_id, provider_npi, provider_name, finding_summary, finding_details, created_at, overdue_at');
