...
async function downloadAndExtract(url: string, label: string): Promise<string> {
  ...
  if (!csvFile) {
    throw new Error(`No NPPES CSV file found in extracted ZIP. Files: ${files.join(', ')}`);
  }

  const csvPath = join(WORK_DIR, csvFile);
  const csvSize = statSync(csvPath).size;
  console.log(`[Download] Found CSV: ${csvFile} (${(csvSize / 1024 / 1024).toFixed(1)} MB)`);
...
async function main() {
  ...
  try {
    ...
    console.log(`[Tracked NPIs] Found ${trackedNpis.size} NPIs in practice_providers`);
  } catch {
    // practice_providers may be empty early on; fall back to state-based
    console.log('[Tracked NPIs] practice_providers empty, falling back to state-based tracking');
  ...
  const sourceFile = options.full ? `full_replacement_${today}` : `weekly_diff_${today}`;
