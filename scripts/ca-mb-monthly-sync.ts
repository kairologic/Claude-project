...
async function main() {
  ...
  console.log('═══════════════════════════════════════════════════════');
  console.log(
    `  Mode:       ${options.resolveOnly ? 'Resolution only' : 'Full sync (ingest + resolve)'}`,
  );
  ...
  if (!options.resolveOnly) {
    ...
    const parseResult = await parseCaMbFile(csvPath, {
      ...
      onProgress: (processed, matched) => {
        process.stdout.write(
          `\r[CA MB] Processed ${processed.toLocaleString()} lines, ${matched.toLocaleString()} matched`,
        );
      },
    });
    ...
    console.log('\n[CA MB] Data quality:');
    console.log(
      `  With address:   ${withAddress.toLocaleString()} (${((withAddress / parseResult.records.length) * 100).toFixed(1)}%)`,
    );
    console.log(
      `  With specialty: ${withSpecialty.toLocaleString()} (${((withSpecialty / parseResult.records.length) * 100).toFixed(1)}%)`,
    );
  ...
  if (!options.skipResolution && !options.dryRun) {
    ...
    if (unresolved.length > 0) {
      ...
      const resolveRate =
        totalResult.total > 0
          ? (
              ((totalResult.resolved_pecos + totalResult.resolved_fuzzy) / totalResult.total) *
              100
            ).toFixed(1)
          : '0';
    ...
    if (validation.total_sample > 0) {
      ...
      console.log(
        `  PECOS FP rate:   ${(validation.pecos_fp_rate * 100).toFixed(2)}% (target: <1%)`,
      );
      console.log(
        `  Fuzzy FP rate:   ${(validation.fuzzy_fp_rate * 100).toFixed(2)}% (target: <3%)`,
      );
      console.log(
        `  Weighted FP rate: ${(validation.weighted_fp_rate * 100).toFixed(2)}% (target: <2%)`,
      );
      ...
      if (!validation.gate_passed) {
        ...
        console.warn(
          '  Review provider_npi_resolutions for false positives and adjust thresholds.',
        );
      }
    } else {
      console.log(
        '[Validation] No validated samples yet. Mark records in provider_npi_resolutions',
      );
