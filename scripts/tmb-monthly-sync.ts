...
function parseArgs(): SyncOptions {
  ...
  if (!filePath) {
    console.error(
      'Usage: npx tsx scripts/tmb-monthly-sync.ts <path-to-PHY-file.txt> [--dry-run] [--active-only] [--limit N]',
    );
    console.error('');
  ...
  const limit = limitIdx >= 0 && limitIdx + 1 < args.length ? parseInt(args[limitIdx + 1], 10) : 0;
...
async function main() {
  ...
  console.log('\n[TMB] Data quality:');
  console.log(
    `  With practice address: ${withPracAddress.toLocaleString()} (${((withPracAddress / records.length) * 100).toFixed(1)}%)`,
  );
  console.log(
    `  TX practice address:   ${txPractice.toLocaleString()} (${((txPractice / records.length) * 100).toFixed(1)}%)`,
  );
  ...
  console.log(
    `  Active licenses:    ${records.filter((r) => r.is_active).length.toLocaleString()}`,
  );
  console.log(
    `  Disciplinary flags: ${rows.filter((r) => r.has_disciplinary_action).length.toLocaleString()}`,
  );
