...
async function main() {
  ...
  console.log('═══════════════════════════════════════════════════════');
  console.log(
    `  Mode:      ${deltaOnly ? 'Delta only' : scanOnly ? 'Scan only' : 'Full (scan + delta)'}`,
  );
  ...
  if (!dryRun) {
    ...
    console.log(
      `  provider_count:  ${pcOk ? 'updated' : 'RPC not found (create update_provider_counts in SQL Editor)'}`,
    );

    const mcOk = await rpcCall('update_mismatch_counts');
    console.log(
      `  mismatch_count:  ${mcOk ? 'updated' : 'RPC not found (create update_mismatch_counts in SQL Editor)'}`,
    );
  }
