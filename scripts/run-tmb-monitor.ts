...
async function main() {
  ...
  console.log(
    `Mode: ${dryRun ? 'DRY RUN' : backfill ? `BACKFILL (${pages} pages)` : 'PRODUCTION'}`,
  );
      ...
      if (backfill) {
        ...
        for (let page = 0; page < pages; page++) {
          ...
          for (const release of releases) {
            const icon =
              release.releaseType === 'EMERGENCY_SUSPENSION'
                ? '🚨'
                : release.releaseType === 'BOARD_MEETING_ACTIONS'
                  ? '📋'
                  : release.releaseType === 'PA_SUSPENSION'
                    ? '⚠️'
                    : '📰';
        ...
        const suspensions = allReleases.filter(
          (r: any) => r.releaseType === 'EMERGENCY_SUSPENSION',
        );
        ...
        console.log(
          `Other releases:            ${allReleases.length - suspensions.length - meetings.length}`,
        );
        if (allReleases.length > 0) {
          console.log(
            `Date range:                ${allReleases[allReleases.length - 1].date} to ${allReleases[0].date}`,
          );
        }
      } else {
        // Single page dry run
