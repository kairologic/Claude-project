...
async function fetchNpi(npi: string): Promise<any> {
  try {
    ...
    return {
      npi: r.number || npi,
      org_name:
        basic.organization_name || `${basic.first_name || ''} ${basic.last_name || ''}`.trim(),
      prac_line1: addr.address_1 || '',
      prac_line2: addr.address_2 || '',
      prac_city: addr.city || '',
...
async function crawl(url: string): Promise<any> {
  try {
    ...
    return {
      ...addr,
      phone,
      specialty_labels: specs,
      provider_names: provs,
...
function getAddress(html: string, text: string) {
  ...
  if (ld) {
    for (const b of ld) {
      try {
        ...
        if (a?.streetAddress)
          return {
            addr_line1: a.streetAddress,
            addr_line2: '',
            addr_city: a.addressLocality || '',
            addr_state: a.addressRegion || '',
            addr_zip: (a.postalCode || '').slice(0, 5),
          };
      } catch {}
    }
  }
  // General pattern
  const m = text.match(
    /(\d{1,5}\s+[A-Za-z0-9\s.,#\-]+?)[\s,]+([A-Z][a-z]+(?:\s[A-Z][a-z]+)?)\s*,\s*([A-Z]{2})\s+(\d{5})/,
  );
  if (m) {
    ...
    return {
      ...
      addr_city: m[2],
      addr_state: m[3],
      addr_zip: m[4].slice(0, 5),
    };
  }
  // TX pattern
  const tx = text.match(
    /(\d{1,5}\s+[A-Za-z0-9\s.,#\-]{5,40})\s*[\n,]\s*([A-Za-z\s]+),?\s*TX\s+(\d{5})/i,
  );
  if (tx)
    return {
      addr_line1: tx[1].trim(),
      addr_line2: '',
      addr_city: tx[2].trim(),
      addr_state: 'TX',
      addr_zip: tx[3],
...
function getPhone(html: string, text: string): string {
  ...
  if (tel) {
    const d = tel[1].replace(/\D/g, '').slice(-10);
    if (d.length === 10) return `(${d.slice(0, 3)}) ${d.slice(3, 6)}-${d.slice(6)}`;
  }
  const jld = html.match(/"telephone"\s*:\s*"([^"]+)"/i);
  if (jld) {
    const d = jld[1].replace(/\D/g, '').slice(-10);
    if (d.length === 10) return `(${d.slice(0, 3)}) ${d.slice(3, 6)}-${d.slice(6)}`;
  }
...
function getProviders(text: string): string[] {
  ...
  const cred = text.matchAll(
    /(?:Dr\.?\s+)?([A-Z][a-z]+(?:\s+[A-Z]\.?)?\s+[A-Z][a-z]{2,})(?:\s*,\s*(?:MD|DO|NP|PA|PA-C|APRN|DNP|PhD|DPM|DC|DDS|DMD|OD|RN|LPC|LCSW|LMFT))/g,
  );
  for (const m of cred) {
    const n = m[1].trim();
...
function addrEq(a: string, b: string): boolean {
  if (!a || !b) return false;
  if (a === b) return true;
  const za = a.match(/\b(\d{5})\b/)?.[1],
    zb = b.match(/\b(\d{5})\b/)?.[1];
...
function specMatch(npiClass: string, siteLabels: string[]): boolean {
  ...
  const syn: Record<string, string[]> = {
    ...
    'clinical social worker': ['counseling', 'mental health', 'behavioral health'],
  };
...
function runChecks(npi: any, site: any): CResult[] {
  ...
  if (npi && site?.addr_line1) {
    ...
    const ok =
      addrEq(nA, sA) ||
      (npi.addresses_secondary || []).some((s: any) =>
        addrEq(norm(s.line1, s.city, s.state, s.zip), sA),
      );
    r.push({
      check_id: 'NPI-01',
      ...
      status: ok ? 'pass' : 'fail',
      score: ok ? 100 : 25,
      title: ok ? 'Address matches NPI' : 'Address mismatch',
      detail: ok
        ? `Verified: ${npi.prac_city}, ${npi.prac_state}`
        : `Site: "${site.addr_line1}, ${site.addr_city}" vs NPI: "${npi.prac_line1}, ${npi.prac_city}"`,
      evidence: {
        npi_address: `${npi.prac_line1}, ${npi.prac_city}, ${npi.prac_state} ${npi.prac_zip}`,
    ...
    r.push({
      ...
      status: 'inconclusive',
      score: 0,
      title: 'Address check incomplete',
      detail: npi ? 'No address on website' : 'NPI data unavailable',
    });
  }
  ...
  if (npi?.prac_phone && site?.phone) {
    ...
    r.push({
      ...
      status: ok ? 'pass' : 'fail',
      score: ok ? 100 : 40,
      title: ok ? 'Phone matches NPI' : 'Phone mismatch',
      detail: ok ? `Verified: ${npi.prac_phone}` : `Site: ${site.phone} vs NPI: ${npi.prac_phone}`,
  ...
  if (npi?.tax_classification && site?.specialty_labels?.length) {
    ...
    r.push({
      ...
      status: ok ? 'pass' : 'warn',
      score: ok ? 100 : 60,
      title: ok ? 'Specialty matches NPI' : 'Specialty discrepancy',
      detail: ok
        ? `Verified: ${npi.tax_classification}`
        : `NPI: "${npi.tax_classification}" vs Site: "${site.specialty_labels.join(', ')}"`,
      evidence: {
        npi_classification: npi.tax_classification,
    ...
    r.push({
      ...
      status: 'inconclusive',
      score: 0,
      title: 'Specialty check incomplete',
      detail: 'Taxonomy or specialty data unavailable',
    });
  }
  ...
  if (site?.provider_names?.length > 0) {
    r.push({
      ...
      status: 'pass',
      score: 80,
      title: `${site.provider_names.length} providers detected`,
      detail: `Found ${site.provider_names.length} provider(s) on website`,
    ...
    r.push({
      ...
      status: 'inconclusive',
      score: 0,
      title: 'Provider roster not detected',
      detail: 'No provider names found on website',
    });
  }
  ...
  for (let i = 0; i < total; i++) {
    ...
    try {
      ...
      const icons = results
        ...
        .join('');
      console.log(
        `  → ${score}/100 (${level}) [${icons}] site:${site ? '✓' : '✗'} npi:${npi ? '✓' : '✗'}`,
      );
      ...
      if (!DRY_RUN) {
        ...
        if (npi) {
          ...
          await db('org_npi_footprint', {
            method: 'POST',
            body: JSON.stringify({
              npi: npi.npi,
              org_name: npi.org_name,
              prac_line1: npi.prac_line1,
              prac_line2: npi.prac_line2,
              prac_city: npi.prac_city,
              prac_state: npi.prac_state,
              prac_zip: npi.prac_zip,
              prac_phone: npi.prac_phone,
              tax_code: npi.tax_code,
              tax_classification: npi.tax_classification,
              enumeration_date: npi.enumeration_date,
              last_update_date: npi.last_update_date,
              addresses_secondary: npi.addresses_secondary,
        ...
        if (site) {
          await db('site_snapshots', {
            method: 'POST',
            body: JSON.stringify({
              npi: p.npi,
              url: p.url,
              addr_line1: site.addr_line1,
              addr_line2: site.addr_line2,
              addr_city: site.addr_city,
              addr_state: site.addr_state,
              addr_zip: site.addr_zip,
              phone: site.phone,
              specialty_labels: site.specialty_labels,
              provider_names: site.provider_names,
          ...
          if (r.status === 'fail' || r.status === 'warn') {
            await db('mismatch_alerts', {
              ...
              body: JSON.stringify({
                ...
                severity: r.severity,
                npi_value:
                  r.evidence?.npi_address ||
                  r.evidence?.npi_phone ||
                  r.evidence?.npi_classification ||
                  '',
                site_value:
                  r.evidence?.site_address ||
                  r.evidence?.site_phone ||
                  r.evidence?.site_specialties?.join(', ') ||
                  '',
                delta_detail: r.detail,
                risk_score: r.score,
                status: 'open',
              }),
            });
