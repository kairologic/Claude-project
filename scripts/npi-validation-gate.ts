...
import { runValidationGate, fetchUnresolvedLicenses } from '../lib/nppes/npi-resolver';
...
async function seedSample(sampleSize: number, states: string[]) {
  ...
  for (let i = 0; i < ids.length; i += BATCH) {
    ...
    await dbRequest(`provider_npi_resolutions?id=in.(${idList})`, {
      method: 'PATCH',
...
async function exportReview(output: string) {
  ...
  const rows = records.map((r: any) => [
    ...
    r.input_state,
    `"${(r.input_specialty || '').replace(/"/g, '""')}"`,
    `"${(r.input_address || '').replace(/"/g, '""')}"`,
    r.pecos_npi,
    `"${(r.pecos_name || '').replace(/"/g, '""')}"`,
...
async function showStats() {
  ...
  console.log(
    `  Resolved:                  ${resolved.length} (${total.length > 0 ? ((resolved.length / total.length) * 100).toFixed(1) : 0}%)`,
  );
  ...
  for (const [method, stats] of byMethod) {
    console.log(
      `    ${method}: ${stats.resolved}/${stats.total} resolved (${stats.total > 0 ? ((stats.resolved / stats.total) * 100).toFixed(1) : 0}%)`,
    );
  }
}
