...
function discoverDashboardRoutes(): DiscoveredRoute[] {
  ...
  return pages.map((p) => {
    ...
    const slug =
      rel === 'page.tsx' ? 'home' : rel.replace(/\//g, '-').replace(/\[.*?\]/g, 'detail');
    return {
      type: 'page',
      path: p,
...
function getExistingSpecs(): Set<string> {
  ...
  return new Set(readdirSync(E2E_DIR).filter((f) => f.endsWith('.spec.ts')));
}
...
function main() {
  ...
  const untested = allRoutes.filter((r) => {
    ...
    return !Array.from(existingSpecs).some(
      (s) =>
        s.includes(slug) ||
        slug.includes(s.replace('.spec.ts', '').replace('.e2e', '').replace('.api', '')),
    );
  });
  ...
  if (generateMode) {
    ...
    for (const route of untested) {
      ...
      const content = route.type === 'page' ? generatePageStub(route) : generateApiStub(route);
