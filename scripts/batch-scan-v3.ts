...
function parseCIDR(cidr: string) {
  ...
  return {
    networkInt: ((p[0] << 24) | (p[1] << 16) | (p[2] << 8) | p[3]) >>> 0,
...
for (const def of CDN_CIDR_DEFS)
  for (const cidr of def.ranges) {
    const p = parseCIDR(cidr);
    CDN_RANGES.push({ ...p, provider: def.provider });
  }

function detectCDNByIP(ip: string): { detected: boolean; provider: string } {
  const n = ipToInt(ip);
  if (n === 0) return { detected: false, provider: '' };
  for (const r of CDN_RANGES) {
    const mask = r.maskBits === 0 ? 0 : (~0 << (32 - r.maskBits)) >>> 0;
    if ((n & mask) === (r.networkInt & mask)) return { detected: true, provider: r.provider };
  }
...
async function detectCDNByCNAME(
  domain: string,
): Promise<{ detected: boolean; provider: string; cname: string }> {
  try {
    ...
    for (const a of data.Answer) {
      ...
      for (const p of CNAME_PATTERNS) {
        if (cname.endsWith(p.pattern)) return { detected: true, provider: p.provider, cname };
      }
    }
...
function detectCDNByHeaders(headers: Record<string, string>): {
  ...
  if (
    ...
    headers['x-cache']?.includes('fastly') ||
  ...
  if (headers['x-akamai-transformed'] || (headers['server'] || '').includes('akamai'))
  ...
  if (headers['x-sucuri-id'] || (headers['server'] || '').includes('sucuri'))
    return { detected: true, provider: 'Sucuri' };
  return { detected: false, provider: '' };
}
...
async function fetchPage(
  url: string,
): Promise<{ html: string; headers: Record<string, string>; success: boolean }> {
  try {
    const u = url.startsWith('http') ? url : `https://${url}`;
    const res = await fetch(u, {
      signal: AbortSignal.timeout(12000),
      headers: { 'User-Agent': 'KairoLogic-Sentry/3.2', Accept: 'text/html' },
      redirect: 'follow',
    });
    ...
    return { html: '', headers: {}, success: false };
  }
}
...
async function runScanChecks(
  url: string,
  domain: string,
): Promise<{ results: CheckResult[]; cdn: CDNResult }> {
  const results: CheckResult[] = [];
  const page = await fetchPage(url);
  const geo = await geoLookup(domain);
  const cdn = geo
    ? await detectCDN(geo.ip, domain, page.headers)
    : await detectCDN('', domain, page.headers);
  ...
  if (!geo) {
    results.push({
      ...
      detail: `Unable to resolve IP for ${domain}`,
      evidence: { domain },
    });
    ...
    results.push({
      ...
      cdn_provider: cdn.provider || undefined,
    });
  } else {
    results.push({
      check_id: 'DR-01',
  ...
  if (cdn.detected) {
    results.push({
      ...
      status: 'warn',
      score: 80,
      title: `CDN transparency: ${cdn.provider}`,
      detail: `Uses ${cdn.provider}. Execute DPA, create Data Flow Map, add attestation.`,
  ...
  if (
    [
      ...
      'LHR',
      'HKG',
    ].some((p) => cfPop.toUpperCase().includes(p))
  )
  ...
  results.push({
    ...
    score: foreignEdges.length > 0 ? 70 : 100,
    title:
      foreignEdges.length > 0
        ? `Foreign edges: ${foreignEdges.join(', ')}`
        : 'No foreign edge nodes',
    detail:
      foreignEdges.length > 0
        ? `CDN foreign indicators: ${foreignEdges.join(', ')}. Static asset only.`
        : 'US-restricted.',
  ...
  if (mxRecords.length === 0) {
    results.push({
      ...
      score: 60,
      title: 'No MX records',
      detail: `No MX for ${domain}.`,
    });
  } else {
    const mxGeo = await Promise.all(
      mxRecords
        .slice(0, 3)
        .map(async (mx) => ({ exchange: mx.exchange, geo: await geoLookup(mx.exchange) })),
    );
    const foreignMX = mxGeo.filter(
      (r) => r.geo && !r.geo.isSovereign && !isKnownUSMail(r.exchange),
    );
    results.push({
      check_id: 'DR-03',
      category: 'data-sovereignty',
      ...
      score: foreignMX.length > 0 ? 10 : 100,
      title:
        foreignMX.length > 0
          ? `Foreign mail: ${foreignMX.map((f) => f.geo?.country).join(', ')}`
          : `MX: ${mxRecords[0].exchange}`,
      detail:
        foreignMX.length > 0
          ? `Foreign: ${foreignMX.map((f) => `${f.exchange} (${f.geo?.country})`).join('; ')}`
          : `${mxRecords.length} MX, all US. Primary: ${mxRecords[0].exchange}`,
  ...
  if (page.success) {
    ...
    if (extDomains.size === 0) {
      ...
      const geoR = await Promise.all(
        toCheck.map(async (d) => ({ domain: d, geo: await geoLookup(d), isUS: isKnownUSSaaS(d) })),
      );
      ...
      results.push({
        ...
        score: foreign.length > 0 ? 40 : 100,
        title:
          foreign.length > 0
            ? `${foreign.length} foreign sub-processor(s)`
            : `${extDomains.size} external, all US`,
        detail:
          foreign.length > 0
            ? `Foreign: ${foreign.map((f) => `${f.domain} (${f.geo?.country})`).join(', ')}`
            : `Sampled ${toCheck.length}, all US.`,
        evidence: {
    ...
    results.push({
      ...
      detail: 'Could not fetch for sub-processor check.',
    });
  }
...
async function main() {
  ...
  const providers = await db(
    'registry?url=not.is.null&url=neq.&select=npi,name,url,city,zip,email,risk_score,risk_level&limit=' +
      MAX_PROVIDERS,
  );
  if (!providers?.length) {
  ...
  for (let i = 0; i < total; i++) {
    ...
    try {
      ...
      const icons = results
        .map((r) => (r.status === 'pass' ? '✓' : r.status === 'fail' ? '✗' : '⚠'))
        .join('');

      if (cdn.detected) {
        cdnCount++;
        console.log(`  → CDN: ${cdn.provider} (${cdn.detectedVia})`);
      } else if (dr01?.status === 'fail') {
        foreignCount++;
        console.log(`  → FOREIGN: ${(dr01.evidence as any)?.country || '?'}`);
      } else {
        usCount++;
      }
      ...
      cdnReport.push({
        ...
        dr01_new: dr01?.status || '?',
        cdn: cdn.detected,
        cdn_prov: cdn.provider,
        cdn_via: cdn.detectedVia,
        score,
        level,
        checks: results.map((r) => `${r.check_id}:${r.status}`).join(' '),
      });

      if (!DRY_RUN) {
        const sess = await db('scan_sessions', {
          method: 'POST',
        ...
        await db(`registry?npi=eq.${p.npi}`, {
          method: 'PATCH',
          body: JSON.stringify({
            risk_score: score,
            risk_level: level,
  ...
  if (SAVE_CDN_REPORT) {
    ...
    const rows = cdnReport.map(
      (r) =>
        `"${r.npi}","${r.name.replace(/"/g, '""')}","${r.url}","${r.domain}","${r.dr01_old}","${r.dr01_new}",${r.cdn},"${r.cdn_prov}","${r.cdn_via}",${r.score},"${r.level}","${r.checks}"`,
    );
    ...
    cdnReport
      .filter((r) => r.cdn)
      .forEach((r) => {
        provs[r.cdn_prov] = (provs[r.cdn_prov] || 0) + 1;
      });
    ...
    if (fixed.length) {
      console.log(`\n  🔧 Fixed false positives (${fixed.length}):`);
      fixed
        .slice(0, 10)
        .forEach((r) => console.log(`    ${r.name} → ${r.cdn_prov} (${r.cdn_via})`));
    }

    const foreign = cdnReport.filter((r) => r.dr01_new === 'fail');
    if (foreign.length) {
      console.log(`\n  🌍 Genuine foreign (${foreign.length}):`);
      foreign.forEach((r) => console.log(`    ${r.name} (${r.domain})`));
    }
  }
  console.log('\n═══════════════════════════════════════════════\n');
}
