...
function parseArgs() {
  ...
  if (!flags.npi) {
    console.error(
      'Usage: npx tsx scripts/sync-payer-directories.ts --npi <NPI> [--payer <code>] [--dry-run] [--verbose]',
    );
    console.error('');
...
async function main() {
  ...
  console.log(
    `  Found ${endpoints.length} active endpoint(s): ${endpoints.map((e) => e.payer_code).join(', ')}`,
  );
  console.log('');
  ...
  if (nppesData) {
    console.log(
      `  Name:     ${nppesData.provider_name || nppesData.first_name + ' ' + nppesData.last_name}`,
    );
    console.log(
      `  Address:  ${nppesData.address_line_1 || '(none)'}, ${nppesData.city}, ${nppesData.state} ${nppesData.zip}`,
    );
  ...
  for (const snap of snapshots) {
    ...
    console.log(
      `│            ${snap.listed_city || ''}, ${snap.listed_state || ''} ${snap.listed_zip || ''}`,
    );
    console.log(`│ Phone:     ${snap.listed_phone || '(none)'}`);
    console.log(
      `│ Specialty: ${snap.listed_specialty_display || '(none)'} [${snap.listed_specialty_code || ''}]`,
    );
  ...
  if (nppesData && snapshots.length > 0) {
    ...
    for (const snap of snapshots) {
      ...
      if (mismatches.length > 0) {
        ...
        for (const m of mismatches) {
          console.log(
            `    ⚠ ${m.field_name} (${m.mismatch_type}): NPPES="${m.nppes_value}" vs Payer="${m.payer_value}"`,
          );
          console.log(
            `      Fix via CAQH: ${m.fix_via_caqh ? 'YES' : 'no'} | Priority: ${m.priority}`,
          );
        }
      } else {
    ...
    if (allMismatches.length > 0) {
      ...
      for (const action of actions) {
        console.log(
          `    Step ${action.step}: ${action.action} [${action.target}] (~${action.effort})`,
        );
...
async function loadNppesData(
  ...
  const { data: provider } = await supabase
    .from('providers')
    .select(
      'npi, organization_name, first_name, last_name, credential, address_line_1, address_line_2, city, state, zip_code, phone, fax, primary_taxonomy_code, taxonomy_desc',
    )
    .eq('npi', npi)
    .limit(1)
    .single();
  ...
  const { data: pecos } = await supabase
    .from('provider_pecos')
    .select(
      'npi, provider_name, first_name, last_name, organization_name, address_line_1, address_line_2, city, state, zip_code, specialty',
    )
    .eq('npi', npi)
    .limit(1)
    .single();
...
async function writeMismatches(
  ...
  for (const m of mismatches) {
    const { error } = await supabase.from('payer_directory_mismatches').insert({
      npi: m.npi,
