...
async function main() {
  ...
  let endpointPath =
    'payer_directory_endpoints?is_active=eq.true&fhir_base_url=not.like.SCRAPE*&fhir_base_url=not.eq.TBD';
  if (PAYER_FILTER) {
  ...
  const providers = await supabaseRequest<
    { npi: string; practice_website_id: string; provider_name: string }[]
  >('rpc/get_active_provider_npis', { method: 'POST', body: JSON.stringify(rpcParams) });
  ...
  if (DRY_RUN) {
    console.log('\n[DRY RUN] Would query:');
    console.log(
      `  ${providers.length} providers x ${endpoints.length} payers = ${providers.length * endpoints.length} lookups`,
    );
    return;
  }
    ...
    for (let i = 0; i < providers.length; i += BATCH_SIZE) {
      ...
      for (const provider of batch) {
        try {
          ...
          await supabaseRequest('payer_directory_snapshots', {
            method: 'POST',
          ...
          if (nppesRows && nppesRows.length > 0) {
            ...
            if (mismatches.length > 0) {
              ...
              for (const m of mismatches) {
                ...
                await supabaseRequest('payer_directory_mismatches', {
                  method: 'POST',
                  body: JSON.stringify(mismatchRow),
                });
              }
            }
          }
          ...
          if (
            ...
            console.warn(
              `  ⚠️ Marking ${endpoint.payer_code} as failed — skipping remaining providers for this payer`,
            );
      ...
      process.stdout.write(
        `\r  Progress: ${progress}/${providers.length} | Listed: ${payerListed} | Not listed: ${payerNotListed} | Mismatches: ${payerMismatches} | Errors: ${payerErrors}`,
      );
    }
