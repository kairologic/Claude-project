...
export default async function WorkflowsPage({ params }: { params: { id: string } }) {
  ...
  const [workflowsResult, kpiResult] = await Promise.all([
    safeQuery(
      admin
        ...
        .select(
          'id, workflow_type, status, provider_npi, provider_name, finding_summary, finding_details, priority, overdue_at, created_at',
        )
        .eq('practice_id', practiceId)
        .or(
          `status.not.in.(resolved,cancelled),updated_at.gt.${new Date(Date.now() - 90 * 24 * 60 * 60 * 1000).toISOString()}`,
        )
  ...
  return <WorkflowsView workflows={allWorkflows} practiceId={practiceId} counts={counts} />;
}
