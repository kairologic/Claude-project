...
import { AlertTriangle, CheckCircle, Shield, FileText, ArrowRight, XCircle, ExternalLink } from 'lucide-react';
...
/** Strip UTM params and trailing slashes from URLs for clean display */
  ...
  return (
    <div className="flex flex-col items-center">
      <svg width="110" height="110" viewBox="0 0 110 110">
        ...
        <circle
          cx="55" cy="55" r="44" fill="none" stroke={ring} strokeWidth="6"
...
function ProductCard({
  ...
  return (
    <div className={`bg-[#0d1a2e] rounded-2xl p-6 md:p-8 flex flex-col relative ${
      recommended ? 'border-2 border-[#c9a84c] shadow-[0_0_30px_rgba(201,168,76,0.15)]' : 'border border-white/10'
    }`}>
      {recommended && (
      ...
      {priceNote && (
        <p className="text-xs text-gray-400 mb-5">{priceNote}</p>
      )}
      ...
      <a
        ...
        className={`flex items-center justify-center gap-2 py-3.5 px-6 rounded-xl font-bold text-sm transition-all duration-200 ${
          recommended
            ? 'bg-[#c9a84c] hover:bg-[#b8933d] text-[#0a1628]'
...
export default function ReportLandingPage() {
    ...
    return (
      <div className="min-h-screen bg-[#0a1628] flex items-center justify-center px-4">
        <div className="text-center max-w-md">
          ...
          <a href="mailto:ravi@kairologic.net" className="inline-flex items-center gap-2 text-[#c9a84c] hover:text-[#d4b44e] transition-colors font-medium">
      ...
      <div className="min-h-screen bg-[#0a1628]">
        ...
        <div className="border-b border-white/10">
          <div className="max-w-4xl mx-auto px-4 py-4">
            <Link href="/" className="inline-flex items-center gap-1 text-xl font-extrabold tracking-tight">
  ...
  const recommendAudit = score >= 60; // Drift and Sovereign tiers
  const recommendSafeHarbor = score < 60; // At Risk tier

  // Urgency messaging
  const getUrgencyMessage = (s: number) => {
    if (s < 60) return 'Your site has critical compliance gaps under Texas SB 1188. Immediate action is recommended to avoid potential enforcement.';
    if (s < 80) return 'Your site has compliance gaps under Texas SB 1188. Here is how to fix them before enforcement begins.';
        ...
        <div className="max-w-3xl mx-auto px-4">
          ...
          <div className="bg-[#0d1a2e] border border-white/10 rounded-2xl p-6 md:p-8">
            <div className="flex flex-col md:flex-row md:items-center gap-6 mb-6">
              ...
              <div className="flex-1 space-y-2">
                <div className="flex items-center gap-3">
                  <span className="text-xs text-gray-500 uppercase tracking-wider w-12 flex-shrink-0">NPI</span>
                  <span className="text-sm text-gray-300 font-mono">{data.npi}</span>
                </div>
                {data.url && data.url !== '__NOT_FOUND__' && (
                  <div className="flex items-center gap-3">
                    <span className="text-xs text-gray-500 uppercase tracking-wider w-12 flex-shrink-0">Site</span>
                    <span className="text-sm text-gray-300">{cleanUrl(data.url)}</span>
                  </div>
                )}
                {data.score !== null && (
                  <div className="flex items-center gap-3">
                    <span className="text-xs text-gray-500 uppercase tracking-wider w-12 flex-shrink-0">Score</span>
            ...
            {(failedFindings.length > 0 || warnFindings.length > 0) && (
              <div>
                <h3 className="text-xs text-gray-500 uppercase tracking-wider font-bold mb-3">
                  Findings ({failedFindings.length} failed, {warnFindings.length} warnings)
                </h3>
      ...
      <section className="py-8 border-t border-white/10">
        <div className="max-w-4xl mx-auto px-4 text-center">
          <p className="text-gray-500 text-xs">
            Questions? Contact <a href="mailto:ravi@kairologic.net" className="text-[#c9a84c] hover:text-[#d4b44e] transition-colors">ravi@kairologic.net</a>
          </p>
