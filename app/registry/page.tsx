...
import { Search, Shield, ShieldCheck, Eye, Lock, ChevronRight, X, AlertTriangle, CheckCircle, Loader2, FileText, ChevronLeft } from 'lucide-react';
  ...
  const handleClaim = async () => {
    ...
    try {
      ...
      await supabase.from('prospects').insert({
        ...
        admin_notes: `Claimed: ${claimModal?.name}. NPI: ${claimForm.npi || 'N/A'}. Score: ${claimModal?.risk_score || 'N/A'}`,
    ...
    <div className="min-h-screen bg-[#070d1b]">
      ...
      <section className="border-y border-white/[0.08] bg-white/[0.03]">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
          <div className="grid grid-cols-4 gap-4">
            {[
              { label: 'Sovereign', value: stats.sovereign, color: 'text-emerald-400', dot: 'bg-emerald-400' },
              { label: 'Inconclusive', value: stats.moderate, color: 'text-amber-400', dot: 'bg-amber-400' },
              { label: 'At Risk', value: stats.atRisk, color: 'text-red-400', dot: 'bg-red-400' },
              { label: 'Pre-Audited', value: stats.pending, color: 'text-slate-400', dot: 'bg-slate-500' },
            ].map(s => (
          ...
          <div className="flex gap-1 overflow-x-auto py-2 -mb-px">
            {CITY_TABS.map(tab => (
              <button key={tab.key} onClick={() => setActiveCity(tab.key)}
                className={`px-4 py-2.5 text-xs font-bold uppercase tracking-wider whitespace-nowrap transition-all rounded-t-lg ${activeCity === tab.key ? 'bg-white/[0.08] text-gold border-b-2 border-gold' : 'text-slate-400 hover:text-slate-200 hover:bg-white/[0.04]'}`}>
                {tab.label}
                <span className={`ml-2 text-[10px] font-mono ${activeCity === tab.key ? 'text-gold/80' : 'text-slate-500'}`}>{cityCounts[tab.key]}</span>
              </button>
            ))}
          </div>
        </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          {loading ? (
            ...
            <div className="text-center py-20">
              <div className="max-w-md mx-auto">
                ...
                <p className="text-slate-400 text-sm mb-6">{searchTerm ? `No entities match "${searchTerm}" in the current region.` : 'No entities found for this region.'}</p>
                <Link href="/#scan" className="inline-flex items-center gap-2 bg-gold hover:bg-gold-light text-navy font-bold px-6 py-3 rounded-lg text-sm transition-colors"><Shield size={16} /> Run a Sentry Scan</Link>
              </div>
            </div>
          ) : (
            <div className="bg-white/[0.03] border border-white/[0.08] rounded-xl overflow-hidden">
              ...
              <div className="grid grid-cols-[2fr_1fr_1fr_1fr_1fr_140px] gap-0 bg-white/[0.04] border-b border-white/[0.08] px-5 py-3">
                {['Practice Name','Sovereignty Status','Residency Signal','Transparency Seal','Last Scanned'].map(h => (
                ...
                {providers.map(p => {
                  ...
                  return (
                    <div key={p.id} className="grid grid-cols-[2fr_1fr_1fr_1fr_1fr_140px] gap-0 px-5 py-3.5 items-center hover:bg-white/[0.03] transition-colors">
                      <div>
                        <div className={`font-semibold text-sm ${hr ? 'text-slate-400' : 'text-white'}`}>{hr ? maskName(p.name) : p.name}</div>
                        <div className="text-[11px] text-slate-500 mt-0.5">{p.city || '\u2014'}{p.zip ? `, ${p.zip}` : ''}</div>
                      </div>
                      ...
                      <div className="text-right">
                        {paid ? (
                          ...
                          <button onClick={() => { setClaimModal(p); setClaimStep('verify'); setClaimForm({ name: '', email: '', npi: '' }); }}
                            className="inline-flex items-center gap-1.5 bg-gold/10 hover:bg-gold/20 border border-gold/20 hover:border-gold/40 text-gold text-[11px] font-bold px-3 py-1.5 rounded-lg transition-all">
                            <Eye size={13} /> Claim &amp; Verify
                          </button>
                        )}
                      </div>
                    </div>
              ...
              <div className="flex items-center justify-between px-5 py-3 border-t border-white/[0.06] bg-white/[0.02]">
                <div className="text-[11px] text-slate-400 font-mono">
                  Showing <span className="text-slate-200 font-bold">{showFrom}\u2013{showTo}</span> of <span className="text-slate-200 font-bold">{totalCount.toLocaleString()}</span>
                </div>
                <div className="flex items-center gap-1">
                  <button onClick={() => setPage(p => Math.max(0, p - 1))} disabled={page === 0}
                    className="flex items-center gap-1 px-3 py-1.5 text-[11px] font-bold text-slate-400 hover:text-white bg-white/[0.04] hover:bg-white/[0.08] border border-white/[0.08] rounded-lg transition-all disabled:opacity-30 disabled:cursor-not-allowed">
          ...
          {providers.length > 0 && (
            <div className="text-center mt-10">
              <p className="text-slate-400 text-sm mb-3">Don&apos;t see your practice listed?</p>
      ...
      {claimModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4" onClick={() => setClaimModal(null)}>
          ...
          <div className="relative bg-[#0c1425] border border-white/[0.1] rounded-2xl max-w-lg w-full shadow-2xl" onClick={e => e.stopPropagation()}>
            {claimStep === 'verify' ? (<>
              <div className="px-6 pt-6 pb-4 border-b border-white/[0.08]">
                <div className="flex items-center justify-between mb-4">
                  ...
                  <button onClick={() => setClaimModal(null)} className="text-slate-400 hover:text-white p-1"><X size={18} /></button>
                </div>
              ...
              <div className="px-6 py-4">
                <div className="bg-amber-500/5 border border-amber-500/15 rounded-lg p-4 mb-5">
                  <div className="flex items-start gap-3"><AlertTriangle className="w-5 h-5 text-amber-400 flex-shrink-0 mt-0.5" /><div><p className="text-amber-200 text-sm font-semibold">We have identified potential statutory exposures for this entity.</p><p className="text-slate-400 text-xs mt-1">Please verify your identity to unlock the Preliminary Forensic Scan.</p></div></div>
                </div>
                <div className="space-y-3">
                  ...
                  <div><label className="text-[10px] font-bold uppercase tracking-wider text-slate-400 mb-1.5 block">Work Email</label><input type="email" value={claimForm.email} onChange={e => setClaimForm({...claimForm, email: e.target.value})} placeholder="you@yourpractice.com" className="w-full px-3.5 py-2.5 bg-white/[0.06] border border-white/[0.12] rounded-lg text-white text-sm placeholder:text-slate-500 focus:outline-none focus:border-gold/50 focus:ring-1 focus:ring-gold/20" /><p className="text-[10px] text-slate-500 mt-1">Use your practice email to verify ownership</p></div>
              ...
              <div className="px-6 py-4 border-t border-white/[0.08] flex items-center justify-between">
                ...
                <button onClick={handleClaim} disabled={!claimForm.name || !claimForm.email || claimSubmitting} className="bg-gold hover:bg-gold-light disabled:opacity-40 text-navy font-bold px-5 py-2.5 rounded-lg text-sm transition-colors flex items-center gap-2">
              ...
              <div className="px-6 py-6">
                ...
                <p className="text-slate-400 text-xs mb-5">A summary has been sent to <strong className="text-white">{claimForm.email}</strong>. To access detailed forensic findings:</p>
                <div className="space-y-2.5">
                  <a href="/#scan" className="flex items-center justify-between w-full bg-white/[0.04] hover:bg-white/[0.07] border border-white/[0.1] rounded-xl p-4 transition-all group/btn"><div className="flex items-center gap-3"><FileText className="w-5 h-5 text-gold" /><div><div className="text-white font-bold text-sm">Full Audit Report</div><div className="text-slate-400 text-xs">Deep forensic analysis with legal evidence mapping</div></div></div><div className="text-right"><div className="text-gold font-black text-lg">$149</div></div></a>
