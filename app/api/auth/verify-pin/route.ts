...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!pendingSessions || pendingSessions.length === 0) {
      return NextResponse.json(
        { error: 'Invalid or expired session. Please request a new login link.' },
        { status: 401 },
      );
    }
    ...
    if (new Date(pending.expires_at) < new Date()) {
      return NextResponse.json(
        { error: 'Verification code expired. Please request a new login link.' },
        { status: 401 },
      );
    }
    ...
    if (!pinRecords || pinRecords.length === 0) {
      return NextResponse.json(
        { error: 'Verification data not found. Please request a new login link.' },
        { status: 401 },
      );
    }
    ...
    await supabaseFetch(`dashboard_tokens?token=eq.${encodeURIComponent(pending_session_id)}`, {
      method: 'PATCH',
      body: JSON.stringify({ used_at: new Date().toISOString() }),
    });
    await supabaseFetch(`dashboard_tokens?token=eq.pin:${encodeURIComponent(pending_session_id)}`, {
      method: 'PATCH',
      body: JSON.stringify({ used_at: new Date().toISOString() }),
    });
