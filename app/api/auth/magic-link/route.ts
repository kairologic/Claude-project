...
export async function POST(request: NextRequest) {
  try {
    ...
    const regRes = await supabaseFetch(
      `registry?email=eq.${encodeURIComponent(emailLower)}&select=npi,name,email,subscription_status,is_paid`,
    );
    ...
    return NextResponse.json({
      success: true,
      message:
        'If this email is associated with a KairoLogic account, you will receive a login link.',
    });
  } catch (err) {
