...
function generatePDF(payload: Record<string, unknown>): string {
  ...
  const providerName = safe(
    (payload.providerName as string) || (payload.name as string),
    'Unknown Practice',
  );
  ...
  function bodyText(
    y: number,
    text: string,
    opts?: { bold?: boolean; color?: [number, number, number]; size?: number; maxWidth?: number },
  ): number {
  ...
  y = bodyText(
    y,
    `This Sovereignty Audit Report provides a comprehensive forensic analysis of ${providerName} (NPI: ${npi}) conducted on ${scanDate}. The analysis was conducted using KairoLogic's Check Engine v2 plugin-based scanning system across Data Residency, AI Transparency, and Clinical Integrity domains.`,
  );
  y += 8;
    ...
    didParseCell: (data: {
      ...
      if (data.section === 'body') {
        if (data.column.index <= 1)
          data.cell.styles.textColor = riskColor(score) as unknown as number[];
  ...
  catRows.push([
    'NPI Integrity (Check Engine v2)',
    `${npiPct}%`,
    npiSt,
    `${npiPass}/${npiTotal} pass`,
  ]);
  ...
  y = bodyText(
    y,
    "Check Engine v2 cross-references the provider's website content against the federal NPPES registry to identify credentialing mismatches that could indicate compliance risk or identity discrepancies.",
  );
  y += 6;
  ...
  if (npiVerification.valid) {
    ...
    const nameMatch =
      npiName.toLowerCase().trim() === providerName.toLowerCase().trim() ? 'MATCH' : 'MISMATCH';
    const npiRows = [
      ['Provider Name', npiName, providerName, nameMatch],
      ['NPI Number', safe(npiVerification.npi as string), npi, 'MATCH'],
      [
        'Provider Type',
        safe(npiVerification.type as string),
        safe(npiVerification.type as string),
        'MATCH',
      ],
      [
        'Primary Specialty',
        safe(npiVerification.specialty as string),
        'From website scan',
        'REVIEW',
      ],
    ...
    doc.autoTable({
      ...
      didParseCell: (data: {
        section: string;
        column: { index: number };
        cell: { styles: { textColor: number[] }; raw: unknown };
      }) => {
    ...
    y = bodyText(
      y,
      'NPI verification unavailable. The NPI could not be validated against the NPPES registry.',
      { bold: true },
    );
  }
  ...
  const findingsSections: { prefix: string | string[]; title: string; description: string }[] = [
    {
      ...
      description:
        'Senate Bill 1188 requires all electronic health records for Texas patients to be physically maintained within the United States. These checks verify that your digital infrastructure routes patient data exclusively through domestic endpoints.',
    },
    {
      prefix: 'AI-',
      title: 'DETAILED FINDINGS: AI TRANSPARENCY (HB 149)',
      description:
        'House Bill 149 requires physicians to disclose when AI tools are used in patient care. These checks verify that your website and patient-facing systems provide clear, conspicuous AI disclosures.',
    },
    {
      prefix: 'ER-',
      title: 'DETAILED FINDINGS: CLINICAL INTEGRITY',
      description:
        'These checks verify compliance with EHR system integrity requirements, including proper data field handling and patient access controls mandated by Texas administrative code.',
    },
    {
      prefix: ['NPI-', 'RST-'],
      title: 'CHECK ENGINE v2: NPI INTEGRITY PLUGINS',
      description:
        "These plugin-based checks are unique to KairoLogic's Check Engine v2 and cross-reference your website content against the federal NPPES registry in real time.",
    },
  ];
  ...
  for (let si = 0; si < findingsSections.length; si++) {
    ...
    const fRows = sectionFindings.map((f) => [
      ...
      ((f.status as string) || 'info').toUpperCase() === 'PASS'
        ? 'PASS'
        : ((f.status as string) || 'info').toUpperCase() === 'FAIL'
          ? 'FAIL'
          : 'WARN',
    ...
    doc.autoTable({
      ...
      didParseCell: (data: {
        ...
        column: { index: number };
        row: { index: number };
        cell: { styles: { textColor: number[]; fillColor: number[] }; raw: unknown };
      }) => {
        if (data.section === 'body') {
  ...
  if (dataBorderMap.length > 0) {
    ...
    y = bodyText(
      y,
      'IP geolocation analysis of all endpoints detected during the scan. Foreign endpoints represent potential SB 1188 violations if they process or cache protected health information.',
    );
    y += 6;
  ...
  if (failed.length === 0) {
    y = bodyText(
      y,
      'No compliance failures detected. Your practice meets all scanned requirements.',
    ...
    for (const f of prioritized) {
      ...
      const techFix = safe((f.technicalFix || f.recommended_fix || f.technical_fix) as string, '');
      ...
      if (techFix) {
        ...
        y = bodyText(
          y,
          'Contact KairoLogic engineering for detailed remediation guidance specific to your infrastructure.',
          { size: 8.5 },
        );
      }
  ...
  const criticalCount = findings.filter(
    (f) => f.status === 'fail' && ((f.id as string) || '').match(/^(DR|AI)-/),
  ).length;
  ...
  if (criticalCount > 0) {
    y = bodyText(
      y,
      `1. Address ${criticalCount} CRITICAL item(s) immediately - these represent active statutory violations with potential enforcement exposure under SB 1188 and HB 149.`,
      { bold: true },
    );
  }
  if (highCount > 0) {
    y = bodyText(
      y,
      `2. Schedule ${highCount} HIGH priority item(s) within 7 days to close remaining compliance gaps.`,
      { bold: true },
    );
  }
  if (warnCount > 0) {
    y = bodyText(
      y,
      `3. Review ${warnCount} WARNING(s) within 30 days for comprehensive compliance posture.`,
      { bold: true },
    );
  }
  y = bodyText(
    y,
    '4. Activate Sentry Shield monitoring to detect future drift. Plugin updates, hosting changes, and new third-party scripts can silently re-introduce compliance gaps within weeks of remediation.',
    { bold: true },
  );
  y += 16;
  ...
  try {
    ...
    if (!npi || !findings || !Array.isArray(findings)) {
      return NextResponse.json(
        { error: 'Missing required fields: npi, findings' },
        { status: 400 },
      );
    }
    ...
    const practiceName =
      body.providerName || body.name || body.npiVerification?.name || 'Unknown Practice';
...
export async function GET(request: NextRequest) {
  try {
    ...
    const response = await fetch(`${url}&select=${selectFields}`, {
      headers: {
        apikey: SUPABASE_ANON_KEY,
        Authorization: `Bearer ${SUPABASE_ANON_KEY}`,
      },
    ...
    const msg = error instanceof Error ? error.message : 'Unknown error';
    return NextResponse.json(
      { error: 'Failed to retrieve reports', message: msg },
      { status: 500 },
    );
  }
}
