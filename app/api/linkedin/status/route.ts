...
export async function GET() {
  try {
    ...
    const { data: connections, error } = await supabase
      ...
      .select(
        'account_type, linkedin_name, linkedin_email, organization_id, organization_name, expires_at, is_active, updated_at',
      )
      .eq('is_active', true);
    ...
    const accounts = connections.map((c) => ({
      ...
      name:
        c.account_type === 'organization' ? c.organization_name || 'Company Page' : c.linkedin_name,
