...
export async function POST(request: NextRequest) {
  try {
    ...
    if (previewToken.is_claimed)
      return NextResponse.json({ error: 'This preview has already been claimed' }, { status: 409 });
    ...
    const practices = await db(
      `practice_websites?id=eq.${practiceWebsiteId}&select=id,name,url,state,practice_group_id`,
    );
    if (!practices?.length)
