...
const TEMPLATES: Record<string, EmailTemplate> = {
  ...
  task_overdue: {
    subject: (v) =>
      `Action needed: ${escapeHtml(v.task_title)} overdue — ${escapeHtml(v.provider_name)}`,
    body: (v) =>
      wrapInLayout(`
  ...
  payer_confirmed: {
    subject: (v) =>
      `${escapeHtml(v.payer_name)} listing confirmed — ${escapeHtml(v.provider_name)}`,
    body: (v) =>
      wrapInLayout(`
  ...
  phantom_listing: {
    subject: (v) =>
      `Alert: ${escapeHtml(v.provider_name)} still listed in ${escapeHtml(v.payer_name)} — action needed`,
    body: (v) =>
      wrapInLayout(`
  ...
  phantom_escalation: {
    subject: (v) =>
      `Urgent: ${escapeHtml(v.provider_name)} still listed in ${escapeHtml(v.directory_count)} directories after 60 days`,
    body: (v) =>
      wrapInLayout(`
      <div style="font-size:20px;font-weight:800;color:#dc2626;margin-bottom:8px">
  ...
  try {
    ...
    if (!event || !recipient_email) {
      return NextResponse.json(
        { error: 'event and recipient_email are required' },
        { status: 400 },
      );
    }
    ...
    const sent = await sendViaResend(recipient_email, subject, html, event, provider_npi);
