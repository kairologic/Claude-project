...
function buildResultsSummaryHTML(vars: Record<string, string>): string {
  ...
  const riskLevel =
    vars.risk_level || (score >= 67 ? 'Low Risk' : score >= 34 ? 'Moderate Risk' : 'High Risk');
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (npi) {
      const providerRes = await fetch(`${SUPABASE_URL}/rest/v1/registry?npi=eq.${npi}&limit=1`, {
        headers: { apikey: SUPABASE_KEY, Authorization: `Bearer ${SUPABASE_KEY}` },
      });
      ...
      if (providers && providers.length > 0) {
        ...
        providerVars = {
          ...
          status_label:
            p.status_label || (score >= 75 ? 'Substantial Compliance' : 'Critical Drift'),
    ...
    if (!recipientEmail) {
      console.log(
        '[Email] No recipient email for NPI:',
        npi,
        '| provider email from form:',
        variables.email,
      );
    ...
    console.log(
      `[Email] Sending "${template_slug}" to ${recipientEmail} (${isInternal ? 'admin' : 'provider'})`,
    );
