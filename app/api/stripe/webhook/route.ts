...
// ── Product Types ──
type ProductType = 'audit-report' | 'safe-harbor' | 'sentry-shield' | 'sentry-watch' | 'unknown';
...
function identifyProduct(session: Record<string, unknown>): ProductInfo {
  ...
  if (
    ...
    combined.includes('sentry shield') ||
    combined.includes('sentry-shield')
  ) {
  ...
  if (
    ...
    combined.includes('sentry watch') ||
    combined.includes('sentry-watch')
  ) {
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (event.type === 'checkout.session.completed') {
      ...
      console.log(
        `[Stripe Webhook] Payment: ${customerEmail}, $${amountTotal}, ${product.displayName} (${product.type}), customer: ${customerId}`,
      );
      ...
      if (!provider && customerEmail) {
        const providers = await supabaseGet(
          'registry',
          `email=eq.${encodeURIComponent(customerEmail)}&limit=1`,
        );
        if (providers?.length) provider = providers[0];
      }
      ...
      if (provider) {
        ...
        if (!provider.dashboard_token) {
          ...
          registryUpdate.dashboard_token = Array.from(tokenBytes)
            .map((b) => b.toString(16).padStart(2, '0'))
            .join('');
        }
        _dashToken =
          (registryUpdate.dashboard_token as string) || (provider.dashboard_token as string) || '';

        await supabasePatch('registry', `id=eq.${provider.id}`, registryUpdate);
        console.log(
          `[Stripe Webhook] Registry updated: ${provider.name} (${provider.npi}) — ${product.type}${product.includesShieldTrial ? ' (90-day Shield trial)' : ''}`,
        );
      } else {
      ...
      if (provider && customerEmail) {
        try {
          const confirmRes = await fetch(
            `${process.env.NEXT_PUBLIC_BASE_URL || 'https://kairologic.net'}/api/email/purchase-confirmation`,
            {
              method: 'POST',
              headers: {
                'Content-Type': 'application/json',
      ...
      if (product.includesShieldTrial && customerId) {
        ...
        console.log(
          `[Stripe Webhook] Creating Shield subscription with ${SHIELD_TRIAL_DAYS}-day trial for customer ${customerId}...`,
        );
        ...
        if (subscriptionResult.subscriptionId) {
          console.log(
            `[Stripe Webhook] Shield trial subscription created: ${subscriptionResult.subscriptionId}`,
          );
          if (provider) {
          ...
          console.error(
            `[Stripe Webhook] Shield trial creation failed: ${subscriptionResult.error}`,
          );
          if (provider) {
            await supabasePatch('registry', `id=eq.${provider.id}`, {
      ...
      if (customerEmail) {
        try {
          const origin =
            request.headers.get('origin') || request.nextUrl.origin || 'https://kairologic.com';
          await fetch(`${origin}/api/email/send`, {
            ...
            body: JSON.stringify({
              ...
              variables: {
                ...
                dashboard_url: _dashToken
                  ? `${origin}/dashboard/${(provider?.npi as string) || npi}?token=${_dashToken}`
                  : '',
    ...
    if (
      ...
      if (subNpi) {
        ...
        console.log(
          `[Stripe Webhook] Subscription ${event.type}: NPI ${subNpi} → ${subscriptionStatus}${subscriptionTier ? ` (${subscriptionTier})` : ''}`,
        );
      }
