  ...
  try {
    ...
    if (existingOrgs?.length > 0) {
      ...
      if (org.trial_status === 'ACTIVE' || org.trial_status === 'EXPIRING') {
        // Find their practice
        const practices = await db(`practice_websites?organization_id=eq.${org.id}&select=id,name`);
        if (practices?.length > 0) {
          ...
          return NextResponse.json({
            ...
            trial_days_remaining: Math.max(
              0,
              Math.ceil((new Date(org.trial_end).getTime() - Date.now()) / 86400000),
            ),
          });
        }
      }
    ...
    try {
      const providers = await db(
        `providers?npi=eq.${npi}&select=npi,first_name,last_name,address_line_1,city,state,zip_code,taxonomy_desc`,
      );
      if (providers?.length > 0) providerData = providers[0];
    } catch {
      /* NPI not in our DB yet — that's fine */
    }
