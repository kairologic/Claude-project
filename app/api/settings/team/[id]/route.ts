...
import { withAuth, API_ERRORS, isValidUUID } from '@/lib/api/with-auth';
 ...
 */
const DELETE_HANDLER = withAuth(async (request: NextRequest, ctx: AuthContext, params?: any) => {
  try {
    ...
    // Delete team member
    const { error } = await ctx.supabase.from('practice_team_members').delete().eq('id', id);
 ...
 * Uses withAuth + manual practice access check (fetches practice_id from team member record)
 */
const PATCH_HANDLER = withAuth(async (request: NextRequest, ctx: AuthContext, params?: any) => {
  try {
    const resolvedParams = params?.params instanceof Promise ? await params.params : params?.params;
    const { id } = resolvedParams || {};
