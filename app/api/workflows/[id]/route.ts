...
const GET_HANDLER = withAuth(
  async (
    ...
    const [workflowRes, tasksRes, eventsRes] = await Promise.all([
      supabase.from('workflow_instances').select('*').eq('id', id).single(),
      supabase
        .from('workflow_tasks')
        .select('*')
        .eq('workflow_id', id)
...
const PATCH_HANDLER = withAuth(
  async (
    ...
    if (body.status && oldStatus && body.status !== oldStatus) {
      const transition = canTransitionWorkflow(oldStatus, body.status as WorkflowStatus);
      if (!transition.allowed) {
        return API_ERRORS.badRequest(transition.reason || 'Invalid status transition', {
