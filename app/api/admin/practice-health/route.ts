...
function detectIssues(p: PracticeHealth): DashboardIssue[] {
  ...
  if (p.scan_status === 'error' || p.scan_status === 'unreachable') {
    issues.push({
      type: 'scan_error',
      severity: 'error',
      message: `Scan status: ${p.scan_status} (${p.consecutive_errors} consecutive errors)`,
    });
  }
  ...
  if (p.last_scan_at) {
    ...
    if (daysSinceScan > staleDays) {
      issues.push({
        type: 'scan_stale',
        severity: 'warning',
        message: `Last scan ${Math.round(daysSinceScan)} days ago (${p.scan_tier} tier)`,
      });
    }
  } else {
  ...
  if (p.providers_total > 0 && !p.last_payer_sync_at) {
    issues.push({
      type: 'no_payer_sync',
      severity: 'info',
      message: 'Payer directory sync not yet run',
    });
  }
  ...
  if (p.mismatch_count > 0 && p.providers_total > 0) {
    ...
    if (pct > 50) {
      issues.push({
        type: 'high_mismatch',
        severity: 'error',
        message: `${pct}% of providers have mismatches`,
      });
    }
  }
