...
const GET_HANDLER = withAuth(async (request: NextRequest, ctx) => {
  try {
    ...
    const endpointMap = new Map<string, { payer_code: string; payer_name: string; auth_type: string }>(
    ...
    const dataMismatches = mismatches.filter((m: any) => m.mismatch_type === 'value_differs').length;
    const payerGaps = mismatches.filter((m: any) =>
      m.mismatch_type === 'acceptance_gap' || m.mismatch_type === 'not_listed'
    ).length;
