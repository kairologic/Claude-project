...
const POST_HANDLER = withAuth(async (request: NextRequest, ctx) => {
  ...
  async function supabaseFindByNpi(npi: string): Promise<Record<string, unknown> | null> {
    try {
      const { data, error } = await supabase.from('registry').select('*').eq('npi', npi).limit(1);
  ...
  try {
    ...
    const batchSize = Math.min(
      parseInt(request.nextUrl.searchParams.get('batch_size') || '10'),
      25,
    );
    ...
    for (let i = 0; i < batch.length; i++) {
      ...
      if (!scanData || scanData.error) {
        result.error =
          (scanData?.message as string) || (scanData?.error as string) || 'Scan failed';
