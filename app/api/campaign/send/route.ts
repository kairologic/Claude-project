...
function extractFindingSummary(scan: ScanResult, findingText: string): string {
  ...
  if (dr01?.status === 'fail' && dr01.evidence) {
    ...
    parts.push(
      `Your primary domain (${ip}) resolves to a server geolocated in ${geo}, routing patient data outside the US.`,
    );
  }

  if (hbFails.length > 0) {
    parts.push(
      `${hbFails.length} AI transparency disclosure${hbFails.length > 1 ? 's are' : ' is'} missing (required by HB 149).`,
    );
  }
  ...
  if (parts.length === 0) {
    if (sbFails.length + hbFails.length > 0) {
      parts.push(
        `We identified ${sbFails.length + hbFails.length} compliance gap${sbFails.length + hbFails.length !== 1 ? 's' : ''} on your website that may require remediation under Texas law.`,
      );
    } else {
      parts.push(
        `Our scan detected areas on your website that may not fully comply with Texas SB 1188 data sovereignty requirements.`,
      );
    }
  }
