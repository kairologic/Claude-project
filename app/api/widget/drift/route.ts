...
const SEVERITY_MAP: Record<string, Record<string, string>> = {
  ...
  third_party_scripts: {
    ...
    content_changed: 'high',
    content_added: 'high',
  },
  data_collection_forms: {
    content_removed: 'medium',
    content_changed: 'high',
    content_added: 'medium',
  },
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    if (!res.ok) {
      const errText = await res.text();
      return NextResponse.json(
        { error: 'Failed to fetch drift events', detail: errText },
        { status: 500 },
      );
    }
...
async function triggerAlert(
  ...
  // Always notify admin
  console.log(
    `[DRIFT ALERT] ${severity.toUpperCase()}: NPI ${npi} — ${categoryLabels[category] || category} ${driftTypeLabels[driftType] || driftType} on ${pageUrl}`,
  );
