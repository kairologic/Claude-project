  ...
  try {
    ...
    if (mode === 'data_driven') {
      ...
      if (mr && mr.total_practices) {
        parts.push(
          `MISMATCH OVERVIEW: ${mr.mismatch_pct}% of ${mr.total_practices} practices have data mismatches (${mr.total_mismatches} total mismatches). Worst single practice has ${mr.worst_mismatch_count} mismatches.`,
        );
      }
      ...
      if (payer?.length) {
        const payerLines = payer
          .map(
            (p) => `${p.field_name} (${p.mismatch_type}): ${p.count} cases, priority ${p.priority}`,
          )
          .join('; ');
