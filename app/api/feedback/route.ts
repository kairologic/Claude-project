...
// Supabase REST API (anon key pattern)
 ...
 * Sends an email notification AND persists the record to Supabase.
 */
      ...
      try {
        const insertRes = await fetch(`${SUPABASE_URL}/rest/v1/feedback`, {
          ...
          body: JSON.stringify({
            ...
            subject,
            description,
            urgency: isIssue ? (urgency || null) : null,
            submitted_by: contactEmail ? `${userName} <${contactEmail}>` : userName,
          }),
        });

        if (insertRes.ok) {
          const rows = await insertRes.json();
          feedbackId = Array.isArray(rows) && rows.length > 0 ? rows[0].id : null;
          console.log(`[Feedback] Persisted to Supabase, id: ${feedbackId}`);
        } else {
          const errText = await insertRes.text();
          console.error(`[Feedback] Supabase insert failed (${insertRes.status}): ${errText}`);
        }
      } catch (dbErr) {
        // Non-fatal: email already sent
        console.error('[Feedback] Supabase insert error:', dbErr);
      }
