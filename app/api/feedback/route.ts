 ...
 * Now also persists to the feedback table in Supabase for tracking.
 */
const POST_HANDLER = withAuth(async (request: NextRequest, ctx: AuthContext) => {
  try {
    ...
    const {
      type, category, urgency, subject, description,
      contactEmail, userName, practiceId, practiceName,
    } = parsed.data;

    if (!userName || !practiceId || !practiceName) {
      return API_ERRORS.badRequest('Missing submitter context (userName, practiceId, practiceName)');
    }
    ...
    try {
      ...
      if (fbError) {
        console.error('[Feedback] Supabase insert error:', fbError);
      } else {
        feedbackId = fbData?.id || null;
        console.log('[Feedback] Saved to DB with id:', feedbackId);
      }
    } catch (dbErr) {
      console.error('[Feedback] DB persistence failed (email will still send):', dbErr);
    }
...
${feedbackId ? `Tracking ID:   ${feedbackId}` : ''}
...
<div class="container">
  ...
  <div class="body">
    ...
    ${isIssue && urgency ? `<div class="field"><div class="lbl">Urgency</div><div class="val"><span class="urgency-badge urgency-${escapeHtml(urgency)}">${escapeHtml(urgency).toUpperCase()}</span></div></div>` : ''}
    <div class="field">
    ...
    ${feedbackId ? `<div class="field"><div class="lbl">Tracking ID</div><div class="val" style="font-family: monospace; font-size: 12px; color: #9AA3AE;">${feedbackId}</div></div>` : ''}
  </div>
  <div class="footer">
    ...
    console.log(`[Feedback] ${typeLabel} sent to ${toEmail} \u2014 practice: ${practiceName}, user: ${userName}`);
...
export { POST_HANDLER as POST };import { NextRequest, NextResponse } from 'next/server';
import { withAuth, API_ERRORS, escapeHtml, parseBody } from '@/lib/api/with-auth';
import type { AuthContext } from '@/lib/api/with-auth';
import { feedbackSchema } from '@/lib/api/validation-schemas';
