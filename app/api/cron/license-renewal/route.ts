...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const { data: licenses, error: licErr } = await supabase
      ...
      .select('id, npi, licensee_name, license_number, state, license_type, license_status, expiration_date')
            ...
            if (!existingWf || existingWf.length === 0) {
              ...
              const { data: wfRows } = await supabase
                ...
                .insert({
                  ...
                  overdue_at: isExpired
                    ...
                    : new Date(Date.now() + (daysUntilExpiry - 7) * 86_400_000).toISOString().split('T')[0],
                })
                .select();
              ...
              if (wfRows && wfRows.length > 0) {
                ...
                const tasks = LICENSE_RENEWAL_TASKS.map(t => ({
                  ...
                  metadata: t.task_type === 'review_license' ? {
                    license_number: lic.license_number,
                    state: lic.state,
                    license_type: lic.license_type,
                    expiration_date: lic.expiration_date,
