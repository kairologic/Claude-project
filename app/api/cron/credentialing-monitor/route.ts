...
export async function GET(req: NextRequest) {
  ...
  async function processMonitorAutoConfirm(task: MonitorTask, wf: WorkflowInstance, log: string[]) {
    ...
    const submitTask = siblingTasks.find(
      (t: any) => t.task_type === 'submit_nppes' || t.task_type === 'download_submit',
    );
    ...
    const nppsAddress = normalizeAddress(
      `${provider.address_line_1}, ${provider.city}, ${provider.state} ${provider.zip_code}`,
    );
    const expected = normalizeAddress(expectedValue);

    if (
      nppsAddress.includes(expected) ||
      expected.includes(nppsAddress) ||
      fuzzyAddressMatch(nppsAddress, expected)
    ) {
  ...
  async function processMonitorNppes(task: MonitorTask, wf: WorkflowInstance, log: string[]) {
    ...
    const practiceAddr = normalizeAddress(
    ...
    if (fuzzyAddressMatch(nppsAddr, practiceAddr)) {
      ...
      await logWorkflowEvent(
        wf.id,
        'nppes_confirmed',
        'NPPES address matches practice — auto-completed',
        {
      ...
      log.push(
        `  ✗ NPPES mismatch: ${wf.provider_npi} — "${nppsAddr}" vs practice "${practiceAddr}"`,
      );
      await updateCheckTimestamp(task.id, task.metadata);
    }
  }
  ...
  async function processMonitorPayerDirectory(
    task: MonitorTask,
    wf: WorkflowInstance,
    log: string[],
  ) {
    ...
    const snapAddr = normalizeAddress(
    ...
    if (practice && fuzzyAddressMatch(snapAddr, practiceAddr)) {
      ...
      log.push(
        `  ✓ ${payerLabel} directory confirmed: ${wf.provider_npi} listed at practice address`,
      );
      ...
      await logWorkflowEvent(
        wf.id,
        'payer_confirmed',
        `${payerLabel} directory listing confirmed`,
        {
          payer: payerCode,
          address: snapAddr,
        },
      ...
      log.push(
        `  ✗ ${payerCode} address mismatch for ${wf.provider_npi}: "${snapAddr}" vs "${practiceAddr}"`,
      );
      await updateCheckTimestamp(task.id, task.metadata);
    }
  }
  ...
  async function processMonitorPhantom(task: MonitorTask, wf: WorkflowInstance, log: string[]) {
      ...
      if (source === 'nppes') {
        ...
        if (providers.length > 0) {
          const addr = normalizeAddress(
            `${providers[0].address_line_1}, ${providers[0].city}, ${providers[0].state}`,
          );
    ...
    log.push(
      `  Phantom check ${wf.provider_npi}: cleared=[${cleared}], still_listed=[${stillListed}], day ${daysSinceCreated}/90`,
    );
    ...
    if (stillListed.length === 0 || daysSinceCreated >= 90) {
      ...
      log.push(
        `  ${allClear ? '✓' : '⚠'} Phantom monitoring complete for ${wf.provider_npi} — ${allClear ? 'all clear' : `${stillListed.length} still listed`}`,
      );
  ...
  async function checkWorkflowCompletion(
    ...
    await logWorkflowEvent(
      wf.id,
      'workflow_completed',
      `${expectedType} workflow auto-completed — all tasks verified`,
      {},
    );
