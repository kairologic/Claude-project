...
function DynamicCTA({ score, npi }: { score: number; npi: string }) {
  ...
  if (isLowScore) {
    // LOW SCORE VIEW (< 75): Safety Orange header
    return (
      <section className="py-16" style={{ background: 'linear-gradient(135deg, #FF8C00, #e07800)' }}>
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <div className="inline-flex items-center gap-2 bg-white/20 px-4 py-2 rounded-full mb-6">
            <AlertTriangle className="w-5 h-5 text-white" />
            <span className="text-white font-bold text-sm uppercase tracking-wider">Immediate Action Required</span>
          </div>
          <h2 className="text-4xl font-display font-bold text-white mb-4">
            Critical Violations Detected
          </h2>
          <p className="text-lg text-white/90 mb-10 max-w-2xl mx-auto">
            Your practice scored <strong>{score}/100</strong>. Statutory penalties for knowing violations can reach $250,000. 
