...
function WidgetTestInner() {
  ...
  const status =
    ...
    (score >= 80 ? 'Sovereign' : score >= 50 ? 'Drift' : 'Violation');
  const scoreColor = score >= 80 ? '#16a34a' : score >= 50 ? '#d97706' : '#dc2626';
  const practiceName = provider?.name || 'Sample Healthcare Practice';

  const viewportWidth =
    viewport === 'desktop' ? 'max-w-4xl' : viewport === 'tablet' ? 'max-w-lg' : 'max-w-sm';
    ...
    <div className="min-h-screen bg-gray-100">
      ...
      <div className="bg-white border-b border-slate-200 px-4 py-3 flex items-center justify-between sticky top-0 z-10">
        <div className="flex items-center gap-3">
          <Link
            href={`/payment/success?npi=${npi}&product=${mode === 'shield' ? 'safe-harbor-shield' : 'safe-harbor-watch'}`}
          ...
          <div>
            ...
            <p className="text-[10px] text-slate-400">
              Sentry {mode === 'shield' ? 'Shield' : 'Watch'} • NPI: {npi || 'Demo'}
            </p>
          </div>
        </div>
          ...
          <div className="p-6 sm:p-10">
            ...
            <div className="flex items-center justify-between mb-8">
              ...
              <div className="flex gap-3">
                ...
                <div className="bg-slate-100 rounded px-3 py-1.5 text-xs text-slate-500">
                  Services
                </div>
                <div className="bg-slate-100 rounded px-3 py-1.5 text-xs text-slate-500">
                  Contact
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-navy text-white py-3 px-4 text-center text-xs">
        This is a preview. The actual widget will be rendered by the script tag on your live
        website.
        <Link
          href={`/payment/success?npi=${npi}&product=${mode}`}
