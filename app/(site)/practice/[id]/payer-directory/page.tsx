...
export default async function PayerDirectoryPage({ params }: { params: { id: string } }) {
  ...
  const { data: snapshots } =
    npiList.length > 0
      ? await admin
          ...
          .select(
            'npi, payer_code, snapshot_date, listed_name_full, listed_address_line1, listed_city, listed_state, listed_zip, listed_phone, listed_specialty_display, listed_accepting_patients',
          )
          .in('npi', npiList)
  ...
  const { data: nppesData } =
    npiList.length > 0
      ? await admin
          .from('providers')
          .select(
            'npi, first_name, last_name, address_line_1, city, state, zip_code, phone, primary_taxonomy_code, taxonomy_desc',
          )
          .in('npi', npiList)
      : { data: [] };
  ...
  const providerRows = (providers || []).map((p) => {
    ...
    return {
      ...
      nppes_address: nppes
        ? [nppes.address_line_1, nppes.city, nppes.state, nppes.zip_code?.slice(0, 5)]
            .filter(Boolean)
            .join(', ')
        : null,
...
function deduplicateSnapshots<T extends { npi: string; payer_code: string }>(snapshots: T[]): T[] {
