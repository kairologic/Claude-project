...
import { 
  ...
  BarChart3, TrendingUp, AlertCircle, Loader2, X, Save, LogOut, FileText, Package, Zap, Pause, FileWarning, UserPlus, Sparkles
} from 'lucide-react';
...
export default function AdminDashboard() {
  ...
  const handleDownloadReport = async (provider: Registry) => {
    ...
    try {
      ...
      const attText = `This document certifies that ${provider.name || 'the above-named practice'} (NPI: ${provider.npi}) has been assessed for compliance with Texas SB 1188 (Data Sovereignty) and HB 149 (AI Transparency) as of ${reportDate}. The findings herein reflect the observable digital infrastructure at the time of scan and do not constitute legal advice or guarantee of regulatory compliance.`;
      const attLines = doc.splitTextToSize(attText, contentW - 16);
      doc.text(attLines, margin + 8, y + 17);
      doc.setDrawColor(...gold); doc.setLineWidth(0.3);
      doc.line(margin + 10, y + 40, margin + 55, y + 40);
      doc.setFontSize(6); doc.setTextColor(...gold);
      doc.text('KairoLogic Compliance Engine', margin + 12, y + 44);
      doc.setFontSize(5.5); doc.setTextColor(160, 170, 190);
      doc.text(`Engine: ${fullReport?.engine_version || 'SENTRY-3.1.0'}`, margin + 12, y + 48);
      doc.line(pageW - margin - 55, y + 40, pageW - margin - 10, y + 40);
      doc.setFontSize(6); doc.setTextColor(...gold);
      doc.text('Date', pageW - margin - 53, y + 44);
      doc.setFontSize(5.5); doc.setTextColor(160, 170, 190);
      doc.text(reportDate, pageW - margin - 53, y + 48);
      y += 58;
      ...
      nextSteps.forEach((step) => {
        ...
        doc.setFontSize(7); doc.setFont('helvetica', 'bold'); doc.setTextColor(255, 255, 255);
        doc.text(step.num, margin + 4, y + 3.5, { align: 'center' });
        doc.setFontSize(8); doc.setFont('helvetica', 'bold'); doc.setTextColor(...navy);
        doc.text(step.title, margin + 12, y + 3);
        doc.setFontSize(7); doc.setFont('helvetica', 'normal'); doc.setTextColor(...gray);
        doc.text(step.desc, margin + 12, y + 8);
        y += 14;
      });
      y += 6;
      ...
      const discLines = doc.splitTextToSize(disclaimerText, contentW - 10);
      doc.text(discLines, margin + 5, y + 5);
