    ...
    const fetchFeedback = async () => {
      try {
        ...
        const newStats: Stats = {
          ...
          in_progress: feedbackWithPracticeNames.filter((f) => f.status === 'in_progress').length,
    ...
    <div style={{ padding: spacing['4xl'], maxWidth: 1400 }}>
      ...
      >
        ...
        <div>
          ...
          <div style={{ display: 'flex', gap: spacing.sm, flexWrap: 'wrap' }}>
            {(['all', 'new', 'reviewed', 'in_progress', 'resolved', 'closed'] as const).map(
              (status) => (
                <button
                  key={status}
              ...
              >
                ...
                <div style={{ position: 'relative' }}>
                  ...
                  {dropdownOpen === item.id && (
                    ...
                    >
                      {(['new', 'reviewed', 'in_progress', 'resolved', 'closed'] as const).map(
                        (status) => (
                          <button
                            key={status}
                            ...
                            style={{
                              ...
                              color:
                                item.status === status ? statusConfig[status].color : colors.navy,
                              border: 'none',
                              textAlign: 'left',
                              ...typography.body,
                              cursor: 'pointer',
