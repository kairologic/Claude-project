...
export default function ContentStudioPage() {
    ...
    try {
      const res = await fetch(`/api/content-studio/schedule/${id}`, {
        ...
        body: JSON.stringify({
          ...
          channels,
          channel_schedules: channelSchedules,
        }),
      });
      if (res.ok) await fetchPosts();
      else {
  ...
  const handleDisconnectLinkedIn = async (accountType?: string) => {
    const label =
      accountType === 'organization' ? 'KairoLogic company page' : 'your personal LinkedIn';
              ...
              <div className="hidden md:flex items-center gap-4 text-xs text-slate-500">
                ...
                {stats.practices_with_mismatches != null && (
                  <span className="text-amber-600">
                    {stats.practices_with_mismatches} mismatches
                  </span>
                )}
              </div>
            )}
                      ...
                      <div className="absolute top-full right-0 mt-1 w-64 bg-white rounded-lg shadow-lg border border-slate-200 z-50 py-1">
                        ...
                        {linkedInStatus.accounts.map((account) => (
                          <button
                            ...
                            onClick={() => {
                              setLinkedInAccount(
                                account.account_type as 'personal' | 'organization',
                              );
                        ...
                        <div className="border-t border-slate-100 mt-1 pt-1">
                          ...
                          {linkedInStatus.accounts.map((account) => (
                            <button
                              ...
                              onClick={() => {
                                handleDisconnectLinkedIn(account.account_type);
                                setShowAccountPicker(false);
                              }}
