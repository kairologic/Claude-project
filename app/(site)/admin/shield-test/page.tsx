...
const BASELINE_CONTENT: Record<string, string> = {
  ai_disclosure:
    'this practice uses ai-powered tools for appointment scheduling and preliminary symptom assessment. all ai-assisted decisions are reviewed by licensed healthcare professionals.',
  ...
  data_collection_forms:
    'https://acme-family-medicine.example.com/api/intake::email,patient_name,patient_phone',
...
const DRIFT_SCENARIOS = [
    ...
    drifts: [
      {
        ...
        content_after:
          'https://acme-family-medicine.example.com/api/intake::email,patient_name,patient_phone,patient_ssn',
      },
    ],
  },
  {
  ...
  },
  {
    ...
    description:
      'AI disclosure removed + new scripts + form action changed — simulates a bad deploy',
    drifts: [
      {
        category: 'ai_disclosure',
        drift_type: 'content_removed',
      ...
      },
      {
        ...
        content_before: BASELINE_CONTENT.data_collection_forms,
        content_after:
          'https://unknown-server.com/collect::email,patient_name,patient_phone,patient_ssn,credit_card',
      },
    ],
  },
];
...
export default function ShieldTestPage() {
  ...
  async function createProviderDirect() {
    try {
      ...
      addLog(
        `API connectivity check: ${checkRes.ok ? 'OK' : 'Failed'}`,
        checkRes.ok ? 'success' : 'error',
      );

      addLog('', 'info');
      addLog(
        '⚠️ Cannot create provider directly from browser. Please run this SQL in Supabase:',
        'warn',
      );
      addLog('', 'info');
      addLog(
        `INSERT INTO registry (id, npi, name, url, email, city, zip, provider_type, subscription_tier, subscription_status, widget_status, is_visible, is_paid, risk_score, status_label)`,
        'sql',
      );
      addLog(
        `VALUES ('TEST-${Date.now()}', '${TEST_NPI}', '${TEST_PROVIDER.name}', '${TEST_PROVIDER.url}', '${TEST_PROVIDER.email}', '${TEST_PROVIDER.city}', '${TEST_PROVIDER.zip}', '${TEST_PROVIDER.provider_type}', 'shield', 'active', 'active', true, true, 85, 'Verified Sovereign')`,
        'sql',
      );
      addLog(
        `ON CONFLICT (npi) DO UPDATE SET subscription_tier = 'shield', subscription_status = 'active', widget_status = 'active', is_paid = true, risk_score = 85, status_label = 'Verified Sovereign';`,
        'sql',
      );
    } catch (e) {
  ...
  async function triggerDrift(scenarioId: string) {
    ...
    try {
      ...
      for (const drift of scenario.drifts) {
        ...
        const currentHash = drift.content_after
          ? await sha256(drift.content_after)
          : await sha256('');
      ...
      if (data.ok) {
        addLog(
          `✅ Drift reported: ${data.inserted} events inserted, ${data.deduplicated} deduplicated`,
          'success',
        );
        if (data.inserted > 0) {
  ...
  async function checkStatus() {
    ...
    try {
      ...
      addLog(
        `📋 Baselines: ${baseCount} categories ${baseCount > 0 ? '✅' : '❌ (need to seed)'}`,
        baseCount > 0 ? 'success' : 'warn',
      );
      ...
      const newCount = driftData.events
        ? driftData.events.filter((e: any) => e.status === 'new').length
        : 0;
      addLog(
        `🚨 Drift events: ${eventCount} total, ${newCount} new/open`,
        eventCount > 0 ? 'warn' : 'success',
      );
      ...
      if (dashData.error === 'access_denied') {
        addLog(
          `🔒 Shield dashboard: ACCESS DENIED — provider needs subscription_tier = 'shield'`,
          'error',
        );
        addLog(
          `   Run this SQL: UPDATE registry SET subscription_tier = 'shield' WHERE npi = '${TEST_NPI}';`,
          'sql',
        );
        ...
        addLog(
          `✅ Shield dashboard: accessible (tier: ${dashData.provider.subscription_tier}, access: ${dashData.provider.access_type})`,
          'success',
        );
  ...
  function loadWidgetPreview(mode: string) {
    return () => {
      ...
      script.onload = () => {
        addLog(
          `✅ ${mode.toUpperCase()} widget loaded — click the badge in the bottom-right corner`,
          'success',
        );
        addLog(
          `   Click the badge to expand and see the drift nudge (if drift events exist)`,
          'info',
        );
  ...
  function removeWidgetPreview() {
    ...
    if (zone) {
      zone.innerHTML =
        '<p class="text-[10px] text-slate-400">Widget will appear in the bottom-right corner of this page after loading.</p>';
    }
  }
      ...
      <div className="max-w-4xl mx-auto space-y-4">
        ...
        <div className="bg-gradient-to-r from-[#0A192F] to-[#1a365d] text-white rounded-2xl p-6">
          ...
          <p className="text-xs text-slate-400">
            Test the complete drift detection pipeline: widget → API → database → admin dashboard →
            provider dashboard
          </p>
          <div className="mt-3 flex items-center gap-2">
            <span className="text-[10px] bg-white/10 px-2 py-1 rounded font-mono">
              Test NPI: {TEST_NPI}
            </span>
            <span className="text-[10px] bg-white/10 px-2 py-1 rounded font-mono">
        ...
        <div className="bg-white rounded-xl border border-slate-100 shadow-sm p-5">
          ...
          <div className="border border-slate-200 rounded-lg p-4 mb-3">
            ...
            <p className="text-[10px] text-slate-500 ml-8">
              Creates &ldquo;{TEST_PROVIDER.name}&rdquo; in the registry with
              subscription_tier=&lsquo;shield&rsquo;. If API fails, SQL will be shown to run
              manually.
            </p>
          </div>
          ...
          <div className="border border-slate-200 rounded-lg p-4 mb-3">
            ...
            <p className="text-[10px] text-slate-500 ml-8">
              Creates the &ldquo;known good&rdquo; baseline for 7 compliance categories. This is
              normally done after a full scan.
            </p>
          </div>
          ...
          <div className="border border-slate-200 rounded-lg p-4 mb-3">
            ...
            <p className="text-[10px] text-slate-500 ml-8">
              Simulates the widget phoning home. Creates a heartbeat record so the widget shows as
              &ldquo;LIVE&rdquo; in dashboards.
            </p>
          </div>
          ...
          <div className="border border-amber-200 bg-amber-50/50 rounded-lg p-4 mb-3">
            ...
            <p className="text-[10px] text-slate-500 ml-8 mb-3">
              Simulate compliance changes. Each scenario sends drift events to the API just like the
              real widget would.
            </p>
            <div className="ml-8 space-y-2">
              {DRIFT_SCENARIOS.map((scenario) => (
                ...
                >
                  ...
                  <button
                    onClick={() => triggerDrift(scenario.id)}
                    disabled={running}
          ...
          <div className="border border-indigo-200 bg-indigo-50/50 rounded-lg p-4 mb-3">
            ...
            <p className="text-[10px] text-slate-500 ml-8 mb-3">
              Load the actual sentry.js widget on this page to see the trust badge, drift nudge, and
              upgrade CTA. Toggle between Watch and Shield to see the different experiences. After
              triggering drift events (Step 4), click the badge to see the drift count appear.
            </p>
            <div className="ml-8 flex flex-wrap gap-2 mb-3">
              <button
                onClick={loadWidgetPreview('watch')}
                disabled={running}
              ...
              <button
                onClick={loadWidgetPreview('shield')}
                disabled={running}
            ...
            <div
              className="ml-8 bg-white rounded-lg border border-slate-200 p-4 text-center min-h-[80px] flex items-center justify-center"
              id="widget-preview-zone"
            >
              <p className="text-[10px] text-slate-400">
                Widget will appear in the bottom-right corner of this page after loading.
              </p>
            </div>
          </div>
        ...
        <div className="bg-white rounded-xl border border-slate-100 shadow-sm overflow-hidden">
          <div className="px-4 py-3 border-b border-slate-100 flex items-center justify-between">
            ...
            <button onClick={clearLog} className="text-[10px] text-slate-400 hover:text-slate-600">
              Clear
            </button>
          </div>
          <div
            className="bg-[#0f172a] p-4 max-h-[400px] overflow-y-auto font-mono text-[11px] leading-relaxed"
            style={{ minHeight: '120px' }}
          >
            {log.length === 0 ? (
              <span className="text-slate-500">
                Run steps 1-5 above to test the drift engine...
              </span>
            ) : (
              log.map((entry, i) => (
                <div
                      ...
                      : entry.type === 'error'
                        ...
                        : entry.type === 'warn'
                          ...
                          : entry.type === 'drift'
                            ...
                            : entry.type === 'sql'
                              ? 'text-cyan-300 bg-cyan-900/20 px-2 py-0.5 rounded my-0.5 inline-block'
                              : 'text-slate-400'
                  }
                >
        ...
        <div className="bg-white rounded-xl border border-slate-100 shadow-sm p-5">
          <h2 className="text-xs font-bold text-slate-600 uppercase tracking-wider mb-3">
            Quick Reference: End-to-End Test Flow
          </h2>
          <div className="text-[11px] text-slate-600 space-y-3">
            <div className="flex gap-3">
              ...
              <div>
                <strong>Create test provider</strong> — Inserts a provider with NPI {TEST_NPI} and
                subscription_tier=&lsquo;shield&rsquo; into the registry. If the button fails (no
                admin API for inserts), copy the SQL from the log and run it in Supabase SQL Editor.
              </div>
            </div>
            <div className="flex gap-3">
              ...
              <div>
                <strong>Seed baselines</strong> — POSTs compliance content hashes to{' '}
                <code className="bg-slate-100 px-1 rounded">/api/widget/baseline</code>. This
                establishes the &ldquo;known good&rdquo; state that drift is measured against.
                Without baselines, no drift can be detected.
              </div>
            </div>
            <div className="flex gap-3">
              ...
              <div>
                <strong>Send heartbeat</strong> — POSTs to{' '}
                <code className="bg-slate-100 px-1 rounded">/api/widget/heartbeat</code>. Creates a
                heartbeat record so the widget shows as &ldquo;LIVE&rdquo; in both Admin Drift tab
                and Provider Shield dashboard.
              </div>
            </div>
            <div className="flex gap-3">
              ...
              <div>
                <strong>Trigger drift</strong> — POSTs simulated drift events to{' '}
                <code className="bg-slate-100 px-1 rounded">/api/widget/drift</code>. Each scenario
                mimics a real compliance change. The API assigns severity, deduplicates, and
                triggers alerts for critical/high events. Events are deduplicated within 1 hour —
                wait or use a different scenario to see new events.
              </div>
            </div>
            <div className="flex gap-3">
              ...
              <div>
                ...
                <ul className="mt-1 space-y-1 list-disc list-inside text-slate-500">
                  <li>
                    <strong>Admin Dashboard → Drift tab:</strong> See all events, heartbeats,
                    acknowledge/resolve events, bulk resolve
                  </li>
                  <li>
                    <strong>Provider Dashboard (/dashboard/{TEST_NPI}):</strong> See compliance
                    score, open alerts, widget status, event details, monitored categories
                  </li>
                  <li>
                    In Admin, try changing an event status to &ldquo;resolved&rdquo; — it should
                    move to the Resolved filter in both dashboards
                  </li>
                </ul>
              </div>
            </div>
