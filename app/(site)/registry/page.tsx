...
const CITY_TABS = [
  { key: 'all' as const, label: 'All Regions', filter: [] as string[] },
  {
    key: 'austin' as const,
    label: 'Austin',
    filter: ['austin', 'round rock', 'cedar park', 'pflugerville', 'leander', 'georgetown'],
  },
  {
    key: 'houston' as const,
    label: 'Houston',
    filter: ['houston', 'sugar land', 'katy', 'pearland', 'the woodlands', 'pasadena'],
  },
  {
    key: 'dallas' as const,
  ...
  {
    key: 'san-antonio' as const,
    label: 'San Antonio',
    filter: ['san antonio', 'new braunfels', 'schertz'],
  },
];
...
function getResidencySignal(scanResult: any) {
  const ds =
    scanResult?.category_scores?.data_sovereignty?.percentage ??
    scanResult?.categoryScores?.data_sovereignty?.percentage;
  if (ds === undefined || ds === null)
    return { icon: '◌', label: 'Pending', color: 'text-slate-500' };
...
function getTransparencySeal(scanResult: any) {
  const ai =
    scanResult?.category_scores?.ai_transparency?.percentage ??
...
function maskName(name: string): string {
  ...
  return (
    ...
    parts
      .slice(1)
      .map((p) => p[0] + '●'.repeat(Math.min(p.length - 1, 4)))
      .join(' ')
  );
}
...
export default function RegistryPage() {
  ...
  const loadProviders = useCallback(async () => {
    try {
      ...
      let query = supabase
        .from('registry')
        .select(
          'id,npi,name,url,city,zip,risk_score,risk_level,status_label,last_scan_timestamp,last_scan_result,updated_at,is_paid,is_visible,subscription_status',
          { count: 'exact' },
        )
        .eq('is_visible', true);
  ...
  useEffect(() => {
    setPage(0);
  }, [activeCity, activeTier, debouncedSearch]);
  ...
  const handleClaimSubmit = async () => {
    ...
    if (providerDomain && emailDomain) {
      ...
      const domainMatch =
        emailDomain === providerDomain ||
        emailDomain.endsWith('.' + providerDomain) ||
        providerDomain.endsWith('.' + emailDomain);

      if (!domainMatch) {
        setClaimError(
          `Email domain must match the provider website (${providerDomain}). Use your organization email.`,
        );
        return;
      }
    }
    ...
    <div className="min-h-screen" style={{ background: '#070d1b' }}>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 py-6">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p
                className="text-slate-500 text-xs ml-11"
      ...
      <div className="border-b" style={{ borderColor: 'rgba(255,255,255,0.04)' }}>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 py-4">
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
            {[
              {
                key: 'sovereign' as TierFilter,
                label: 'Sovereign',
                count: stats.sovereign,
                color: '#34d399',
                bg: 'rgba(52,211,153,0.08)',
              },
              {
                key: 'drift' as TierFilter,
              ...
              {
                key: 'violation' as TierFilter,
                label: 'Violation',
                count: stats.violation,
                color: '#f87171',
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 pb-8">
        ...
        >
          {/* Table Header */}
          <div
            className="hidden md:grid grid-cols-12 gap-4 px-5 py-3 text-xs font-semibold uppercase tracking-wider border-b"
            style={{
              ...
              return (
                ...
                >
                  ...
                  <div className="col-span-2">
                    <span
                      className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-bold ${tier.bg} ${tier.border} border`}
                    >
                      <span
                  ...
                  <div className="col-span-2">
                    <div className="flex items-center gap-2">
                      ...
                      <span className={`text-xs font-medium ${residency.color}`}>
                        {residency.label}
                      </span>
                    </div>
                  </div>
                  ...
                  <div className="col-span-2">
                    <div className="flex items-center gap-2">
                      ...
                      <span className={`text-xs font-medium ${transparency.color}`}>
                        {transparency.label}
                      </span>
                    </div>
                  </div>
                  ...
                  <div className="col-span-2 text-right">
                    {isPaid ? (
                      <Link
                        href={`/scan/results?npi=${p.npi}&mode=verified`}
        ...
        <div className="mt-8 pt-6 border-t" style={{ borderColor: 'rgba(255,255,255,0.04)' }}>
          <p
            className="text-slate-600 text-[10px] leading-relaxed max-w-3xl"
            style={{ fontFamily: "'JetBrains Mono', monospace" }}
          >
            PUBLIC-INTEREST DISCLOSURE — Data is collected and analyzed by the SENTRY engine in
            accordance with SB&nbsp;1188 (Data Sovereignty) and HB&nbsp;149 (AI Transparency)
            statutory frameworks. This registry does not constitute a legal endorsement,
            certification, or guarantee of compliance. Signals marked &ldquo;Pending&rdquo; indicate
            automated preliminary analysis only. Healthcare entities may claim their listing to
            initiate a verified forensic audit. &copy;&nbsp;{new Date().getFullYear()} KairoLogic.
            ...
            {claimStep === 'verify' ? (
              <>
                ...
                >
                  ...
                  <p className="text-slate-400 text-xs mt-1">
                    {claimModal.city}
                    {claimModal.zip ? `, ${claimModal.zip}` : ''}
                    {claimModal.npi && (
                      <span className="ml-2 text-slate-500">NPI: {claimModal.npi}</span>
                    )}
                  </p>
                </div>
                ...
                <div className="px-6 py-5">
                  ...
                  >
                    <div className="flex items-start gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-slate-400 text-xs mt-1">
                          To protect the privacy of forensic data, verify your identity using your
                          organization email to unlock the Preliminary Forensic Scan.
                        </p>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="space-y-3">
                    ...
                    <div>
                      ...
                      <input
                        ...
                        placeholder={
                          claimModal.url
                            ? `you@${extractDomain(claimModal.url) || 'yourpractice.com'}`
                            : 'you@yourpractice.com'
                        }
                        className="w-full px-3 py-2.5 rounded-lg text-sm text-white placeholder-slate-600 outline-none transition-all"
                        style={{
                          background: 'rgba(255,255,255,0.05)',
                  ...
                  {claimError && (
                    <div
                      className="mt-3 rounded-lg p-3 text-xs text-red-300"
                      style={{
              ...
              <>
                ...
                <div className="px-6 py-5">
                      ...
                      <div className="flex items-center gap-5 mb-6">
                        ...
                        <div>
                          ...
                          <p className="text-slate-400 text-xs mt-2">
                            Composite compliance score based on SB 1188 and HB 149 analysis.
                          </p>
                        </div>
                      </div>
                    );
                  ...
                  {(() => {
                    const cs =
                      claimModal.last_scan_result?.category_scores ||
                            ...
                            >
                              <div className="flex items-center justify-between mb-1.5">
                                ...
                                <div className="flex items-center gap-2">
                                  ...
                                  <span
                                    className={`text-[9px] font-bold px-1.5 py-0.5 rounded ${catTier.bg} ${catTier.cssColor}`}
                                  >
                                    {catTier.label}
                                  </span>
                                </div>
                  ...
                  {(() => {
                    const { critical, high, warnings, passes, total } = generateFindingsSummary(
                      claimModal.last_scan_result,
                    );
                      ...
                      >
                        ...
                        {critical.length > 0 && (
                          <div className="flex items-center gap-2 text-xs text-red-300 py-1">
                            ...
                            <span>
                              <strong className="text-red-400">{critical.length} critical</strong>{' '}
                              exposure{critical.length !== 1 ? 's' : ''} requiring immediate
                              attention
                            </span>
                          </div>
                        )}
                        {high.length > 0 && (
                          <div className="flex items-center gap-2 text-xs text-orange-300 py-1">
                            ...
                            <span>
                              ...
                              finding{high.length !== 1 ? 's' : ''} detected
                            </span>
                          </div>
                        )}
                        {warnings.length > 0 && (
                  ...
                  <p className="text-slate-400 text-xs mb-5">
                    ...
                    <strong className="text-white">{claimForm.email}</strong>. To access detailed
                    forensic findings, remediation roadmap, and data border map:
                  </p>
                  ...
                  <div className="space-y-2.5">
                    ...
                    >
                      <div>
                        ...
                        <div className="text-slate-500 text-xs">
                          Deep forensic analysis with legal evidence mapping
                        </div>
                      </div>
                      <div className="text-right">
                        <div className="text-lg font-black" style={{ color: '#d4a017' }}>
                          $149
                        </div>
                      </div>
                    </a>
                    ...
                    >
                      ...
                      <div>
                        ...
                        <div className="text-slate-500 text-xs">
                          Full audit + compliance remediation package
                        </div>
                      </div>
                      <div className="text-right">
                        <div className="text-lg font-black" style={{ color: '#d4a017' }}>
