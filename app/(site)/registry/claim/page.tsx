    ...
    (async () => {
      try {
        ...
        const { data } = await supabase
          .from('registry')
          .select(
            'id,npi,name,city,zip,risk_score,risk_level,last_scan_result,is_paid,subscription_status,url,email',
          )
          .eq('id', providerId)
          .single();
    ...
    return (
      <div className="min-h-screen bg-[#070d1b] flex items-center justify-center">
        <div className="text-center">
          ...
          <Link href="/registry" className="text-gold hover:text-gold-light font-bold">
        ...
        {step === 'verify' ? (
          <div className="bg-[#0c1425] border border-white/[0.1] rounded-2xl shadow-2xl overflow-hidden">
            ...
            <div className="px-8 py-6">
              <div
                className={`bg-${alertColor}-500/5 border border-${alertColor}-500/15 rounded-lg p-5 mb-6`}
              >
                <div className="flex items-start gap-3">
                  <AlertTriangle
                    className={`w-5 h-5 text-${alertColor}-400 flex-shrink-0 mt-0.5`}
                  />
                  <div>
                ...
                <div>
                  ...
                  {emailError ? (
                    <p className="text-[11px] text-amber-400 mt-1.5 flex items-center gap-1">
                      <AlertCircle size={11} />
                      {emailError}. Click verify again to proceed anyway.
                    </p>
                  ) : (
                    <p className="text-[10px] text-slate-500 mt-1">
                      Use your practice email to verify ownership
                    </p>
                  )}
                </div>
                <div>
                  <label className="text-[10px] font-bold uppercase tracking-wider text-slate-400 mb-1.5 block">
                    NPI Number <span className="text-slate-500">(optional)</span>
                  </label>
                  <input
                    type="text"
                    value={form.npi}
                    onChange={(e) =>
                      setForm({ ...form, npi: e.target.value.replace(/\D/g, '').slice(0, 10) })
                    }
              ...
              {score > 0 ? (
                <>
                    ...
                    <div className="flex items-center justify-between mb-1">
                      <div>
                        ...
                        <div className="text-slate-500 text-[10px] font-mono mt-0.5">
                          NPI: {provider.npi} &middot; Engine: SENTRY-3.1
                        </div>
                      </div>
                      <div className="text-right">
                        ...
                        <span
                          className={`text-[10px] font-bold px-2 py-0.5 rounded ${score >= 80 ? 'bg-emerald-500/20 text-emerald-400' : score >= 60 ? 'bg-amber-500/20 text-amber-400' : 'bg-red-500/20 text-red-400'}`}
                        >
                  ...
                  <div className="mb-5">
                    <div className="text-white font-bold text-sm mb-3">
                      Category Breakdown &mdash; Path to 100%
                    </div>
                    {[
                      {
                      ...
                      return (
                        <div
                          key={cat.key}
                          className="bg-white/[0.03] border border-white/[0.06] rounded-lg p-4 mb-2"
                        >
                          <div className="flex items-center justify-between mb-2">
                            ...
                            <div className="flex items-center gap-2">
                              <span
                                className={`font-bold text-sm ${pct >= 80 ? 'text-emerald-400' : pct >= 60 ? 'text-amber-400' : 'text-red-400'}`}
                              >
                                {pct}%
                              </span>
                              <span
                                className={`text-[9px] font-bold px-1.5 py-0.5 rounded ${tierColor}`}
                              >
                                {tierLabel}
                              </span>
                            </div>
                          ...
                          <div className="text-[10px] text-slate-500">
                            ...
                            {failed > 0 && (
                              <span className="text-red-400"> &middot; {failed} failed</span>
                            )}
                            {warned > 0 && (
                              <span className="text-amber-400"> &middot; {warned} warnings</span>
                            )}
                          </div>
                        </div>
                      );
                  ...
                  {(() => {
                    ...
                    const hasFindings =
                      critical.length > 0 || warnings.length > 0 || passes.length > 0;
                    return hasFindings ? (
                      <div className="bg-white/[0.03] border border-white/[0.06] rounded-xl p-5 mb-6 space-y-3">
                        ...
                        {critical.length > 0 && (
                          <div className="flex items-center gap-2 text-[11px] text-red-300 py-1">
                            ...
                            <span className="font-bold text-red-400">
                              {critical.length} critical exposure{critical.length !== 1 ? 's' : ''}{' '}
                              detected
                            </span>
                        ...
                        {passes.length > 0 && (
                          <div>
                            <div className="text-[10px] font-bold text-emerald-400 uppercase tracking-wider mb-1.5 mt-2">
                              Passing ({passes.length})
                            </div>
                            {passes.slice(0, 3).map((f, i) => (
                              <div
                                key={i}
                                className="flex items-center gap-2 text-[11px] text-emerald-300 py-0.5"
                              >
                            ...
                            {passes.length > 3 && (
                              <div className="text-[10px] text-slate-500 mt-1">
                                +{passes.length - 3} more passing checks
                              </div>
                            )}
                          </div>
                        )}
                ...
                <div className="bg-amber-500/5 border border-amber-500/15 rounded-xl p-6 mb-6 text-center">
                  ...
                  <p className="text-slate-400 text-xs mt-1">
                    Run a full audit to generate compliance findings.
                  </p>
                </div>
              )}

              <p className="text-slate-400 text-xs mb-6">
                A summary has been sent to <strong className="text-white">{form.email}</strong>. To
                unlock the full remediation roadmap and evidence documentation:
              </p>
              ...
              {(() => {
                ...
                return (
                  <div className="space-y-3">
                    ...
                    <a
                      href={`https://buy.stripe.com/4gM5kDdpw0HP18x3057Re08${clientRef}`}
                      target="_blank"
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <div className="text-slate-400 text-xs">
                            Forensic analysis + remediation roadmap
                          </div>
                    ...
                    <a
                      href={`https://buy.stripe.com/00w28retAeyF6sRgQV7Re07${clientRef}`}
                      target="_blank"
                      rel="noopener noreferrer"
                    ...
                    >
                      <div className="absolute -top-2.5 left-4 bg-gold text-navy text-[9px] font-bold uppercase tracking-wider px-2.5 py-0.5 rounded-full">
                        Recommended
                      </div>
                      <div className="flex items-center gap-3 mt-1">
                        ...
                        <div>
                          ...
                          <div className="text-slate-400 text-xs">
                            Audit + policies + AI disclosures + remediation
                          </div>
                          <div className="text-green-400 text-xs font-bold mt-0.5">
                            + 3 months Sentry Shield FREE
                          </div>
                    ...
                    <a
                      href={`https://buy.stripe.com/aFa3cv3OW4Y54kJ3057Re06${clientRef}`}
                      target="_blank"
                      rel="noopener noreferrer"
                    ...
                    >
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <div className="text-slate-400 text-xs">
                            24/7 monitoring + live dashboard + free audit report
                          </div>
                          <div className="text-emerald-400 text-xs font-bold mt-0.5">
                            Quarterly forensic reports &bull; Annual certification
                          </div>
                        </div>
                      </div>
                      <div className="flex items-center gap-2">
