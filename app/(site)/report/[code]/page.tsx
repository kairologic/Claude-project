...
function cleanUrl(url: string): string {
  try {
    const u = new URL(url);
    return (
      u.hostname.replace(/^www\./, '') + (u.pathname === '/' ? '' : u.pathname.replace(/\/$/, ''))
    );
  } catch {
    return url;
  }
    ...
    <div className={`flex items-start gap-3 py-3 px-3 rounded-lg ${bgColor}`}>
      ...
      <div className="min-w-0 flex-1">
        <div className="flex items-center gap-2 flex-wrap">
          ...
          {finding.clause && (
            <span className="text-[10px] text-gray-500 font-mono">{finding.clause}</span>
          )}
        </div>
        {detail && !isPass && <p className="text-xs text-gray-400 mt-0.5">{detail}</p>}
      </div>
      <span
        className={`text-[10px] font-bold uppercase tracking-wider px-2 py-0.5 rounded flex-shrink-0 ${isFail ? 'bg-red-400/10 text-red-400' : isWarn ? 'bg-amber-400/10 text-amber-400' : 'bg-green-400/10 text-green-400'}`}
      >
      ...
      <div className="p-2 space-y-1">
        {findings
          .sort((a, b) => {
            ...
            return (
              (order[a.status || 'inconclusive'] ?? 2) - (order[b.status || 'inconclusive'] ?? 2)
            );
          })
          .map((f, i) => (
            <FindingRow key={`${f.id || f.check_id || i}`} finding={f} />
          ))}
      </div>
    </div>
  );
...
function ProductCard({
  ...
  return (
    <div
      className={`bg-[#0d1a2e] rounded-2xl p-6 md:p-8 flex flex-col relative ${recommended ? 'border-2 border-[#c9a84c] shadow-[0_0_30px_rgba(201,168,76,0.15)]' : 'border border-white/10'}`}
    >
      {recommended && (
        <div className="absolute -top-3.5 left-1/2 -translate-x-1/2">
          <span className="bg-[#c9a84c] text-[#0a1628] text-xs font-bold px-4 py-1.5 rounded-full uppercase tracking-wider whitespace-nowrap">
            Recommended for You
          </span>
        </div>
      )}
      ...
      <a
        ...
        className={`flex items-center justify-center gap-2 py-3.5 px-6 rounded-xl font-bold text-sm transition-all duration-200 ${recommended ? 'bg-[#c9a84c] hover:bg-[#b8933d] text-[#0a1628]' : 'bg-white/10 hover:bg-white/15 text-white border border-white/10'}`}
      >
...
export default function ReportLandingPage() {
    ...
    return (
      <div className="min-h-screen bg-[#0a1628] flex items-center justify-center px-4">
        <div className="text-center max-w-md">
          <div className="inline-flex items-center justify-center w-16 h-16 bg-[#0d1a2e] rounded-full mb-6">
            <Shield className="w-8 h-8 text-gray-500" />
          </div>
          <h1 className="text-2xl font-bold text-white mb-3">Report Not Found</h1>
          <p className="text-gray-400 mb-6">
            This report link may be invalid or expired. If you believe this is an error, please
            contact us for assistance.
          </p>
          <a
            href="mailto:ravi@kairologic.net"
            className="inline-flex items-center gap-2 text-[#c9a84c] hover:text-[#d4b44e] transition-colors font-medium"
          >
      ...
      <div className="min-h-screen bg-[#0a1628]">
        <div className="border-b border-white/10">
          <div className="max-w-4xl mx-auto px-4 py-4">
            ...
            >
              ...
              <span className="text-[#c9a84c]">Logic</span>
            </Link>
          </div>
        </div>
        <div className="max-w-2xl mx-auto px-4 py-16 text-center">
          ...
          <p className="text-gray-400 mb-8">
            Your audit report for {data.practice_name} has already been purchased and is available
            for download.
          </p>
          {data.report_id && (
            <a
              href={`/api/report?reportId=${data.report_id}&download=true`}
              className="inline-flex items-center gap-2 bg-[#c9a84c] hover:bg-[#b8933d] text-[#0a1628] font-bold py-3 px-8 rounded-xl transition-colors"
            >
  ...
  const npiFails = data.npi_checks.filter(
  ...
  const getUrgencyMessage = (s: number) => {
    if (s < 60)
      return 'Your site has critical compliance gaps under Texas SB 1188 and HB 149. Immediate action is recommended to avoid potential enforcement.';
    if (s < 80)
      return 'Your site has compliance gaps under Texas SB 1188 and HB 149. Here is how to fix them before enforcement begins.';
    ...
    <div className="min-h-screen bg-[#0a1628]">
      ...
      <div className="border-b border-white/10">
        <div className="max-w-4xl mx-auto px-4 py-4 flex items-center justify-between">
          ...
          <span className="text-xs text-gray-500 hidden sm:block">
            Texas Healthcare Compliance Platform
          </span>
        </div>
      </div>
            ...
            <div className="flex flex-col md:flex-row md:items-center gap-6">
              ...
              <div className="flex-1 space-y-2">
                <div className="flex items-center gap-3">
                  <span className="text-xs text-gray-500 uppercase tracking-wider w-12 flex-shrink-0">
                    NPI
                  </span>
                  <span className="text-sm text-gray-300 font-mono">{data.npi}</span>
                </div>
                {data.url && data.url !== '__NOT_FOUND__' && (
                  <div className="flex items-center gap-3">
                    <span className="text-xs text-gray-500 uppercase tracking-wider w-12 flex-shrink-0">
                      Site
                    </span>
                    <span className="text-sm text-gray-300">{cleanUrl(data.url)}</span>
                  </div>
                )}
                <div className="flex items-center gap-3">
                  <span className="text-xs text-gray-500 uppercase tracking-wider w-12 flex-shrink-0">
                  ...
                  <span className="text-sm text-red-400 font-medium">
                    {totalIssues} compliance issue{totalIssues !== 1 ? 's' : ''} found
                  </span>
                </div>
              </div>
            </div>
      ...
      <section className="pb-12">
        <div className="max-w-3xl mx-auto px-4 space-y-4">
          <FindingsGroup
            title="SB 1188 \u2014 Data Sovereignty"
            icon={Globe}
            iconColor="text-blue-400"
            findings={data.sb1188_findings}
            failCount={sb1188Fails}
          />
          <FindingsGroup
            title="HB 149 \u2014 AI Transparency"
            icon={Bot}
            iconColor="text-purple-400"
            findings={data.hb149_findings}
            failCount={hb149Fails}
          />
          <FindingsGroup
            title="NPI Integrity"
            icon={Database}
            iconColor="text-cyan-400"
            findings={data.npi_checks}
            failCount={npiFails}
          />
            ...
            <div className="bg-[#0d1a2e] border border-red-400/20 rounded-xl p-4">
              <div className="flex items-center gap-2 text-red-400 text-sm mb-3">
                ...
                <span className="font-bold">
                  {foreignEndpoints.length} Foreign Endpoint
                  {foreignEndpoints.length !== 1 ? 's' : ''} Detected
                </span>
              </div>
              <div className="flex flex-wrap gap-2">
                {foreignEndpoints.map((ep, i) => (
                  <span
                    key={i}
                    className="text-xs bg-red-400/10 text-red-300 px-2.5 py-1 rounded-md font-mono"
                  >
                    {ep.domain || 'unknown'}{' '}
                    <span className="text-red-400/60">({ep.country || 'Foreign'})</span>
                  </span>
                ))}
              </div>
            </div>
      ...
      <section className="py-12 bg-[#081320]">
        <div className="max-w-3xl mx-auto px-4">
          ...
          <p className="text-gray-400 text-center text-sm mb-10">
            Both options include 3 months of Sentry Shield continuous monitoring, free.
          </p>
          <div className="grid md:grid-cols-2 gap-6">
            <ProductCard
              title="Sovereignty Audit Report"
              price="$149"
              priceNote="One-time + 3 months Sentry Shield FREE"
              features={[
                'Full forensic compliance analysis',
                'Evidence documentation package',
                'Legal compliance mapping (SB 1188 + HB 149)',
                'Remediation roadmap with priorities',
                '3 months Sentry Shield monitoring included',
              ]}
              stripeLink={STRIPE_LINK_AUDIT}
              npi={data.npi}
              recommended={recommendAudit}
            />
            <ProductCard
              ...
              priceNote="One-time + 3 months Sentry Shield FREE"
              features={[
                'Everything in the Audit Report, plus:',
                'Ready-to-use policy templates',
                'AI disclosure kit (HB 149 compliant)',
                'Compliance evidence ledger',
                'Remediation implementation guides',
                '3 months Sentry Shield monitoring included',
              ]}
              stripeLink={STRIPE_LINK_SAFE_HARBOR}
              npi={data.npi}
              recommended={recommendSafeHarbor}
            />
          </div>
        </div>
      </section>
        ...
        <div className="max-w-3xl mx-auto px-4">
          <div className="bg-[#0d1a2e] border border-white/10 rounded-2xl p-6 md:p-8">
            ...
            <p className="text-sm text-gray-300 leading-relaxed mb-4">
              Compliance is not a one-time fix. Your website changes, third-party scripts update,
              and new regulations take effect. Sentry Shield watches your site continuously so you
              do not have to.
            </p>
            <div className="grid sm:grid-cols-3 gap-4">
              <div className="bg-[#0a1628] rounded-xl p-4">
                ...
                <p className="text-xs text-gray-400 leading-relaxed">
                  If your site drifts out of compliance, such as new third-party scripts, changed
                  data routing, or removed policies, we alert you before it becomes a problem.
                </p>
              </div>
              <div className="bg-[#0a1628] rounded-xl p-4">
                <div className="flex items-center gap-2 mb-2">
                ...
                <p className="text-xs text-gray-400 leading-relaxed">
                  Access a private compliance dashboard for your practice showing your current
                  score, scan history, NPI integrity status, and any open issues.
                </p>
              </div>
              <div className="bg-[#0a1628] rounded-xl p-4">
                <div className="flex items-center gap-2 mb-2">
                ...
                <p className="text-xs text-gray-400 leading-relaxed">
                  Display a compliance verification badge on your website, showing patients and
                  partners that your practice takes data sovereignty seriously.
                </p>
              </div>
            </div>
            <p className="text-xs text-gray-500 mt-4 text-center">
              Both purchase options include 3 months of Sentry Shield at no extra cost. After that,
              monitoring continues at $79/month, or you can cancel anytime.
            </p>
          </div>
        </div>
      </section>
      ...
      <section className="py-10">
        <div className="max-w-3xl mx-auto px-4">
          <div className="grid grid-cols-3 gap-6 text-center">
            <div>
              <div className="text-2xl font-bold text-white">39,760</div>
              <div className="text-xs text-gray-500 mt-1">TX Providers Monitored</div>
            </div>
            <div>
              <div className="text-2xl font-bold text-white">SB 1188</div>
              <div className="text-xs text-gray-500 mt-1">Data Sovereignty</div>
            </div>
            <div>
              <div className="text-2xl font-bold text-white">HB 149</div>
              <div className="text-xs text-gray-500 mt-1">AI Transparency</div>
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="py-8 border-t border-white/10">
        <div className="max-w-4xl mx-auto px-4 text-center">
          <p className="text-gray-500 text-xs">
            ...
            <a
              href="mailto:ravi@kairologic.net"
              className="text-[#c9a84c] hover:text-[#d4b44e] transition-colors"
            >
              ravi@kairologic.net
            </a>
          </p>
