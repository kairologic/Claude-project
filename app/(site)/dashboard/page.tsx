...
function DashboardContent() {
  ...
  const [session, setSession] = useState<{ token: string; npi: string; email: string } | null>(
    null,
  );
  ...
  const verifyToken = async (token: string) => {
    try {
      ...
      const deviceToken =
        typeof window !== 'undefined' ? localStorage.getItem('kl_device_token') : null;
    ...
    return (
      <div className="min-h-screen bg-navy flex items-center justify-center px-4">
        <div className="text-center max-w-md">
          ...
          <Link
            href="/dashboard/login"
            className="inline-flex items-center gap-2 bg-gold text-navy font-bold px-6 py-3 rounded-lg"
          >
  ...
  const { provider, categories, borderMap, scanHistory, violations, driftAlerts, subscription } =
    data;
  ...
  const scoreBg =
    score >= 75 ? 'bg-green-500/10' : score >= 50 ? 'bg-amber-500/10' : 'bg-red-500/10';
    ...
    <div className="min-h-screen bg-navy flex">
      ...
      <div className="w-60 bg-[#0a1220] border-r border-white/[0.06] flex flex-col fixed inset-y-0 left-0 z-40">
        <div className="p-5 border-b border-white/[0.06]">
          ...
          <p className="text-gold text-[10px] font-bold uppercase tracking-[1.5px] mt-0.5">
            Sentry Shield
          </p>
        </div>
        ...
        <div className="p-4 border-t border-white/[0.06]">
          ...
          <Link
            ...
            target="_blank"
            className="flex items-center gap-2 text-slate-500 hover:text-gold text-xs mt-3 transition-colors"
          >
            <FileText size={12} /> User Guide
          </Link>
          <button
            onClick={handleLogout}
            className="flex items-center gap-2 text-slate-500 hover:text-slate-300 text-xs mt-3 transition-colors"
          >
            <LogOut size={12} /> Sign out
          </button>
          <button
            onClick={handleFullLogout}
            className="text-slate-600 hover:text-red-400 text-[10px] mt-1 transition-colors"
          >
            Forget this device
          </button>
        </div>
        ...
        <div className="sticky top-0 z-30 border-b border-white/[0.06] bg-navy/80 backdrop-blur-xl px-8 py-4 flex items-center justify-between">
          <div>
            ...
            <p className="text-xs text-slate-500">
              NPI: {provider.npi} • {provider.url} • {provider.city}
            </p>
          </div>
          <div className="flex items-center gap-2">
            ...
            <a
              href={provider.latest_report_url || '/sample-report.pdf'}
              target="_blank"
        ...
        <div className="p-8 space-y-6">
                ...
                <div>
                  ...
                  <div className="text-xs text-slate-400">
                    ...
                    <span className="text-green-400 font-semibold">
                      {formatTimeAgo(provider.last_scan_timestamp)}
                    </span>
                  </div>
                </div>
              </div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  {Object.entries(categories).length > 0 ? (
                    Object.entries(categories).map(([key, cat]: [string, any]) => {
                      const catTooltip =
                        key.includes('residen') || key.includes('sovereign')
                          ? TOOLTIPS.dataResidency
                          : key.includes('ai') || key.includes('transparen')
                            ? TOOLTIPS.aiTransparency
                            : key.includes('clinical')
                              ? TOOLTIPS.clinicalIntegrity
                              : key.includes('alert')
                                ? TOOLTIPS.activeAlerts
                                : key.includes('npi')
                                  ? TOOLTIPS.npiIntegrity
                                  : '';
                      return (
                        <div
                          key={key}
                        ...
                        >
                          ...
                          <div className="text-[11px] text-slate-500 mt-1">
                            {cat.passed} passed, {cat.failed} failed
                          </div>
              ...
              <div className="grid grid-cols-2 gap-6">
                ...
                <div className="bg-white/[0.02] border border-white/[0.06] rounded-xl p-5">
                  <h3 className="text-sm font-bold text-white mb-3 flex items-center gap-2">
                    <Calendar size={15} />{' '}
                    <Tooltip content={TOOLTIPS.scanScore}>
                ...
                <div className="bg-white/[0.02] border border-white/[0.06] rounded-xl p-5">
                  <h3 className="text-sm font-bold text-white mb-3 flex items-center gap-2">
                    <Bell size={15} />{' '}
                    <Tooltip content={TOOLTIPS.driftMonitor}>
            ...
            <div className="bg-white/[0.02] border border-white/[0.06] rounded-xl overflow-hidden">
              <div className="px-5 py-4 border-b border-white/[0.06]">
                <h3 className="text-sm font-bold text-white flex items-center gap-2">
                  <Map size={15} />{' '}
                  <Tooltip content={TOOLTIPS.dataResidency}>
                    <span>Data Border Map — Live</span>
                  </Tooltip>
                </h3>
              </div>
                ...
                <thead>
                  <tr className="border-b border-white/[0.06]">
                    {[
                      ...
                      <th
                        key={h.label}
                        className="text-left px-4 py-3 text-[10px] text-slate-500 font-bold uppercase tracking-wider"
                      >
                        <Tooltip content={h.tip} position="bottom">
                          <span>{h.label}</span>
                        </Tooltip>
                      </th>
                    ))}
                  ...
                  {borderMap.length > 0 ? (
                    borderMap.map((node, i) => (
                      <tr key={i} className="border-b border-white/[0.04] last:border-0">
                        ...
                        <td
                          className={`px-4 py-3 text-xs font-semibold ${node.isSovereign ? 'text-green-400' : 'text-red-400'}`}
                        >
                          {node.isSovereign ? '🇺🇸' : '🌐'} {node.city}, {node.country}
                        </td>
                        <td className="px-4 py-3">
                          <span
                            className={`inline-flex items-center gap-1.5 text-xs font-semibold ${node.isSovereign ? 'text-green-400' : 'text-red-400'}`}
                          >
                            <span
                              className={`w-1.5 h-1.5 rounded-full ${node.isSovereign ? 'bg-green-400' : 'bg-red-400'}`}
                            />
                    ...
                    <tr>
                      <td colSpan={5} className="px-4 py-8 text-center text-slate-500 text-sm">
                        No border map data yet. Run a scan to populate.
                      </td>
                    </tr>
                  )}
                </tbody>
                  ...
                  >
                    ...
                    >
                      {alert.resolved ? (
                        ...
                        <AlertTriangle
                          size={16}
                          className={alert.severity === 'high' ? 'text-red-400' : 'text-amber-400'}
                        />
                      )}
                    </div>
                    <div className="flex-1">
                      ...
                      <div className="text-[10px] text-slate-500 mt-2">
                        {formatDate(alert.timestamp)} • {alert.severity} priority
          ...
          {activeTab === 'audit' && (
            <div className="max-w-2xl space-y-4">
              <h3 className="text-sm font-bold text-white flex items-center gap-2">
                <FileText size={15} />{' '}
                <Tooltip content={TOOLTIPS.auditReport}>
              ...
              <div className="bg-white rounded-xl p-6 relative overflow-hidden">
                <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 rotate-[25deg] text-3xl font-extrabold text-black/[0.03] pointer-events-none whitespace-nowrap">
                  KAIROLOGIC
                </div>
                <div className="flex items-center justify-between mb-4">
                  <div className="text-sm font-extrabold">
                    <span className="text-navy">KAIRO</span>
                    <span className="text-gold">LOGIC</span>
                  </div>
                  <div className="text-[8px] text-gold font-bold uppercase tracking-[1.5px]">
                ...
                <div className="bg-navy rounded-lg p-4 text-center mb-4">
                  ...
                  <div className={`text-[10px] font-bold uppercase tracking-wider ${scoreColor}`}>
                    {scoreLabel}
                  </div>
                </div>
          ...
          {activeTab === 'widget' && (
            <div className="max-w-2xl space-y-4">
              <h3 className="text-sm font-bold text-white flex items-center gap-2">
                <Code size={15} />{' '}
                <Tooltip content={TOOLTIPS.widgetEmbed}>
                  <span>Your Compliance Widget</span>
                </Tooltip>
              </h3>
              <p className="text-xs text-slate-400">
                Embed this on your website footer to show patients your compliance status.
              </p>
              ...
              <div className="bg-[#0a1220] rounded-xl p-4 font-mono text-xs text-green-400 leading-relaxed overflow-x-auto">
                ...
                &lt;div id=&quot;kairologic-widget&quot;&gt;&lt;/div&gt;
                <br />
                &lt;script src=&quot;https://kairologic.net/sentry-widget.js&quot;
                <br />
                &nbsp;&nbsp;data-npi=&quot;{provider.npi}&quot; async&gt;
                <br />
                &lt;/script&gt;
              </div>
          ...
          {['quarterly', 'certification', 'settings'].includes(activeTab) && (
            <div className="bg-white/[0.02] border border-white/[0.06] rounded-xl p-12 text-center">
              ...
              <p className="text-xs text-slate-500">
                {activeTab === 'quarterly' &&
                  'Quarterly compliance reports will be generated automatically.'}
                {activeTab === 'certification' &&
                  'Annual compliance certification will be available after 12 months of monitoring.'}
                {activeTab === 'settings' &&
                  'Account settings, notification preferences, and billing management.'}
              </p>
            </div>
          )}
        </div>
