...
const PRODUCT_CONFIG: Record<
  string,
  { title: string; subtitle: string; icon: any; color: string; includes: string[] }
> = {
  report: {
...
function SuccessPageInner() {
  ...
  const fetchReport = useCallback(
    async (npi: string): Promise<{ report: ScanReport | null; ageDays: number }> => {
      // Try scan_reports table first (formal reports)
      try {
        const res = await fetch(
          `${SUPABASE_URL}/rest/v1/scan_reports?npi=eq.${npi}&order=report_date.desc&limit=1`,
          {
      ...
      try {
        ...
        const scanResult =
          typeof provider.last_scan_result === 'string'
        ...
        const reportDate = scanTs
          ? new Date(typeof scanTs === 'number' ? scanTs : scanTs).toISOString()
          : new Date().toISOString();
        const ageDays = scanTs
          ? Math.floor(
              (Date.now() - new Date(typeof scanTs === 'number' ? scanTs : scanTs).getTime()) /
                (1000 * 60 * 60 * 24),
            )
          : 0;
        ...
        const report: ScanReport = {
          ...
          scan_meta: scanResult.meta || {
            checksRun: scanResult.findings?.length || 0,
            duration: scanResult.scanDuration ? `${scanResult.scanDuration}ms` : undefined,
          },
  ...
  useEffect(() => {
    const init = async () => {
      ...
      if (sessionId) {
        ...
        if (urlProduct && PRODUCT_CONFIG[urlProduct])
          product = urlProduct as PurchaseInfo['product'];
      ...
      <div className="max-w-3xl mx-auto">
        ...
        <div className="bg-white shadow-xl rounded-2xl overflow-hidden border border-gray-100 mb-6">
          <div className="bg-gradient-to-r from-navy via-navy to-navy p-6 text-center border-b-4 border-gold">
            ...
            <p className="text-gold text-[10px] uppercase tracking-[0.2em] mt-1 font-semibold">
              Purchase Confirmed
            </p>
          </div>
          <div className="p-8 md:p-10 text-center">
            ...
            <p className="text-gray-500 text-sm mb-4">
              Thank you for your purchase. Your deliverables are ready below.
            </p>
            ...
            <div className="flex items-start gap-3">
              ...
              <div>
                <h3 className="font-bold text-green-800 text-sm mb-1">
                  Sentry Shield Activated — 90 Days FREE
                </h3>
                <p className="text-xs text-green-700 leading-relaxed">
                  Your purchase includes 3 months of Sentry Shield continuous compliance monitoring
                  at no extra cost. This includes a live dashboard, website compliance widget, drift
                  alerts, and quarterly reports. After your trial, you can continue Shield at
                  $79/mo, switch to Watch at $39/mo, or cancel anytime.
                </p>
              </div>
            </div>
          </div>
        ...
        <div className="space-y-4">
                ...
                {purchaseInfo?.reportData ? (
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-5">
                      <div className="bg-slate-50 rounded-lg p-3 text-center">
                        <div
                          ...
                          style={{
                            color:
                              (purchaseInfo.reportData.sovereignty_score || 0) >= 80
                                ? '#16a34a'
                                : (purchaseInfo.reportData.sovereignty_score || 0) >= 50
                                  ? '#d97706'
                                  : '#dc2626',
                          }}
                        >
                      ...
                      <div className="bg-amber-50 border border-amber-200 rounded-lg p-4">
                        <div className="flex items-center gap-2 mb-1">
                          ...
                          <span className="text-sm font-semibold text-amber-800">
                            Report is older than {REPORT_MAX_AGE_DAYS} days
                          </span>
                        </div>
                        <p className="text-xs text-amber-600 mb-3">
                          Your most recent scan is {purchaseInfo.reportAge} days old. We recommend a
                          fresh scan for the most accurate report.
                        </p>
                        <div className="flex flex-col sm:flex-row gap-2">
                          <Link href={`/scan?npi=${purchaseInfo.npi}`} className="flex-1">
                            <button className="w-full bg-amber-600 hover:bg-amber-700 text-white font-semibold py-2.5 px-4 rounded-lg text-sm transition-colors">
                              Run Fresh Scan &rarr; Generate Report
                            </button>
                          </Link>
                          <button
                            onClick={handleDownloadReport}
                  ...
                  <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                    ...
                    <p className="text-xs text-blue-600 mb-3">
                      We couldn&apos;t find a recent scan for NPI {purchaseInfo?.npi || 'N/A'}. Run
                      a scan to generate your report.
                    </p>
                    <Link href={`/scan?npi=${purchaseInfo?.npi || ''}`}>
                      <button className="bg-blue-600 hover:bg-blue-700 text-white font-semibold py-2.5 px-4 rounded-lg text-sm transition-colors">
                        Run Compliance Scan
                      </button>
                    </Link>
                  </div>
            ...
            <div className="bg-white rounded-xl shadow-md border border-slate-200 overflow-hidden">
              ...
              <div className="p-6">
                <p className="text-sm text-gray-600 mb-4">
                  Your complete remediation kit. Each document is ready to use &mdash; no
                  customization required.
                </p>
                <div className="space-y-2">
                  {[
                    {
                      name: 'SB 1188 Data Sovereignty Policy',
                      file: 'sb1188-policy-pack.pdf',
                      icon: '\uD83D\uDCCB',
                    },
                    {
                      name: 'HB 149 AI Disclosure Kit',
                      file: 'ai-disclosure-kit.zip',
                      icon: '\uD83E\uDD16',
                    },
                    {
                      name: 'Audit Evidence Ledger Templates',
                      file: 'evidence-ledger.xlsx',
                      icon: '\uD83D\uDCC1',
                    },
                    {
                      name: 'Staff Training Guide',
                      file: 'staff-training-guide.pdf',
                      icon: '\uD83D\uDC65',
                    },
                    {
                      name: 'Annual Compliance Roadmap',
                      file: 'compliance-roadmap.pdf',
                      icon: '\uD83D\uDCC5',
                    },
                    {
                      name: 'Implementation Blueprint',
                      file: 'implementation-blueprint.pdf',
                      icon: '\uD83D\uDD27',
                    },
                  ].map((d) => (
                    <a
                      key={d.name}
                      href={`/downloads/safe-harbor/${d.file}`}
                    ...
                    >
                      ...
                      <span className="flex-1 text-sm font-medium text-slate-700 group-hover:text-orange-700">
                        {d.name}
                      </span>
                ...
                <p className="text-[10px] text-slate-400 mt-3">
                  Documents are generic templates aligned to Texas SB 1188 and HB 149. Customize
                  with your practice details for best results.
                </p>
              </div>
            </div>
          )}
          ...
          {hasMonitoring && (
            <div className="bg-white rounded-xl shadow-md border border-slate-200 overflow-hidden">
              <div
                className={`px-6 py-4 border-b flex items-center gap-3 ${monitoringMode === 'shield' ? 'bg-gradient-to-r from-green-50 to-emerald-50 border-green-200' : 'bg-gradient-to-r from-blue-50 to-indigo-50 border-blue-200'}`}
              >
                <Shield
                  className={`w-5 h-5 ${monitoringMode === 'shield' ? 'text-green-600' : 'text-blue-600'}`}
                />
                <h2 className="font-bold text-navy text-sm">
                  Sentry {monitoringMode === 'shield' ? 'Shield' : 'Watch'} —{' '}
                  {isTrialShield ? '90-Day Trial' : 'Active'}
                </h2>
                <span className="ml-auto bg-green-100 text-green-700 text-[10px] font-bold px-2 py-0.5 rounded-full">
              ...
              <div className="p-6 space-y-6">
                ...
                <div>
                  ...
                  <p className="text-xs text-gray-600 mb-3">
                    Access your live compliance dashboard to monitor your practice&apos;s data
                    sovereignty status in real time.
                  </p>
                  <a
                    href={`/dashboard/${purchaseInfo?.npi || ''}?token=${purchaseInfo?.dashboardToken || ''}`}
                ...
                <div>
                  ...
                  <p className="text-xs text-gray-600 mb-3">
                    Add this code to your website to display your live compliance status badge.
                  </p>
                  <div className="relative">
                    ...
                    <button
                      onClick={() => handleCopyCode(getWidgetCode())}
                  ...
                  <div className="mt-4 bg-slate-50 border border-slate-100 rounded-lg p-4">
                    ...
                    <div className="space-y-2 text-xs text-slate-600">
                      {[
                        ...
                        'A small shield icon will appear in the corner of your site \u2014 patients can click it to see your compliance status',
                      ].map((step, i) => (
                        <div key={i} className="flex items-start gap-2">
                          <span className="bg-navy text-white text-[10px] font-bold w-5 h-5 rounded-full flex items-center justify-center flex-shrink-0 mt-0.5">
                            {i + 1}
                          </span>
                          <span>{step}</span>
                        </div>
                    ...
                    <p className="text-[10px] text-slate-400 mt-3">
                      Works with WordPress, Squarespace, Wix, custom HTML, and any website platform.
                    </p>
                  </div>

                  <div className="mt-3">
                    <Link
                      href={`/widget/test?npi=${purchaseInfo?.npi || ''}`}
                      className="inline-flex items-center gap-1.5 text-xs text-blue-600 hover:text-blue-800 font-medium transition-colors"
                    >
                      <ExternalLink className="w-3 h-3" /> Preview how the widget looks on your site
                      &rarr;
                    </Link>
                  </div>
                </div>
          ...
          {(() => {
            ...
            if (product === 'report' || product === 'safe-harbor')
              return (
                <div className="bg-gradient-to-r from-green-50 to-emerald-50 border border-green-200 rounded-xl p-6 text-center">
                  <div className="text-3xl mb-2">🛡️</div>
                  <h3 className="text-green-800 font-bold text-sm mb-1">
                    Your Sentry Shield is active for 90 days
                  </h3>
                  <p className="text-green-600 text-xs max-w-lg mx-auto mb-3">
                    Your website is now being monitored 24/7. You&apos;ll receive drift alerts if
                    anything changes. After 90 days, continue Shield at $79/mo, switch to Watch at
                    $39/mo, or cancel anytime.
                  </p>
                  <a
                    href={`/dashboard/${purchaseInfo?.npi || ''}?token=${purchaseInfo?.dashboardToken || ''}`}
              ...
              return (
                <div className="bg-gradient-to-r from-green-50 to-emerald-50 border border-green-200 rounded-xl p-6 text-center">
                  ...
                  <p className="text-green-600 text-xs max-w-lg mx-auto">
                    Sentry Shield provides continuous monitoring, quarterly forensic reports, live
                    dashboard access, and annual certification. Your compliance is in expert hands.
                  </p>
                </div>
              );
              ...
              return (
                <div className="bg-gradient-to-r from-green-900 to-slate-900 rounded-xl p-6 text-center">
                  <h3 className="text-white font-bold text-sm mb-1">
                    Want full visibility? Upgrade to Shield.
                  </h3>
                  <p className="text-slate-400 text-xs mb-4 max-w-lg mx-auto">
                    Get a live compliance dashboard, quarterly forensic reports, annual
                    certification seal, and priority support.
                  </p>
                  {/* TODO: Replace with live Shield payment link */}
                  <a
                    href="https://buy.stripe.com/SHIELD_LINK_PLACEHOLDER"
                    className="inline-block bg-green-600 hover:bg-green-700 text-white font-bold py-3 px-6 rounded-lg text-sm transition-colors"
                  >
                    Upgrade to Sentry Shield &mdash; $79/mo
                  </a>
                  <p className="text-[10px] text-slate-500 mt-2">
                    Only $40/mo more for enterprise-grade compliance management
                  </p>
                </div>
              );
          ...
          <div className="text-center py-4">
            <p className="text-slate-400 text-xs">
              Questions? Email{' '}
              <a href="mailto:compliance@kairologic.com" className="text-blue-600 hover:underline">
                compliance@kairologic.com
              </a>{' '}
              or call (512) 402-2237
            </p>
            <Link
              href="/"
              className="inline-flex items-center gap-1 text-xs text-slate-500 hover:text-navy mt-2 transition-colors"
            >
