...
async function gatherStats(orgId: string): Promise<PracticeStats | null> {
  ...
  const forms = await db(`update_requests?organization_id=eq.${orgId}&select=status`);
...
export async function sendDay7Email(orgId: string, email: string): Promise<void> {
  ...
  if (stats.mismatch_count > 0)
    findings.push(`<strong>${stats.mismatch_count}</strong> NPPES mismatches detected`);
  if (stats.forms_generated > 0)
    findings.push(
      `<strong>${stats.forms_generated}</strong> correction form${stats.forms_generated > 1 ? 's' : ''} generated`,
    );
  if (stats.forms_confirmed > 0)
    findings.push(
      `<strong>${stats.forms_confirmed}</strong> update${stats.forms_confirmed > 1 ? 's' : ''} confirmed live in NPPES`,
    );
  if (stats.departed_count > 0)
    findings.push(
      `<strong>${stats.departed_count}</strong> departed provider${stats.departed_count > 1 ? 's' : ''} detected`,
    );
  ...
  const findingsHtml =
    findings.length > 0
      ? findings
          .map(
            (f) =>
              `<div style="padding:8px 0;border-bottom:1px solid #f1f5f9;font-size:14px;color:#334155;">\u2713 ${f}</div>`,
          )
          .join('')
