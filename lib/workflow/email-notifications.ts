...
> = {
  ...
  task_needs_attention: {
    subject: (v) =>
      `Action needed: ${v.task_title || 'A task'} for ${v.provider_name || 'provider'}`,
    html: (v) =>
      emailWrapper(`
  ...
  overdue_warning: {
    subject: (v) =>
      `Overdue: ${v.provider_name || 'Provider'} update pending ${v.days_overdue || ''} days`,
    html: (v) =>
      emailWrapper(`
  ...
  overdue_action: {
    subject: (v) =>
      `URGENT: ${v.provider_name || 'Provider'} update ${v.days_overdue || '28+'}+ days overdue`,
    html: (v) =>
      emailWrapper(`
  ...
  workflow_resolved: {
    subject: (v) =>
      `Resolved: ${v.provider_name || 'Provider'} ${v.workflow_type || 'workflow'} complete`,
    html: (v) =>
      emailWrapper(`
...
export async function sendWorkflowNotification(
  ...
  const vars: TemplateVars = {
    ...opts.vars,
    workflow_url:
      opts.vars.workflow_url || `${DASHBOARD_URL}/dashboard?workflow=${opts.workflow_id}`,
  };
    ...
    if (data && data.length > 0) {
      if (recipientType === 'practice_admin') {
        const admins = data.filter(
          (u: { role: string }) => u.role === 'admin' || u.role === 'owner',
        );
        return admins.length > 0 ? admins.map((u: { email: string }) => u.email) : [data[0].email]; // fallback to first user
      }
