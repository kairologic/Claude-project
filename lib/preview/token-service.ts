 ...
 */
export async function claimPreviewToken(tokenId: string, userId: string): Promise<void> {
  await db(`preview_tokens?id=eq.${tokenId}`, {
    method: 'PATCH',
    body: JSON.stringify({
      is_claimed: true,
 ...
 */
export async function markFollowupSent(tokenId: string, followupDay: 1 | 2 | 3): Promise<void> {
  await db(`preview_tokens?id=eq.${tokenId}`, {
    method: 'PATCH',
