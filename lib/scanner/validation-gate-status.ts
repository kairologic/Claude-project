...
export class ValidationGateStatusService {
  ...
  async getGateStatus(forceRefresh = false): Promise<GateStatusResult> {
    ...
    // Separate by method
    const pecosRecords = validatedRecords.filter((r) => r.method === 'PECOS_EXACT');
    const fuzzyRecords = validatedRecords.filter((r) => r.method === 'NPPES_FUZZY');
    ...
    const pecos_fp_rate = pecosRecords.length > 0 ? pecosFP / pecosRecords.length : 0;
    const fuzzy_fp_rate = fuzzyRecords.length > 0 ? fuzzyFP / fuzzyRecords.length : 0;

    // Weighted: proportional to total validated records per method
    const totalValidated = validatedRecords.length;
    const weighted_fp_rate = totalValidated > 0 ? (pecosFP + fuzzyFP) / totalValidated : 0;
...
export async function stampDeltaEventVerification(
  ...
  const verificationStatus = await gateService.getVerificationStatus(detectionSource, confidence);
 ...
 */
export function shouldSendAlert(deltaEvent: { verification_status?: VerificationStatus }): boolean {
 ...
 */
export function canGenerateForm(deltaEvent: { verification_status?: VerificationStatus }): {
  allowed: boolean;
  requiresConfirmation: boolean;
