...
function portalSection(field: string): string {
  if (field.startsWith('address') || field === 'city' || field === 'state' || field === 'zip_code') return 'Practice Location';
...
export async function generateNPPESForm(data: NPPESFormData): Promise<Uint8Array> {
  ...
  const dateStr = new Date(genDate).toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' });
  ...
  p1.drawText(titleR, { x: W - m - fontB.widthOfTextAtSize(titleR, 12), y: H - 32, size: 12, font: fontB, color: WHITE });
  p1.drawText(dateStr, { x: W - m - fontR.widthOfTextAtSize(dateStr, 9), y: H - 46, size: 9, font: fontR, color: GRAY_400 });
  ...
  if (rec) {
    if (rec.address_line_1) {
      const parts = [rec.address_line_1, rec.address_line_2, [rec.city, rec.state, rec.zip_code].filter(Boolean).join(', ')].filter(Boolean);
  ...
  y -= 6;
  p1.drawText(`Field being corrected: ${fieldLabel}`, { x: m, y, size: 9, font: fontR, color: GRAY_600 });
  y -= 24;
  ...
  p1.drawRectangle({ x: m, y: y - bh, width: bw, height: bh, color: GRAY_50, borderColor: GRAY_200, borderWidth: 1 });
  ...
  const rx = m + bw + 16;
  p1.drawRectangle({ x: rx, y: y - bh, width: bw, height: bh, color: GRAY_50, borderColor: GRAY_200, borderWidth: 1 });
  ...
  // Source note
  const sourceLabel = data.approvedSource === 'website' ? 'Approved from website data'
    : data.approvedSource === 'nppes' ? 'NPPES data confirmed correct'
  ...
  p1.drawText('See page 2 for a step-by-step visual walkthrough with annotated screenshots.', {
    x: m, y, size: 9, font: fontB, color: BLUE,
  });
  y -= 24;

  // Note box
  const noteH = 50;
  p1.drawRectangle({ x: m, y: y - noteH, width: cw, height: noteH, color: GOLD_PALE, borderColor: GOLD, borderWidth: 1 });
  p1.drawText('Note:', { x: m + 10, y: y - 18, size: 9, font: fontB, color: NAVY });
  p1.drawText('If you have already submitted this correction, no action is needed. KairoLogic monitors', { x: m + 10, y: y - 32, size: 8, font: fontR, color: GRAY_600 });
  p1.drawText('NPPES weekly and will automatically close this workflow when the update appears live.', { x: m + 10, y: y - 42, size: 8, font: fontR, color: GRAY_600 });

  // Footer
  hline(p1, 56, m, W - m);
  p1.drawText(`Workflow ID: ${data.workflowId}`, { x: m, y: 40, size: 7, font: fontR, color: GRAY_400 });
  p1.drawText(`Generated: ${new Date(genDate).toISOString()}`, { x: m, y: 30, size: 7, font: fontR, color: GRAY_400 });
  p1.drawText('kairologic.net', { x: W - m - fontR.widthOfTextAtSize('kairologic.net', 7), y: 40, size: 7, font: fontR, color: GRAY_400 });
  p1.drawText('Confidential - For practice use only', { x: W - m - fontR.widthOfTextAtSize('Confidential - For practice use only', 7), y: 30, size: 7, font: fontR, color: GRAY_400 });
  ...
  p2.drawText(walkTitle, { x: W - m - fontB.widthOfTextAtSize(walkTitle, 11), y: H - 28, size: 11, font: fontB, color: WHITE });

  y = H - h2H - 16;
  p2.drawText('Follow these steps in the NPPES portal at nppes.cms.hhs.gov', { x: m, y, size: 9, font: fontR, color: GRAY_600 });
  y -= 24;

  // ── Panel drawing helper ──────────────────────────────────────────────

  function drawPanel(
    page: PDFPage, startY: number, stepNum: number, title: string,
    desc: string, panelH: number,
    ...
    page.drawRectangle({ x: m, y: mockY - mockH, width: cw, height: mockH, color: WHITE, borderColor: GRAY_200, borderWidth: 1 });
  ...
  y = drawPanel(p2, y, 1, 'Log in to NPPES',
    ...
    (page, px, py, pw, ph) => {
      ...
      page.drawText('National Plan & Provider Enumeration System', { x: px + 70, y: py - 13, size: 7, font: fontR, color: rgb(180 / 255, 210 / 255, 240 / 255) });
      ...
      page.drawRectangle({ x: fx, y: fy - 28, width: 160, height: 16, color: WHITE, borderColor: GRAY_200, borderWidth: 1 });

      page.drawText('Password', { x: fx, y: fy - 37, size: 7, font: fontR, color: GRAY_400 });
      page.drawRectangle({ x: fx, y: fy - 52, width: 160, height: 16, color: WHITE, borderColor: GRAY_200, borderWidth: 1 });
      ...
      page.drawText('Use your CMS I&A login', { x: fx + 190, y: fy - 72, size: 7, font: fontB, color: GOLD });
    },
  );

  // ── STEP 2: Find & Edit NPI ───────────────────────────────────────────

  y = drawPanel(p2, y, 2, 'Find and edit the NPI',
    ...
    (page, px, py, pw, ph) => {
      ...
      // Action icons
      page.drawRectangle({ x: lx + 348, y: rowY - 4, width: 20, height: 14, color: GRAY_100, borderColor: GRAY_200, borderWidth: 1 });
      ...
      page.drawText('Click pencil to Edit', { x: lx + 418, y: rowY - 2, size: 7, font: fontB, color: GOLD });
    },
  );
  ...
  y = drawPanel(p2, y, 3, `Update the ${fieldLabel}`, step3Desc, 140,
    (page, px, py, pw, ph) => {
      page.drawRectangle({ x: px + 1, y: py - 1, width: pw - 2, height: 18, color: BLUE });
      page.drawText('CMS  |  NPPES  >  Edit NPI Application', { x: px + 8, y: py - 13, size: 7, font: fontB, color: WHITE });

      // Left sidebar
      const sideW = 110;
      page.drawRectangle({ x: px + 1, y: py - ph + 1, width: sideW, height: ph - 20, color: GRAY_50, borderColor: GRAY_200, borderWidth: 0.5 });

      const navItems = ['Provider Info', 'Other Info', 'Practice Location', 'Endpoint Info', 'Review', 'Submit'];
      let navY = py - 28;
      for (const item of navItems) {
        const isActive = item === section;
        if (isActive) page.drawRectangle({ x: px + 2, y: navY - 3, width: sideW - 2, height: 14, color: BLUE });
        page.drawText(item, { x: px + 10, y: navY, size: 7, font: isActive ? fontB : fontR, color: isActive ? WHITE : GRAY_600 });
        navY -= 14;
      }
      ...
      if (isPhone) {
        ...
        page.drawRectangle({ x: fx, y: fy - 36, width: pw - sideW - 40, height: 16, color: WHITE, borderColor: GREEN, borderWidth: 1.5 });
        page.drawText(approvedDisplay, { x: fx + 4, y: fy - 32, size: 8, font: fontB, color: GREEN });

        drawCallout(page, fx + pw - sideW - 30, fy - 28, 'D', fontB);
        page.drawText('Enter approved', { x: fx + pw - sideW - 18, y: fy - 24, size: 6, font: fontB, color: GOLD });
        page.drawText('phone number', { x: fx + pw - sideW - 17, y: fy - 32, size: 6, font: fontB, color: GOLD });
      } else {
        // Address fields
        ...
        page.drawRectangle({ x: fx, y: fy - 68, width: 100, height: 16, color: WHITE, borderColor: GREEN, borderWidth: 1.5 });
        ...
        page.drawRectangle({ x: fx + 110, y: fy - 68, width: 40, height: 16, color: WHITE, borderColor: GREEN, borderWidth: 1.5 });
        ...
        page.drawRectangle({ x: fx + 160, y: fy - 68, width: 60, height: 16, color: WHITE, borderColor: GREEN, borderWidth: 1.5 });
        page.drawText(zipPart, { x: fx + 164, y: fy - 64, size: 8, font: fontB, color: GREEN });

        drawCallout(page, fx + pw - sideW - 30, fy - 32, 'D', fontB);
        page.drawText('Enter approved', { x: fx + pw - sideW - 18, y: fy - 28, size: 6, font: fontB, color: GOLD });
        page.drawText('address here', { x: fx + pw - sideW - 15, y: fy - 36, size: 6, font: fontB, color: GOLD });
      }
    },
  );
  ...
  y = drawPanel(p2, y, 4, 'Error Check and Submit',
    'Click "Error Check" to validate, then "Complete NPI Application" to submit your update.',
    130,
    (page, px, py, pw, ph) => {
      page.drawRectangle({ x: px + 1, y: py - 1, width: pw - 2, height: 18, color: BLUE });
      page.drawText('CMS  |  NPPES  >  Submit', { x: px + 8, y: py - 13, size: 7, font: fontB, color: WHITE });

      const cx = px + pw / 2;

      // Success message
      page.drawRectangle({ x: cx - 130, y: py - 34, width: 260, height: 16, color: GREEN_PALE, borderColor: GREEN, borderWidth: 1 });
      page.drawText('No errors found. Ready to submit.', { x: cx - 80, y: py - 31, size: 8, font: fontR, color: GREEN });

      // Error Check button
      page.drawRectangle({ x: cx - 120, y: py - 60, width: 90, height: 18, color: GRAY_100, borderColor: GRAY_200, borderWidth: 1 });
      ...
      page.drawText('Complete NPI Application', { x: cx + 20, y: py - 56, size: 8, font: fontB, color: WHITE });
      drawCallout(page, cx + 163, py - 51, 'F', fontB);

      // Closing note
      page.drawText('After submitting, NPPES typically processes updates within 1-2 business weeks.', {
        x: cx - 160, y: py - ph + 14, size: 7, font: fontR, color: GRAY_400,
      });
      page.drawText('KairoLogic will automatically detect the change and close your workflow.', {
        x: cx - 145, y: py - ph + 4, size: 7, font: fontR, color: GRAY_400,
      });
    },
  );
  ...
  hline(p2, 56, m, W - m);
  p2.drawText('Page 2 of 2  |  NPPES Portal Visual Guide', { x: m, y: 40, size: 7, font: fontR, color: GRAY_400 });
  p2.drawText('Screens may vary slightly. Portal: nppes.cms.hhs.gov', { x: m, y: 30, size: 7, font: fontR, color: GRAY_400 });
  p2.drawText('kairologic.net', { x: W - m - fontR.widthOfTextAtSize('kairologic.net', 7), y: 40, size: 7, font: fontR, color: GRAY_400 });
