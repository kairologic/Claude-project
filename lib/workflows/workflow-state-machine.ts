import { WorkflowType, WorkflowStatus, TaskStatus } from '@/lib/types/dashboard-schema';
...
export const WORKFLOW_TASK_TEMPLATES: Record<WorkflowType, TaskTemplate[]> = {
  nppes_update: [
    ...
    {
      taskType: 'download_submit',
      title: 'Download form & submit to NPPES',
      description: 'Download pre-filled PDF, upload at NPPES portal, confirm submission',
      order: 2,
    ...
    },
    {
      ...
      description: 'Auto-checks NPPES weekly sync for your update',
      order: 3,
      initialStatus: 'pending',
      isAutomated: true,
      estimatedDays: 5,
 ...
 */
export function getNextStatus(currentStatus: WorkflowStatus, event: string): WorkflowStatus | null {
...
export function calculateProgress(tasks: { status: TaskStatus }[]): number {
  ...
  const inProgressCount = tasks.filter((task) => task.status === 'active').length;

  const progress = ((completedCount + inProgressCount) / tasks.length) * 100;
