...
function getCatPct(
  catScores: AuditReportInput['categoryScores'],
  key: string,
  altKey: string,
): { percentage: number; passed: number; findings: number; level: string } {
...
export async function generateAuditPDF(input: AuditReportInput): Promise<void> {
  ...
  const reportDate = input.reportDate
    ? new Date(input.reportDate).toLocaleDateString('en-US', {
        year: 'numeric',
        month: 'long',
        day: 'numeric',
      })
  ...
  doc.text(`Practice: ${practiceName}`, margin, y);
  y += 6;
  doc.text(`NPI: ${npi}`, margin, y);
  y += 6;
  doc.text(`Website: ${websiteUrl || 'N/A'}`, margin, y);
  y += 6;
  ...
  const cdnLine = input.cdnDetection?.detected
    ? ` | CDN Detection: ${input.cdnDetection.provider || 'Detected'} (via ${input.cdnDetection.detectedVia || 'analysis'})`
    : '';
  doc.text(
    `Engine: ${engineVersion} | ${checksRun} checks executed | Duration: ${scanDuration}${cdnLine} | Living Compliance Ledger: Active`,
    margin,
    y,
  );
  ...
  if (isSample) {
    ...
    doc.setTextColor(...GOLD);
    doc.text(
      'SAMPLE REPORT | This is a demonstration report with simulated provider data.',
      pageW / 2,
      y + 6,
      { align: 'center' },
    );
  ...
  const drawPageHeader = () => {
    ...
    doc.text(`CONFIDENTIAL${isSample ? ' | SAMPLE REPORT' : ''}`, pageW / 2, 6, {
      align: 'center',
    });
  };
  drawPageHeader();
  ...
  doc.setFontSize(6.5);
  doc.text(
    `This scan has been recorded in your Living Compliance Ledger as an immutable audit event.`,
    margin + 4,
    y + 9,
  );
  ...
  (doc as any).autoTable({
    startY: y,
    head: [
      ['OVERALL SCORE', 'RISK LEVEL', 'CHECKS RUN', 'CHECKS PASSED', 'CHECKS FAILED', 'WARNINGS'],
    ],
    body: [
      [
        `${score} / 100`,
        complianceLabel.toUpperCase(),
        String(checksRun),
        String(checksPassed),
        String(checksFailed),
        String(checksWarn),
  ...
  (doc as any).autoTable({
    ...
    body: [
      [
        ...
        `${drCat.percentage}%`,
        drCat.level,
        `${drCat.passed}/${drCat.findings} pass${input.cdnDetection?.detected ? ' (incl. CDN advisory)' : ''}`,
      ],
      [
        'AI Transparency (HB 149)',
        `${aiCat.percentage}%`,
        aiCat.level,
        `${aiCat.passed}/${aiCat.findings} pass`,
      ],
      [
        'Clinical Integrity',
        `${ciCat.percentage}%`,
        ciCat.level,
        `${ciCat.passed}/${ciCat.findings} pass`,
      ],
    ],
    margin: { left: margin, right: margin },
  ...
  doc.setTextColor(...GRAY);
  const snapDesc =
    'Automated detection of your hosting stack, CDN configuration, email routing, and third-party services. This snapshot is recorded in your Compliance Ledger and compared against future scans to detect infrastructure drift.';
  ...
  const primaryDomain = websiteUrl
    ? websiteUrl.replace(/^https?:\/\//, '').replace(/\/$/, '')
    : 'N/A';
  const primaryNode = borderMap.find((b) => b.type === 'primary' || b.domain === primaryDomain);
  const mxNode = borderMap.find((b) => b.type === 'mail');
  const foreignScripts = borderMap.filter(
    (b) => !b.isSovereign && b.type !== 'primary' && b.type !== 'mail',
  );
  const thirdPartyCount = borderMap.filter(
    (b) => b.type === 'sub-processor' || b.type === 'cdn',
  ).length;
  ...
  const infraRows: string[][] = [
    ...
    [
      'CDN Provider',
      input.cdnDetection?.detected
        ? `${input.cdnDetection.provider || 'Detected'} (detected via ${input.cdnDetection.detectedVia || 'analysis'})`
        : 'None detected',
    ],
  ];
  if (input.cdnDetection?.detected) {
    infraRows.push([
      'CDN Data Residency',
      'US edge nodes active. SB 1188 compliance requires documented proof that patient data routes through US nodes only.',
    ]);
  }
  infraRows.push(
    [
      'Hosting Country',
      primaryNode
        ? `${primaryNode.country || 'United States'}`
        : 'United States (detected via IP geolocation)',
    ],
    [
      'MX Records (Email)',
      mxNode
        ? `${mxNode.provider || mxNode.domain || 'Detected'} (${mxNode.isSovereign ? 'US provider' : 'Foreign'})`
        : 'Detected via scan',
    ],
    [
      'Third-Party Scripts',
      `${thirdPartyCount} detected${foreignScripts.length > 0 ? `, ${foreignScripts.length} flagged as foreign` : ''}`,
    ],
  );
  ...
  const categoryGroups = [
    {
      ...
      prefix: 'DR',
      desc: 'Senate Bill 1188 requires all electronic health records for Texas patients to be physically maintained within the United States. These checks verify that your digital infrastructure routes patient data exclusively through domestic endpoints.',
    },
    {
      key: 'ai_transparency',
      title: 'DETAILED FINDINGS: AI TRANSPARENCY (HB 149)',
      prefix: 'AI',
      desc: 'House Bill 149 requires physicians to disclose when AI tools are used in patient care. These checks verify that your website and patient-facing systems provide clear, conspicuous AI disclosures.',
    },
    {
      key: 'clinical_integrity',
      title: 'DETAILED FINDINGS: CLINICAL INTEGRITY',
      prefix: 'ER',
      desc: 'These checks verify compliance with EHR system integrity requirements, including proper data field handling and patient access controls mandated by Texas administrative code.',
    },
  ];
  ...
  if (borderMap.length > 0) {
    ...
    doc.setTextColor(...GRAY);
    doc.text(
      "IP geolocation analysis of all endpoints detected during the scan. CDN-hosted endpoints are classified based on CDN provider's US edge network presence, not raw IP geolocation.",
      margin,
      y + 2,
    );
    y += 10;
    ...
    doc.setTextColor(...GRAY);
    doc.text(
      `Summary: ${borderMap.length} endpoints mapped. ${usNodes.length} domestic (US), ${foreignNodes.length} foreign.`,
      margin,
      y + 2,
    );
    y += 8;
  ...
  const warnFindings = findings.filter(
    (f) => f.status === 'warn' && (f.recommendedFix || f.technicalFix || f.recommended_fix),
  );
  ...
  if (remediationItems.length > 0) {
    ...
    doc.setTextColor(...GRAY);
    doc.text(
      'Priority-sorted action items with technical fixes. Hand this section to your web developer, hosting provider, or MSP for immediate implementation.',
      margin,
      y + 2,
    );
    y += 10;
    ...
    remediationItems.sort(
      (a, b) => (priOrder[a.severity || 'medium'] || 4) - (priOrder[b.severity || 'medium'] || 4),
    );

    remediationItems.forEach((f) => {
      const fix =
        f.recommendedFix || f.technicalFix || f.recommended_fix || f.detail || f.description || '';
      if (!fix) return;
  ...
  const criticalItems = findings.filter(
  ...
  doc.text(
    'Your 3-month Shield trial includes: 7-tab live dashboard, NPI Integrity monitoring,',
    margin + 6,
    y + 12,
  );
  doc.text(
    'Data Border mapping, drift alerts, and the "Data & AI Trust" badge for your website.',
    margin + 6,
    y + 16,
  );
  doc.setFont('helvetica', 'bold');
  doc.text(
    'Start your trial: https://kairologic.net/scan | Questions: compliance@kairologic.net',
    margin + 6,
    y + 20,
  );
  y += 28;
  ...
  doc.setTextColor(...GRAY);
  doc.text(
    "This report was generated by KairoLogic's automated compliance scanning platform and has been verified against current Texas regulatory requirements.",
    margin,
    y + 2,
  );
  y += 10;
  ...
  (doc as any).autoTable({
    ...
    body: [
      ...
      [
        'Checks Executed',
        `${checksRun} automated compliance checks (${findings
          .map((f) => f.id)
          .filter(Boolean)
          .join(', ')})`,
      ],
      [
        'Data Sources',
        'DNS/IP geolocation, CDN CIDR range database, HTTP response analysis, page content analysis, CNAME pattern matching',
      ],
      [
        'Report Integrity',
        `SHA-256 hash of scan data recorded in Compliance Ledger (Entry: ${ledgerEntryId})`,
      ],
      [
        'Ledger Status',
        'Active | All findings recorded. Infrastructure snapshot stored. Drift monitoring enabled.',
      ],
    ],
    margin: { left: margin, right: margin },
  ...
  doc.setTextColor(...GRAY);
  const disclaimer =
    'This document does not constitute legal advice. Providers should consult qualified legal counsel for regulatory compliance decisions. Compliance status is point-in-time and subject to change with website updates, regulatory changes, or infrastructure modifications.';
  ...
  for (let i = 1; i <= totalPages; i++) {
    ...
    doc.text(
      `Report ID: ${reportId} | Generated: ${reportDate} | Engine: ${engineVersion}`,
      margin,
      pageH - 6,
    );
