...
export async function generateFormData(
  ...
  const providers = await db(
    `providers?npi=eq.${npi}&select=npi,first_name,last_name,address_line_1,city,state,zip_code,phone,primary_taxonomy_code`,
  );
...
export async function generateFormPDF(formData: FormData): Promise<Buffer> {
  ...
  doc.setFontSize(9);
  doc.text(
    `Generated ${new Date(formData.generated_at).toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' })}`,
    pageWidth - margin,
    25,
    { align: 'right' },
  );
  ...
  doc.setFontSize(11);
  doc.text(`Provider: ${formData.provider_name}`, margin, y);
  y += 6;
  doc.setFontSize(9);
  ...
  doc.text(
    `${formData.estimated_manual_time_min} minutes ($${formData.estimated_manual_cost})`,
    margin + 70,
    y + 7,
  );
  doc.setTextColor(51, 65, 85);
  ...
  for (const field of formData.fields) {
    ...
    doc.setFont('helvetica', 'normal');
    doc.setTextColor(148, 163, 184);
    doc.text(`NPPES Section: ${field.nppes_section}`, pageWidth - margin - 3, y + 5.5, {
      align: 'right',
    });
    y += 12;
  ...
  doc.text(`Form ID: ${formData.npi}-${Date.now().toString(36)}`, pageWidth - margin, y, {
    align: 'right',
  });
 ...
 */
export async function generateBulkForms(practiceWebsiteId: string): Promise<BulkFormResult> {
...
export async function generateBulkPDF(bulkResult: BulkFormResult): Promise<Buffer> {
  ...
  doc.setFontSize(10);
  doc.text(
    `${bulkResult.total_providers} providers \u00B7 ${bulkResult.total_mismatches} corrections needed`,
    margin,
    92,
  );
  doc.text(
    `Estimated savings: $${bulkResult.total_estimated_savings.toLocaleString()}`,
    margin,
    100,
  );
  doc.text(
    `Generated: ${new Date(bulkResult.generated_at).toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' })}`,
    margin,
    108,
  );
  ...
  for (const form of bulkResult.forms) {
    doc.text(
      `\u2022  ${form.provider_name} (NPI: ${form.npi}) \u2014 ${form.fields.length} correction${form.fields.length > 1 ? 's' : ''}`,
      margin + 4,
      tocY,
    );
    tocY += 6;
  ...
  for (const form of bulkResult.forms) {
    ...
    doc.setTextColor(148, 163, 184);
    doc.text(
      `NPI: ${form.npi} \u00B7 ${form.fields.length} correction${form.fields.length > 1 ? 's' : ''} needed`,
      margin,
      23,
    );
