...
export async function runCredentialingAssessment(
  ...
  const { data: provider } = await supabase
    .from('providers')
    .select(
      'npi, first_name, last_name, address_line_1, city, state, zip_code, phone, taxonomy_desc',
    )
    .eq('npi', npi)
    .maybeSingle();
  ...
  const { data: payerSnapshots } = await supabase
    ...
    .select(
      'payer_code, listed_address_line1, listed_city, listed_state, listed_phone, snapshot_date',
    )
    .eq('npi', npi)
...
function generateTasksFromAssessment(
  ...
  if (assessment.nppes === 'needs_update') {
    tasks.push({
      ...
      description:
        'Submit address correction to NPPES portal — current address does not match practice',
  ...
  const caqhPullPayers = CAQH_PAYERS.filter(
    (p) =>
      assessment[p as keyof AssessmentResult] === 'wrong_address' ||
      assessment[p as keyof AssessmentResult] === 'not_listed',
  );
  const needsCaqh = caqhPullPayers.length > 0 || assessment.caqh_inferred === 'possibly_stale';
  ...
  if (assessment.pecos === 'needs_reassignment') {
    tasks.push({
      ...
      description:
        'Provider is Medicare-enrolled in a different state — reassign billing to this practice TIN. Expected: 30-60 days.',
      status: 'pending',
      metadata: {
        group: 'submit_wait',
    ...
    tasks.push({
      ...
      description:
        'Provider NPI not found in PECOS enrollment data — submit Medicare enrollment application. Expected: 60-90 days.',
      status: 'pending',
      metadata: {
        group: 'submit_wait',
        portal_url: 'https://pecos.cms.hhs.gov/',
