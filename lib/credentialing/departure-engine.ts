...
export async function runDepartureAssessment(
  ...
  const assessment: Record<string, ListingStatus> = {
    ...
    pecos: (await verifyPecosEnrollment(supabase, npi, practice?.state)).enrolled
      ? 'listed'
      : 'not_listed',
...
function generateDepartureTasks(assessment: Record<string, ListingStatus>): DepartureTask[] {
  ...
  tasks.push({
    ...
    description:
      'Remove provider listing, bio, and photo from "Our Team" and provider directory pages.',
    status: 'active',
  ...
  if (assessment.pecos === 'listed') {
    tasks.push({
      ...
      description:
        'Terminate provider Medicare reassignment to this practice TIN. Expected: 30-60 days.',
      status: 'pending',
      metadata: {
        group: 'submit_wait',
