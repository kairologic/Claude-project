...
const CDN_CIDR_DEFINITIONS: { provider: string; ranges: string[] }[] = [
  ...
  {
    provider: 'Azure CDN',
    ranges: ['13.107.246.0/24', '13.107.213.0/24', '152.199.0.0/16'],
  },
  {
    provider: 'Vercel',
    ranges: ['76.76.21.0/24', '76.223.0.0/16'],
  },
];
 ...
 */
export function detectCDNByIP(ip: string): {
  detected: boolean;
  provider: string;
  method: 'ip_range';
} {
 ...
 */
export async function detectCDNByCNAME(
  domain: string,
): Promise<{ detected: boolean; provider: string; cname: string; method: 'cname' }> {
  try {
...
export function detectCDNByHeaders(headers: Record<string, string>): {
  detected: boolean;
  provider: string;
  method: 'headers';
  evidence: string[];
...
export async function detectCDN(
  ...
  if (headerResult.detected) {
    return {
      detected: true,
      provider: headerResult.provider,
      detectedVia: 'headers',
      confidence: 'high',
