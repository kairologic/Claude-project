...
export async function runAcceptanceGapCheck(
  ...
  console.log(`[AcceptanceGap] Checking ${practices.length} practices with claimed payer acceptance.`);
  ...
  for (const practice of practices) {
    ...
    if (needsReverification.length > 0 && options.fhirClient && options.payerEndpoints) {
      ...
      console.log(`[AcceptanceGap] Re-verifying ${needsReverification.length} not-listed providers for ${practice.name || practice.id}...`);
      ...
      for (const item of needsReverification) {
        ...
        try {
          const result = await options.fhirClient.verifyNotListed(
            item.npi,
            item.name,
            endpoint,
          );
          ...
          if (result.found) {
            console.log(`  [reverify] ✓ ${item.npi} FOUND in ${item.payer_code} — false positive corrected`);
          } else {
    ...
    const gaps = detectAcceptanceGaps(
      practice.id,
      practice.accepted_payers,
      providerStatuses,
      { name: practice.name, url: practice.url },
    );
