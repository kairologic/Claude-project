...
export function detectMismatches(
  ...
  const isListed =
    snapshot.listed_name_full || snapshot.listed_name_last || snapshot.listed_address_line1;
  ...
  // ── Address comparison ──
  const nppesAddr = normalizeAddress(nppes.address_line_1, nppes.city, nppes.state, nppes.zip);
 ...
 */
export function buildCorrectionActions(mismatches: DirectoryMismatch[]): CorrectionAction[] {
  ...
  if (nppesNeeded.length > 0) {
    ...
    actions.push({
      ...
      effort: '10 min',
      details:
        'Submit update at nppes.cms.hhs.gov or use the KairoLogic pre-filled NPPES update form.',
    });
  }
 ...
 */
export function buildAcceptanceGapMismatch(gap: AcceptanceGapInput): DirectoryMismatch {
