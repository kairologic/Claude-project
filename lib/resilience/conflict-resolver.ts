...
export function detectConflict(
  ...
  const othersDisagreeWithNppes = nppesValue && valid.some(
    (dp) => dp.source !== 'nppes' && normalize(dp.value) !== nppesValue
  );
...
export function autoResolve(
  ...
  const recommended = conflict.data_points.find(
    (dp) => dp.source === conflict.recommended_source
  );
  ...
  const agreeing = conflict.data_points.filter(
    (dp) => normalize(dp.value) === recNorm
  );
