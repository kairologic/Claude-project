...
export function calculateWorkflowHealth(workflow: {
  ...
  const daysOverdue = overdue && now > overdue
    ? Math.floor((now - overdue) / 86_400_000)
    : 0;
  ...
  const score = Math.max(0, Math.round(100 - agePenalty - overduePenalty - stalledPenalty - errorPenalty));
...
export async function bulkCancelStale(
  ...
  if (dryRun) {
    console.log(`[DRY RUN] Would cancel ${staleWfs.length} stale workflows (>${staleDays} days old)`);
  ...
  const events = staleWfs.map((wf) => ({
    ...
    details: {
      ...
      days_open: Math.floor(
        (Date.now() - new Date(wf.created_at).getTime()) / 86_400_000
      ),
    },
  }));
