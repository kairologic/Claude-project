...
const PATTERNS = {
  ...
  physicianEntry:
    /([A-Z][a-z]+(?:\s+[A-Z][a-z]+)*),\s*([A-Z][a-z]+(?:\s+[A-Z][a-z]+)*(?:\s+[A-Z]\.?)?),\s*(?:M\.D\.|D\.O\.),\s*Lic\.\s*No\.\s*([A-Z]\d+),\s*(.+?)(?:\s*$|\s*·)/gm,
...
export class TMBNewsroomMonitor {
  ...
  async backfill(
      ...
      try {
        ...
        if (cutoffDate) {
          ...
          if (oldestOnPage < cutoffDate) {
            ...
            console.log(
              `[TMB Backfill] Page ${page + 1}: ${filtered.length}/${releases.length} releases after cutoff`,
            );
        ...
        if (newReleases.length > 0) {
          console.log(
            `[TMB Backfill] Page ${page + 1}: ${newReleases.length} new releases to process`,
          );
          await this.processReleases(newReleases, result);
        } else {
    ...
    console.log(
      `[TMB Backfill] Complete: ${totalReleases} new releases across ${pages} pages${stoppedEarly ? ' (stopped at cutoff date)' : ''}`,
    );
    return result;
  }
  ...
  async fetchNewsroomPage(page: number = 0): Promise<PressRelease[]> {
    const url = page === 0 ? TMB_NEWSROOM_URL : `${TMB_NEWSROOM_URL}?page=${page}`;
  ...
  parseNewsroomHtml(html: string): PressRelease[] {
    ...
    const rowPattern =
      /<a\s+href="(\/about-us\/newsroom\/[^"]+)"[^>]*>([^<]+)<\/a>[\s\S]*?(\d{1,2}\/\d{1,2}\/\d{4})/g;
    let match;
  ...
  async processEmergencySuspension(release: PressRelease): Promise<number> {
    ...
    if (!matches || matches.length === 0) {
      console.log(
        `[TMB Monitor] No match found for ${release.physicianName} (${licenseNumber || 'no lic#'})`,
      );
      return 0;
    }
    ...
    console.log(
      `[TMB Monitor] Created ${eventsCreated} delta event(s) for ${release.physicianName}`,
    );
  ...
  async processBoardMeetingRelease(release: PressRelease): Promise<{
    ...
    console.log(
      `[TMB Monitor] Board meeting: ${actions.length} total, ${matched} matched, ${eventsCreated} events`,
    );
  ...
  private parseBoardMeetingActions(html: string, release: PressRelease): BoardMeetingAction[] {
    ...
    const entryPattern =
      /([A-Z][a-zA-Z'-]+(?:\s+[A-Z][a-zA-Z'-]+)*),\s*([A-Z][a-zA-Z'-]+(?:\s+[A-Z][a-zA-Z'-]+)*(?:\s+[A-Z]\.?)?),\s*(?:M\.D\.|D\.O\.),\s*Lic\.\s*No\.\s*([A-Z]\d{3,5}),\s*([A-Za-z\s.'-]+?)(?:\s*(?:·|$|\.))/g;
    let match;
