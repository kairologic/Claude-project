...
export function scorePractice(signals: PracticeSignals): OutreachScore {
  ...
  const total = components.mismatch_severity
  ...
  let action: string;
  if (tier === 'tier1') {
    action = 'Immediate outreach: high mismatch volume + compliance risk. Personalized email with compliance report.';
  } else if (tier === 'tier2') {
    action = 'Priority outreach: significant mismatches. Send automated report with ROI calculator link.';
  } else if (tier === 'tier3') {
...
export function toCSVRows(scores: OutreachScore[]): string {
  const header = 'practice_id,practice_name,state,url,total_score,tier,mismatch_severity,provider_scale,compliance_risk,website_quality,recency,recommended_action';
