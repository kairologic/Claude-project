    ...
    <div>
      ...
      <div className="flex items-center justify-between mb-8">
        ...
        <button
          ...
          className="flex items-center gap-2 bg-navy text-white px-4 py-2 rounded-lg hover:bg-gold hover:text-navy transition-colors text-sm font-bold"
        >
          <RefreshCw size={14} />
          Refresh
        </button>
      ...
      {loading ? (
        ...
        <div className="space-y-4">
          {templates.map((t) => (
            <div
              key={t.id}
              className={`bg-white rounded-xl border ${editing === t.id ? 'border-navy shadow-lg' : 'border-gray-200'} overflow-hidden`}
            >
              {/* Template Header */}
              <div className="flex items-center justify-between p-5">
                <div className="flex items-center gap-4">
                  <div
                    className={`w-3 h-3 rounded-full ${t.is_active ? 'bg-green-500' : 'bg-gray-300'}`}
                  />
                  <div>
                ...
                <div className="border-t border-navy/20 bg-slate-50 p-5 space-y-4">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <select
                        ...
                        onChange={(e) =>
                          setEditForm({ ...editForm, trigger_event: e.target.value })
                        }
                        className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm"
                      >
                    ...
                    <div>
                      ...
                      <select
                        ...
                        onChange={(e) =>
                          setEditForm({ ...editForm, recipient_type: e.target.value })
                        }
                        className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm"
                      >
                  ...
                  <div>
                    ...
                    <p className="text-xs text-gray-400 mt-1">
                      Variables: {'{{'} practice_name {'}}'}, {'{{'} npi {'}}'}, {'{{'} status_label{' '}
                      {'}}'}, {'{{'} report_id {'}}'}, {'{{'} score {'}}'}, {'{{'} year {'}}'}
                    </p>
                  </div>
                  <div className="flex items-center gap-3">
                    <button
                      onClick={saveTemplate}
                      className="flex items-center gap-2 bg-navy text-white px-6 py-2.5 rounded-lg font-bold text-sm hover:bg-gold hover:text-navy transition-colors"
                    >
                      <Save size={14} />
                      Save Changes
                    </button>
