...
export function DriftTab({ showNotification }: DriftTabProps) {
  ...
  const staleWidgets = providerSummaries.filter(
    (s) => s.heartbeats.length > 0 && !s.widgetActive,
  ).length;
  ...
  const selectedProvider = selectedNpi
    ? providerSummaries.find((s) => s.npi === selectedNpi)
    : null;
      ...
      <div className="space-y-4">
        ...
        <div className="bg-white rounded-xl border border-slate-100 shadow-sm p-4">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              {sp.tier === 'shield' && (
                <button
                  onClick={() => generateDashboardToken(sp.npi)}
              ...
              {sp.dashboardToken && (
                <a
                  href={`/dashboard/${sp.npi}?token=${sp.dashboardToken}`}
                  target="_blank"
              ...
              {sp.newCount > 0 && (
                <button
                  onClick={() => bulkResolveProvider(sp.npi)}
        ...
        <div className="grid grid-cols-2 sm:grid-cols-5 gap-3">
          {[
            ...
            {
              label: 'Critical',
              value: sp.criticalCount,
              color: sp.criticalCount > 0 ? 'text-red-700' : 'text-slate-800',
              alert: sp.criticalCount > 0,
            },
            {
              label: 'High',
              value: sp.highCount,
              color: sp.highCount > 0 ? 'text-orange-700' : 'text-slate-800',
              alert: sp.highCount > 0,
            },
            {
              label: 'Open',
              value: sp.newCount,
              color: sp.newCount > 0 ? 'text-amber-700' : 'text-slate-800',
            },
            ...
            <div className="divide-y divide-slate-50">
              {sp.heartbeats.map((hb, i) => {
                ...
                return (
                  <div
                    key={i}
                    className={`px-4 py-2.5 flex items-center justify-between text-xs ${stale ? 'bg-amber-50/50' : ''}`}
                  >
                    <div className="flex items-center gap-2">
                      {stale ? (
                      ...
                      <span
                        className={`px-1.5 py-0.5 rounded text-[9px] font-bold ${hb.widget_mode === 'shield' ? 'bg-green-100 text-green-700' : 'bg-blue-100 text-blue-700'}`}
                      >
        ...
        <div className="flex items-center gap-2">
          ...
          {['all', 'critical', 'high', 'medium', 'low'].map((sev) => (
            ...
            >
              {sev === 'all'
                ? `All (${sp.events.length})`
                : `${sev} (${sp.events.filter((e) => e.severity === sev).length})`}
            </button>
          ))}
        </div>
          ...
          <div className="space-y-2">
            {selectedEvents.map((event) => {
              ...
              return (
                <div
                  key={event.id}
                  className={`bg-white rounded-xl border shadow-sm overflow-hidden ${sevConfig.border}`}
                >
                  <div
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2">
                        ...
                        <span
                          className={`px-1.5 py-0.5 rounded text-[9px] font-bold uppercase ${sevConfig.bg} ${sevConfig.text}`}
                        >
                          {event.severity}
                        </span>
                      </div>
                      <div className="text-[10px] text-slate-500 mt-0.5">
                        {DRIFT_TYPE_LABELS[event.drift_type] || event.drift_type} ·{' '}
                        <span className="font-mono">{event.page_url}</span>
                      </div>
                    </div>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mt-3">
                        {event.content_before && (
                          <div>
                            ...
                            <div className="bg-red-50 rounded-lg p-3 text-[10px] font-mono text-red-800 max-h-28 overflow-y-auto whitespace-pre-wrap break-all">
                              {event.content_before}
                            </div>
                          </div>
                        )}
                        {event.content_after !== null && (
                          <div>
                            ...
                            <div className="bg-green-50 rounded-lg p-3 text-[10px] font-mono text-green-800 max-h-28 overflow-y-auto whitespace-pre-wrap break-all">
                      ...
                      <div className="mt-3 flex flex-wrap gap-3 text-[10px] text-slate-500">
                        <span>
                          ...
                          <code className="font-mono">
                            ...
                            {(event.current_hash || '').substring(0, 12)}
                          </code>
                        </span>
                        <span>
                          <strong>Detected:</strong> {new Date(event.created_at).toLocaleString()}
                        </span>
                        {event.resolved_at && (
                          <span>
                            <strong>Resolved:</strong>{' '}
                            {new Date(event.resolved_at).toLocaleString()} by {event.resolved_by}
                          </span>
                        )}
                      </div>
                      {event.status === 'new' && (
                        <div className="mt-3 flex items-center gap-2">
                          <button
                            onClick={(e2) => {
                              e2.stopPropagation();
                              updateEventStatus(event.id, 'acknowledged');
                            }}
                          ...
                          <button
                            onClick={(e2) => {
                              e2.stopPropagation();
                              updateEventStatus(event.id, 'resolved');
                            }}
                            className="flex items-center gap-1 px-3 py-1.5 text-[10px] font-bold bg-green-100 text-green-700 hover:bg-green-200 rounded-lg"
                          >
                          ...
                          <button
                            onClick={(e2) => {
                              e2.stopPropagation();
                              updateEventStatus(event.id, 'false_positive');
                            }}
                            className="flex items-center gap-1 px-3 py-1.5 text-[10px] font-bold bg-slate-100 text-slate-600 hover:bg-slate-200 rounded-lg"
                          >
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3">
        ...
        <div
          className={`rounded-xl p-4 border shadow-sm ${totalCritical > 0 ? 'bg-red-50 border-red-200' : 'bg-white border-slate-100'}`}
        >
          <div
            className={`text-[10px] font-bold uppercase tracking-wider ${totalCritical > 0 ? 'text-red-500' : 'text-slate-400'}`}
          >
            Critical
          </div>
          <div
            className={`text-2xl font-black mt-1 ${totalCritical > 0 ? 'text-red-700' : 'text-slate-800'}`}
          >
            {totalCritical}
          </div>
        </div>
        <div
          className={`rounded-xl p-4 border shadow-sm ${totalNew > 0 ? 'bg-amber-50 border-amber-200' : 'bg-white border-slate-100'}`}
        >
          <div
            className={`text-[10px] font-bold uppercase tracking-wider ${totalNew > 0 ? 'text-amber-500' : 'text-slate-400'}`}
          >
            Unresolved
          </div>
          <div
            className={`text-2xl font-black mt-1 ${totalNew > 0 ? 'text-amber-700' : 'text-slate-800'}`}
          >
            {totalNew}
          </div>
        </div>
        ...
        <div
          className={`rounded-xl p-4 border shadow-sm ${staleWidgets > 0 ? 'bg-amber-50 border-amber-200' : 'bg-white border-slate-100'}`}
        >
          <div
            className={`text-[10px] font-bold uppercase tracking-wider ${staleWidgets > 0 ? 'text-amber-500' : 'text-slate-400'}`}
          >
            Stale (48h+)
          </div>
          <div
            className={`text-2xl font-black mt-1 ${staleWidgets > 0 ? 'text-amber-700' : 'text-slate-800'}`}
          >
            {staleWidgets}
          </div>
        </div>
      ...
      {filteredSummaries.length === 0 ? (
        <div className="bg-white rounded-xl border border-slate-100 shadow-sm p-12 text-center">
          ...
          <p className="text-xs text-slate-400 mt-1">
            Drift events will appear when widgets are installed and baselines are seeded.
          </p>
        </div>
      ) : (
        <div className="space-y-2">
              ...
              >
                <div className="px-5 py-4 flex items-center gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="text-[10px] text-slate-400 mt-0.5">
                      ...
                      {summary.heartbeats.length > 0 && (
                        <>
                          {' '}
                          · {summary.heartbeats.length} page
                          {summary.heartbeats.length !== 1 ? 's' : ''} monitored
                        </>
                      )}
                    </div>
                  </div>
                  ...
                  <div className="flex items-center gap-3 flex-shrink-0">
                    ...
                    <div className="text-center">
                      <div
                        className={`text-lg font-black ${summary.newCount > 0 ? 'text-amber-700' : 'text-green-600'}`}
                      >
