  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-3 sm:grid-cols-6 gap-2">
        ...
        <div
          className={`rounded-lg p-3 shadow-sm border text-center ${stats.unread > 0 ? 'bg-red-50 border-red-200' : 'bg-white'}`}
        >
          <div className={`text-lg font-bold ${stats.unread > 0 ? 'text-red-600' : ''}`}>
            {stats.unread}
          </div>
      ...
      {loading ? (
        ...
        <div className="text-center py-16 bg-white rounded-xl border">
          ...
          <p className="text-xs text-slate-300 mt-1">
            Prospects appear here when visitors scan, fill forms, or book calls
          </p>
        </div>
      ) : (
                    ...
                    <React.Fragment key={p.id}>
                      ...
                      >
                        ...
                        <td className="px-4 py-3">
                          <span
                            className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full border text-[9px] font-bold uppercase ${src.color}`}
                          >
                            {src.icon} {src.label}
                          </span>
                        </td>
                        ...
                        <td className="px-4 py-3">
                          <div className="text-xs text-slate-500 max-w-[200px] truncate">
                            {p.source === 'scan' && p.scan_score !== undefined && (
                              <span
                                className={`font-bold ${p.scan_score >= 67 ? 'text-emerald-600' : p.scan_score >= 34 ? 'text-amber-600' : 'text-red-600'}`}
                              >
                        ...
                        <td className="px-4 py-3">
                          <span
                            className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full border text-[9px] font-bold uppercase ${st.color}`}
                          >
                            {st.icon} {p.status}
                          </span>
                        </td>
                        ...
                        <td className="px-4 py-3">
                          <span
                            className={`text-[9px] font-bold uppercase tracking-wide ${PRIORITY_CONFIG[p.priority] || ''}`}
                          >
                            {p.priority}
                          </span>
                        </td>
                        ...
                        <td className="px-4 py-3 text-right" onClick={(e) => e.stopPropagation()}>
                          <div className="flex items-center justify-end gap-1">
                            ...
                            {p.form_data && Object.keys(p.form_data).length > 0 && (
                              <button
                                onClick={() =>
                                  setShowFormDataId(showFormDataId === p.id ? null : p.id)
                                }
                          ...
                          >
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-xs">
                              ...
                              <div className="space-y-2">
                                ...
                                {p.phone && (
                                  <div>
                                    ...
                                    <span className="font-bold text-slate-700">{p.phone}</span>
                                  </div>
                                )}
                                {p.npi && (
                                  <div>
                              ...
                              <div className="space-y-2">
                                  ...
                                  <>
                                    ...
                                    {p.scan_risk_level && (
                                      <div>
                                        ...
                                        <span className="font-bold">{p.scan_risk_level}</span>
                                      </div>
                                    )}
                                    {p.scan_report_id && (
                                      <div>
                                ...
                                {p.source === 'contact' && (
                                  <>
                                    ...
                                    {p.message && (
                                      <div className="bg-white rounded-lg p-2 border mt-1 text-slate-600 whitespace-pre-wrap max-h-32 overflow-y-auto">
                                        {p.message}
                                      </div>
                                    )}
                                  </>
                                  ...
                                  <>
                                    {p.appointment_date && (
                                      <div>
                                        ...
                                        <span className="font-bold">{p.appointment_date}</span>
                                      </div>
                                    )}
                                    {p.appointment_time && (
                                      <div>
                                        <span className="text-slate-500">Time:</span>{' '}
                                        <span className="font-bold">{p.appointment_time}</span>
                                      </div>
                                    )}
                                    {p.meeting_url && (
                                      <div>
                                        ...
                                        <a
                                          href={`https://${p.meeting_url}`}
                                          target="_blank"
                                          rel="noopener noreferrer"
                                          className="font-bold text-blue-600 hover:underline flex items-center gap-1"
                                        >
                                          {p.meeting_url} <ExternalLink size={10} />
                                        </a>
                                      </div>
                                    )}
                                  </>
                              ...
                              <div className="space-y-2">
                                ...
                                {p.admin_notes && (
                                  <div className="bg-white rounded-lg p-2 border text-slate-600 whitespace-pre-wrap">
                                    {p.admin_notes}
                                  </div>
                                )}
                                {p.assigned_to && (
                                ...
                                <div className="flex gap-1 pt-2">
                                  <button
                                    onClick={() => startEdit(p)}
                                    className="px-2.5 py-1 bg-[#00234E] text-white rounded text-[9px] font-bold uppercase flex items-center gap-1 hover:bg-[#C5A059]"
                                  >
                                    <Edit size={10} /> Edit
                                  </button>
                                  {p.form_data && Object.keys(p.form_data).length > 0 && (
                                    <button
                                      ...
                                      className="px-2.5 py-1 bg-blue-600 text-white rounded text-[9px] font-bold uppercase flex items-center gap-1 hover:bg-blue-700"
                                    >
                            ...
                            {showFormDataId === p.id && p.form_data && (
                              <div className="mt-3 bg-white rounded-lg border p-3">
                                <div className="flex items-center justify-between mb-2">
                                  ...
                                  <button
                                    onClick={() => setShowFormDataId(null)}
                                ...
                                <div className="grid grid-cols-2 gap-2 text-xs">
                                  {Object.entries(p.form_data).map(([key, val]) => (
                                    <div key={key} className="flex gap-2">
                                      <span className="text-slate-400 font-mono text-[10px] min-w-[120px]">
                                        {key}:
                                      </span>
                      ...
                      {isEditing && (
                        <tr>
                          <td
                            colSpan={7}
                            className="bg-amber-50/50 px-4 py-4 border-t border-amber-100"
                          >
                            <div className="space-y-3">
                              ...
                              <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                                ...
                                <div>
                                  ...
                                  <input
                                    ...
                                    onChange={(e) =>
                                      setEditForm({ ...editForm, practice_name: e.target.value })
                                    }
                                    className="w-full px-2.5 py-1.5 text-sm bg-white border border-slate-200 rounded-lg focus:ring-2 focus:ring-[#C5A059] focus:outline-none"
                                  />
                                </div>
                                <div>
                                  ...
                                  <input
                                    ...
                                    value={editForm.npi || ''}
                                    onChange={(e) =>
                                      setEditForm({ ...editForm, npi: e.target.value })
                                    }
                                ...
                                <div>
                                  ...
                                  <select
                                    ...
                                    onChange={(e) =>
                                      setEditForm({
                                        ...editForm,
                                        priority: e.target.value as Prospect['priority'],
                                      })
                                    }
                                    className="w-full px-2.5 py-1.5 text-sm bg-white border border-slate-200 rounded-lg"
                                  >
                              ...
                              <div className="flex justify-end gap-2 pt-2 border-t border-amber-200">
                                ...
                                <button
                                  onClick={saveEdit}
                                  className="px-4 py-1.5 bg-[#00234E] text-white text-xs font-bold rounded-lg hover:bg-[#C5A059] flex items-center gap-1"
                                >
