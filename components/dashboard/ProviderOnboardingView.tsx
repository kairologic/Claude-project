...
export default function ProviderOnboardingView({
  ...
  const handleLookup = useCallback(async () => {
    ...
    try {
      const { data, error } = await supabase
        .from('providers')
        .select('first_name, last_name, address_line_1, city, state, zip_code, phone, primary_taxonomy_code, taxonomy_desc')
        .eq('npi', npiInput)
        .single();
  ...
  const activeOnboardings = workflows.filter((w) => w.status !== 'completed' && w.status !== 'cancelled');
          ...
          <div key={workflow.id} style={cardStyle}>
            <div style={cardContentStyle}>
              ...
              <div style={providerDetailsStyle}>
                NPI: {workflow.provider_npi} | Created: {new Date(workflow.created_at).toLocaleDateString()}
              </div>
              ...
              <>
                {isDuplicate && (
                  <div style={warningStyle}>
                    ⚠️ This provider is already on your roster. You may want to update their existing record instead.
                  </div>
                )}
                ...
                <div style={resultsPanelStyle}>
                  ...
                  <div style={resultDetailStyle}>
                    <strong>Address:</strong> {providerData.address_line_1}, {providerData.city}, {providerData.state}{' '}
