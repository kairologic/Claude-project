...
const ONBOARDING_STAGES: Stage[] = [
  ...
  { key: 'monitoring', label: 'MONITOR', taskTypes: ['monitor_nppes', 'monitor_payer_directory', 'monitor_pecos', 'monitor_auto_confirm'] },
...
function computeStageStatus(stage: Stage, tasks: Task[], stageIndex: number, totalStages: number): StageStatus {
  ...
  if (stage.key === 'complete') {
    ...
    const allDone = nonMonitorTasks.length > 0 && nonMonitorTasks.every(t => t.status === 'completed' || t.status === 'skipped');
...
export default function CredentialingTimeline({
  ...
  const stages = workflowType === 'credentialing_onboarding'
    ? ONBOARDING_STAGES
    : DEPARTURE_STAGES;
  ...
  return (
    <div style={{ padding: '4px 0' }}>
        ...
        {stages.map((stage, i) => {
          ...
          return (
            <div key={stage.key} style={{
              display: 'flex', alignItems: 'center', flex: isLast ? '0 0 auto' : 1,
            }}>
              {/* Circle */}
              <div style={{
                position: 'relative',
                ...
                <div style={{
                  ...
                  ...circleStyle(status),
                  ...(status === 'active' ? { animation: 'credPulse 2s ease-in-out infinite' } : {}),
                }}>
              ...
              {!isLast && (
                <div style={{
                  flex: 1, height: 2, minWidth: 12, marginBottom: 14,
                  background: stageStatuses[i + 1] === 'completed' || status === 'completed'
                    ? colors.green
                    : colors.gray200,
                  ...(stageStatuses[i + 1] === 'pending' || stageStatuses[i + 1] === 'skipped'
                    ? { backgroundImage: `repeating-linear-gradient(90deg, ${colors.gray200} 0, ${colors.gray200} 4px, transparent 4px, transparent 8px)`, background: 'none', backgroundSize: '8px 2px', backgroundRepeat: 'repeat-x' }
                    : {}),
                }} />
              )}
            </div>
      ...
      {estDate && (
        <div style={{
          fontSize: 9, color: colors.gray400, textAlign: 'right',
...
function circleStyle(status: StageStatus): React.CSSProperties {
  switch (status) {
    ...
    case 'skipped':
      return { background: colors.gray100, color: colors.gray200, border: `2px solid ${colors.gray200}` };
    case 'pending':
    default:
