  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
      {entries.map(([key, value]) => {
        ...
        const effectiveKey =
          key === 'approved_value' && fieldContext === 'phone'
            ? 'phone'
            : key === 'approved_value' && fieldContext === 'address_line_1'
              ? 'address'
              : key;
        ...
        const finalValue =
          key === 'field'
            ? FIELD_LABELS[value as string] || titleCaseValue(value as string)
            : displayValue;
        ...
        return (
          <div key={key} style={{ display: 'flex', gap: 8, alignItems: 'baseline' }}>
            ...
            <span style={{ fontSize: 12, color: '#1e293b', fontWeight: 500 }}>{finalValue}</span>
          </div>
        );
      })}
    </div>
  ...
  const filteredEvents = useMemo(() => {
    return events.filter((event) => {
      ...
      const matchesSearch =
        ...
        (workflowMap[event.workflow_id]?.workflow_type || '')
          .toLowerCase()
    ...
    <div style={{ padding: '32px', backgroundColor: '#ffffff', minHeight: '100vh' }}>
      <div style={{ maxWidth: '1200px', margin: '0 auto' }}>
        ...
        <div style={{ marginBottom: '32px' }}>
          <h1
            style={{ fontSize: '32px', fontWeight: '700', color: '#1e293b', marginBottom: '8px' }}
          >
