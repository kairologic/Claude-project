...
export default function ProviderReleaseView({
  ...
  const handleStartReleaseWorkflow = useCallback(async () => {
    ...
    try {
      ...
      setSuccessMessage(
        `Provider departure workflow started for ${provider.provider_name}`
      );
      ...
      setError(
        err instanceof Error ? err.message : 'Error creating release workflow'
      );
    } finally {
      setIsCreating(false);
    }
  }, [selectedProvider, departureDate, departureReason, departureNotes, activeProviders, practiceId, supabase]);
  ...
  const getDaysFromDeparture = (workflow: Workflow): number | null => {
    ...
    const days = Math.floor(
      (Date.now() - new Date(departureDate).getTime()) / 86400000
    );
    return days;
  };
    ...
    <div style={containerStyle}>
      ...
      <div style={{ marginBottom: '40px' }}>
        <h2 style={sectionTitleStyle}>
          Active Release Workflows ({activeReleases.length})
        </h2>
        ...
        {activeReleases.length === 0 ? (
          ...
          <div style={workflowsGridStyle}>
            {activeReleases.map((workflow) => {
              ...
              const isMonitoring =
                daysFromDeparture !== null && daysFromDeparture <= 90;
              ...
              return (
                <div key={workflow.id} style={{ position: 'relative' }}>
                  ...
                  {isMonitoring && daysFromDeparture !== null && (
                    ...
                    >
                      <div style={monitoringIndicatorStyle}>
                        ⏱ {daysFromDeparture} days
                      </div>
                    </Tooltip>
                  )}
                </div>
        ...
        <div style={checklistContainerStyle}>
          {releaseTaskTemplate.map((task, idx) => (
            <div
              key={task.order}
              style={
                idx === releaseTaskTemplate.length - 1
                  ? checklistItemLastStyle
                  : checklistItemStyle
              }
            >
