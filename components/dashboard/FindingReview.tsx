...
export default function FindingReview({ finding, comparisonData, onProceedToApprove }: FindingReviewProps) {
  const fieldName = finding.field || 'Unknown field';
  const websiteValue = finding.website_value || '';
  const npbesValue = finding.nppes_value || '';
  const hasMultipleSources = comparisonData && comparisonData.sources && comparisonData.sources.length > 0;
              ...
              {comparisonData!.sources.map((source, idx) => {
                ...
                return (
                  <tr
                    key={idx}
                    style={{
                      borderBottom: idx < comparisonData!.sources.length - 1 ? `1px solid ${colors.gray200}` : 'none',
