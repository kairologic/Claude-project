...
export function SeverityBadge({ severity }: { severity: string }) {
  ...
  const bgMap: Record<string, string> = {
    action: colors.redPale,
    warning: colors.goldPale,
    info: colors.bluePale,
  ...
  return (
    <span
      style={{
        ...
        padding: '3px 10px',
        borderRadius: radii.full,
        background: bgMap[severity] || colors.gray100,
        color,
        display: 'inline-block',
      }}
    >
...
export function WorkflowCard({ workflow: wf, onClick }: WorkflowCardProps) {
  ...
  const typeInfo = workflowTypeLabels[wf.workflow_type] || {
    label: wf.workflow_type.toUpperCase(),
    tooltip: '',
  };
  const isOverdue =
    wf.overdue_at && new Date(wf.overdue_at) < new Date() && wf.status === 'action_needed';
  ...
  return (
    <div
      ...
      role="button"
      tabIndex={0}
      aria-label={`${typeInfo.label} workflow for ${wf.provider_name || 'Unknown'}`}
      onKeyDown={(e) => {
      ...
      onMouseOver={(e) => {
        const el = e.currentTarget as HTMLElement;
        el.style.borderLeftColor = colors.navy;
        el.style.boxShadow = shadows.md;
    ...
    >
      ...
      <div style={{ fontSize: 11, color: colors.gray600, marginBottom: 8, lineHeight: 1.4 }}>
        {desc}
      </div>
      {wf.status !== 'resolved' && (
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          ...
          <span style={{ fontSize: 10, fontWeight: 600, color: dueColor, whiteSpace: 'nowrap' }}>
            {dueText}
          </span>
        </div>
      )}
