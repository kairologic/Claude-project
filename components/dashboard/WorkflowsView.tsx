...
function WorkflowsViewInner({ workflows, practiceId, counts }: WorkflowsViewProps) {
  ...
  const filtered = useMemo(() => {
    let list =
      activeFilter === 'all' ? workflows : workflows.filter((w) => w.status === activeFilter);
    ...
    return [...list].sort((a, b) => {
      const aOverdue =
        a.overdue_at && new Date(a.overdue_at) < new Date() && a.status === 'action_needed' ? 1 : 0;
      const bOverdue =
        b.overdue_at && new Date(b.overdue_at) < new Date() && b.status === 'action_needed' ? 1 : 0;
  ...
  function closeDetail() {
    ...
    // Clean URL param
    router.replace(
      `/practice/${practiceId}/workflows${activeFilter !== 'all' ? `?status=${activeFilter}` : ''}`,
      { scroll: false },
    );
  }
  ...
  return (
    <div>
      {/* Filter bar */}
      <div
        style={{ display: 'flex', gap: 6, marginBottom: spacing.lg, flexWrap: 'wrap' }}
        role="tablist"
      >
        {filters.map((f) => {
          ...
          return (
            <button
              ...
              onMouseOver={(e) => {
                if (!isActive) (e.currentTarget as HTMLElement).style.borderColor = colors.navy;
              }}
              onMouseOut={(e) => {
                if (!isActive) (e.currentTarget as HTMLElement).style.borderColor = colors.gray200;
              }}
            >
              {f.label} ({count})
            </button>
        ...
        {typeFilters.map((f) => {
          ...
          return (
            <button
              ...
              onMouseOver={(e) => {
                if (!isActive) (e.currentTarget as HTMLElement).style.borderColor = colors.gold;
              }}
              onMouseOut={(e) => {
                if (!isActive) (e.currentTarget as HTMLElement).style.borderColor = colors.gray200;
              }}
      ...
      {/* Results count + Correction Packet button */}
      <div
        style={{
          display: 'flex',
      ...
      {/* Detail panel */}
      {detailId && (
        <WorkflowDetailPanel workflowId={detailId} practiceId={practiceId} onClose={closeDetail} />
      )}
    </div>
  );
}
