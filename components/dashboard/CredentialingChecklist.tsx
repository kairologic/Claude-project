...
interface CredentialingChecklistProps {
  workflowId: string;
...
/**
 * CredentialingChecklist — for onboarding workflows.
 * Groups tasks into Immediate / Submit & Wait / Monitoring sections.
 * Each task shows its portal link and current status.
 */
export default function CredentialingChecklist({
  workflowId,
  providerName,
  tasks,
  onTaskAction,
  onBack,
}: CredentialingChecklistProps) {
  ...
  const groupOrder = ['immediate', 'submit_wait', 'monitoring', 'complete'];

  // Map task types to portal info
  function getPortalForTask(taskType: string): { name: string; url: string } | null {
    const map: Record<string, string> = {
      enroll_caqh: 'caqh',
      credential_payers: 'uhc', // links to CAQH since most pull from it
      enroll_pecos: 'pecos',
      update_website: '',
      npi_lookup: 'nppes',
    };
      ...
      <div>
        ...
        <div style={{
          ...
          <div style={{
            height: '100%',
            width: `${progress}%`,
            background: progress === 100 ? colors.green : colors.blue,
            borderRadius: 3,
                ...
                return (
                  ...
                  >
                    ...
                    <div style={{ flex: 1 }}>
                      ...
                      {/* Action row — portal link + mark complete */}
                      {task.status === 'active' && (
