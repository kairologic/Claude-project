...
const TECHNICAL_FIXES = {
  'DR-01': {
    technical_finding:
      'Primary EHR domain resolves to IP addresses located outside the United States. Server geolocation analysis indicates hosting infrastructure in non-US regions, violating Texas data sovereignty requirements.',
...
6. Obtain compliance attestation from hosting provider confirming US data residency`,
  ...
  'DR-02': {
    technical_finding:
      'CDN edge nodes detected serving EHR content from non-US locations. HTTP headers indicate cache nodes in EU or APAC regions, creating potential data sovereignty violations.',
...
6. Test from multiple US locations to confirm edge node compliance`,
  ...
  'DR-03': {
    technical_finding:
      'Mail exchange (MX) records route through servers with OCONUS (Outside Continental US) IP addresses. PHI transmitted via email may transit foreign infrastructure.',
...
7. Add SPF/DKIM records specifying US-only authorized mail servers`,
  ...
  'DR-04': {
    technical_finding:
      'Third-party JavaScript resources and API endpoints detected with hosting infrastructure outside US borders. Analytics, chat widgets, or scheduling tools may transmit PHI to foreign servers.',
...
6. Conduct quarterly vendor audits requiring data residency attestations`,
  ...
  'AI-01': {
    technical_finding:
      'No conspicuous disclosure found indicating use of artificial intelligence in patient care pathways. Homepage and patient-facing pages lack required "clear and conspicuous" AI usage notice.',
...
6. HTML example:
  ...
  'AI-02': {
    technical_finding:
      'AI disclosure links detected with obscured visibility properties. CSS analysis shows font-size < 12px, low opacity, or negative z-index indicating "dark pattern" design to hide disclosures.',
...
6. Test with WAVE accessibility tool to verify visibility`,
  ...
  'AI-03': {
    technical_finding:
      'AI-powered diagnostic tools (symptom checkers, risk calculators) lack mandatory disclaimer stating that licensed practitioner has reviewed AI output before clinical use.',
...
6. Store attestations in EHR for compliance audits`,
  ...
  'AI-04': {
    technical_finding:
      'Interactive chatbot systems lack initial disclosure notice. Users are not informed they are interacting with AI rather than human staff at conversation initiation.',
...
6. Include disclosure in chat widget footer: "AI-Powered Chat"`,
  ...
  'ER-01': {
    technical_finding:
      'Patient intake forms lack required "Biological Sex" field with distinct Male/Female options based on reproductive biology. Form may use gender identity field instead of biological sex.',
...
8. Optionally maintain separate "Gender Identity" field for comprehensive care`,
  ...
  'ER-02': {
    technical_finding:
      'Patient portal lacks distinct authentication and access pathway for parents/legal guardians of minor patients. No clear mechanism for parental access to minor records without impediments.',
...
8. Technical implementation:
  ...
  'ER-03': {
    technical_finding:
      'Patient portal lacks specific documentation fields or communication options for metabolic health, nutrition, and dietary counseling as required for comprehensive care tracking.',
...
7. Generate automated metabolic health summaries for provider review`,
  ...
  'ER-04': {
    technical_finding:
      'Patient registration forms contain prohibited data fields collecting credit scores, voter registration status, or other explicitly forbidden personal information not relevant to healthcare.',
...
const RiskScanWidget: React.FC<RiskScanWidgetProps> = ({
  ...
  const saveToRegistry = async (scanResults: any) => {
    ...
    try {
      ...
      let response = await fetch(`${SUPABASE_URL}/rest/v1/registry?npi=eq.${scanResults.npi}`, {
        method: 'PATCH',
        headers: {
      ...
      if (patchedRows === 0) {
        ...
        response = await fetch(`${SUPABASE_URL}/rest/v1/registry?on_conflict=npi`, {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
            apikey: SUPABASE_ANON_KEY,
  ...
  const runRealScan = async (targetUrl: string, targetNpi: string) => {
    ...
    try {
      ...
      for (const f of data.findings.filter((f: any) => f.id.startsWith('DR-'))) {
        ...
        addLog(
          `${icon} ${f.id}: ${f.detail.substring(0, 100)}${f.detail.length > 100 ? '...' : ''}`,
          type,
        );
      }
      ...
      if (data.npiVerification) {
        if (data.npiVerification.valid) {
          addLog(
            `[OK] NPI Verified: ${data.npiVerification.name} (${data.npiVerification.type}) - ${data.npiVerification.specialty}`,
            'success',
          );
        } else {
      ...
      if (data.meta) {
        addLog(
          `📊 Engine: ${data.meta.engine} | Duration: ${data.meta.duration} | Checks: ${data.meta.checksRun} (${data.meta.checksPass} pass, ${data.meta.checksFail} fail, ${data.meta.checksWarn} warn)`,
          'info',
        );
      }
  ...
  const runScan = async () => {
    ...
    try {
      ...
      setUrlError('Please enter a valid website URL (e.g., https://yourpractice.com)');
      addLog(
        '[ERROR] Invalid URL format — enter a website address like https://yourpractice.com',
        'error',
      );
      return;
    }
    ...
    if (email && email.includes('@')) {
      ...
      if (freeProviders.includes(emailDomain)) {
        try {
          ...
          setEmailHint(
            `Please use your practice email (e.g., you@${urlDomain}) to verify ownership`,
          );
        } catch {
          setEmailHint('Please use your practice email, not a free email provider');
        }
        addLog(
          '[ERROR] Practice email required — free email providers are not accepted for compliance verification',
          'error',
        );
        return;
      } else {
    ...
    try {
      ...
      try {
        ...
        if (!valRes.ok || !valData.valid) {
          ...
          if (errors.some((e: string) => e.toLowerCase().includes('npi'))) {
            setNpiError(
              errors.find((e: string) => e.toLowerCase().includes('npi')) || 'Invalid NPI',
            );
          }
          if (
            errors.some(
              (e: string) => e.toLowerCase().includes('email') || e.toLowerCase().includes('mail'),
            )
          ) {
            setEmailHint(
              errors.find(
                (e: string) =>
                  e.toLowerCase().includes('email') || e.toLowerCase().includes('mail'),
              ) || 'Invalid email',
            );
          }
          setScanning(false);
      ...
      try {
        ...
        addLog(
          '[ERROR] Real scan failed. Cannot produce compliance results without server-side analysis.',
          'error',
        );
        setScanning(false);
        return;
      ...
      const scanResults = {
        ...
        riskMeterLevel,
        complianceStatus:
          scanData.complianceStatus ||
          (score >= 67 ? 'Sovereign' : score >= 34 ? 'Drift' : 'Violation'),
      ...
      try {
        await fetch(`${SUPABASE_URL}/functions/v1/calculate-risk`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
      ...
      if (reportId) {
        ...
        try {
          await fetch(`${SUPABASE_URL}/rest/v1/registry?npi=eq.${npi}`, {
            method: 'PATCH',
            headers: {
              'Content-Type': 'application/json',
              apikey: SUPABASE_ANON_KEY,
      ...
      try {
        ...
        const topFindingsSummary = topIssues
          .slice(0, 5)
          .map(
            (f: { name?: string; id?: string; status?: string; detail?: string }) =>
        ...
        if (providerEmail) {
          ...
          fetch('/api/email/send', {
            ...
            body: JSON.stringify({
              ...emailPayload,
              variables: { email: providerEmail, practice_name: providerName || 'Provider' },
            }),
          })
        ...
        fetch('/api/email/send', {
          ...
          body: JSON.stringify({
            ...
            variables: {
              ...
              practice_name: providerName || `NPI: ${npi}`,
    ...
    <div className="w-full max-w-4xl mx-auto p-6 bg-gradient-to-br from-slate-50 to-slate-100 rounded-xl shadow-lg">
      ...
      {results && (
        <div className="bg-white p-6 rounded-lg shadow-sm">
            ...
            <div className="mb-4 px-4 py-2 bg-slate-100 border border-slate-200 rounded-lg flex items-center gap-2 text-sm text-slate-600">
              ...
              <span>
                ...
                <strong className="text-slate-800 capitalize">
                  {results.pageContext.type?.replace(/_/g, ' ') || 'General'}
                </strong>
              </span>
              {results.pageContext.pageTitle && results.pageContext.pageTitle !== 'Unknown' && (
                <span className="text-slate-400 ml-1">
                  — &quot;{results.pageContext.pageTitle.substring(0, 60)}&quot;
                </span>
              )}
              {results.pageContext.hasPatientPortal && (
                <span className="ml-2 px-2 py-0.5 bg-blue-100 text-blue-700 rounded text-xs font-medium">
                  Portal Detected
                </span>
              )}
              {results.pageContext.hasChatbot && (
            ...
            <div className="flex items-center justify-between mb-4">
              <div>
                ...
                {results.engineVersion && (
                  <p className="text-xs text-slate-400 mt-0.5">Engine: {results.engineVersion}</p>
                )}
              </div>
              <div className="text-right">
                <div
                  className={`text-4xl font-bold ${
                    results.riskScore >= 67
                      ? 'text-green-600'
                      : results.riskScore >= 34
                        ? 'text-amber-600'
                        : 'text-red-600'
                  }`}
                >
                ...
                <div
                  className={`text-sm font-semibold px-3 py-0.5 rounded-full inline-block mt-1 ${
                    results.riskMeterLevel === 'Sovereign'
                      ? 'bg-green-100 text-green-700'
                      : results.riskMeterLevel === 'Drift'
              ...
              <div className="mt-4 pt-4 border-t border-blue-200">
                ...
                <div className="space-y-3">
                  {[
                    ...
                    const barColor =
                      pct >= 67 ? 'bg-green-500' : pct >= 34 ? 'bg-amber-500' : 'bg-red-500';
                    const textColor =
                      pct >= 67 ? 'text-green-700' : pct >= 34 ? 'text-amber-700' : 'text-red-700';
                    const bgColor =
                      pct >= 67 ? 'bg-green-50' : pct >= 34 ? 'bg-amber-50' : 'bg-red-50';
                    return (
                      <div key={cat.key} className={`p-3 rounded-lg ${bgColor}`}>
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            <span
                              className={`text-xs px-2 py-0.5 rounded-full font-medium ${
                                catScore.level === 'Sovereign'
                                  ? 'bg-green-200 text-green-800'
                                  : catScore.level === 'Drift'
                        ...
                        <div className="text-xs text-slate-500">
                          ...
                          {catScore.failed > 0 && (
                            <span className="text-red-600 ml-1">• {catScore.failed} failed</span>
                          )}
                          {catScore.warnings > 0 && (
                            <span className="text-amber-600 ml-1">
                              • {catScore.warnings} warning{catScore.warnings > 1 ? 's' : ''}
                            </span>
                          )}
                        </div>
                      </div>
          ...
          {results.dataBorderMap &&
            results.dataBorderMap.length > 0 &&
            (() => {
                      ...
                      return (
                        <div key={idx} className="px-5 py-2.5 flex items-center justify-between">
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            <span
                              className={`font-semibold text-xs ${isUS ? 'text-green-600' : 'text-red-600'}`}
                            >
                              {isUS ? '✓' : '✗'}
                            </span>
                          </div>
                  ...
                  {remaining > 0 && (
                    <div className="bg-slate-50 px-5 py-2.5 text-center border-t border-slate-200">
                      <span className="text-xs text-slate-500">
                        + {remaining} more endpoint{remaining > 1 ? 's' : ''} analyzed
                      </span>
                      <span className="text-xs text-slate-400 ml-2">•</span>
                      <span className="text-xs text-blue-600 font-medium ml-2">
                        Full map available in detailed report
                      </span>
                    </div>
                  )}
                </div>
          ...
          <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              {[
                ...
                },
                {
                  prefix: 'ER-',
                  label: 'Clinical Integrity',
                  catKey: 'clinical_integrity',
                  icon: <Lock className="w-6 h-6" />,
                  statute: 'SB 1188',
                },
              ].map((category) => {
                const categoryFindings =
                  results.findings?.filter((f: any) => f.id.startsWith(category.prefix)) || [];
                ...
                const scoreTextColor =
                  pct >= 67 ? 'text-green-700' : pct >= 34 ? 'text-amber-700' : 'text-red-700';
                const scoreBgColor =
                  pct >= 67 ? 'bg-green-100' : pct >= 34 ? 'bg-amber-100' : 'bg-red-100';
                const barColor =
                  pct >= 67 ? 'bg-green-500' : pct >= 34 ? 'bg-amber-500' : 'bg-red-500';
                ...
                return (
                  <div
                    key={category.catKey}
                    className={`bg-gradient-to-br ${statusBg} border rounded-xl p-5 relative overflow-hidden`}
                  >
                    {/* Score badge - top right */}
                    <div
                      className={`absolute top-3 right-3 ${scoreBgColor} ${scoreTextColor} text-2xl font-black px-3 py-1 rounded-lg`}
                    >
                      {pct}%
                    </div>

                    {/* Icon + label */}
                    <div className={`${scoreTextColor} mb-3`}>{category.icon}</div>
            ...
            {(() => {
              ...
              const failCount =
                results.findings?.filter((f: any) => f.status === 'fail').length || 0;
              ...
              const LINKS = {
                report: `https://buy.stripe.com/4gM5kDdpw0HP18x305${clientRef}`,
                safeHarbor: `https://buy.stripe.com/00w28retAeyF6sRgQV${clientRef}`,
                shield: `https://buy.stripe.com/aFa3cv3OW4Y54kJ305${clientRef}`,
              };
              ...
              if (score >= 80) {
                return (
                  <div className="mt-6 space-y-2.5">
                    <div className="bg-gradient-to-r from-green-50 to-emerald-50 border border-green-200 rounded-xl p-5">
                      <div className="flex items-center gap-2 mb-2">
                        ...
                        <h4 className="font-bold text-green-800 text-sm">
                          Your score is strong — keep it that way
                        </h4>
                      </div>
                      <p className="text-xs text-green-700 mb-4">
                        Plugin updates, hosting changes, and new scripts can silently break your
                        compliance overnight. Monitoring catches drift before it becomes a
                        violation.
                      </p>
                    </div>

                    {/* Option 1: Audit Report */}
                    <a
                      href={LINKS.report}
                      target="_blank"
                      ...
                      <div className="flex items-center justify-between">
                        <div className="flex-1">
                          ...
                          <p className="text-xs text-slate-500">
                            Forensic analysis with findings, border map, and remediation roadmap
                          </p>
                          <p className="text-xs font-bold text-emerald-600 mt-1">
                            + 3 months Sentry Shield included
                          </p>
                        </div>
                    ...
                    {/* Option 2: Safe Harbor — Most Popular */}
                    <a
                      href={LINKS.safeHarbor}
                      target="_blank"
                      rel="noopener noreferrer"
                    ...
                    >
                      ...
                      <div className="flex items-center justify-between mt-0.5">
                        <div className="flex-1">
                          ...
                          <p className="text-xs text-slate-500">
                            Everything in the Audit Report <strong>plus</strong> ready-made
                            policies, AI disclosures, staff training, evidence templates, and
                    ...
                    {/* Option 3: Sentry Shield — Recommended */}
                    <a
                      href={LINKS.shield}
                      target="_blank"
                      rel="noopener noreferrer"
                      className="block border-2 border-emerald-500 bg-gradient-to-r from-emerald-50 to-green-50 rounded-xl p-4 transition-all hover:shadow-lg relative cursor-pointer"
                    >
                      <div className="absolute -top-2.5 left-4 bg-emerald-600 text-white text-[9px] font-bold uppercase tracking-wider px-2.5 py-0.5 rounded-full">
                        Recommended
                      </div>
                      <div className="flex items-center justify-between mt-0.5">
                        <div className="flex-1">
                          <div className="flex items-center gap-2 mb-1">
                            ...
                            <span className="font-bold text-slate-800 text-sm">Sentry Shield</span>
                          </div>
                          <p className="text-xs text-slate-500">
                            Continuous compliance monitoring with drift alerts, live dashboard, and
                            automated re-scans
                          </p>
                          <p className="text-xs font-bold text-emerald-600 mt-1">
                            Live dashboard, drift alerts, and automated re-scans
                          </p>
                        </div>
                        <div className="text-right ml-4 flex-shrink-0">
                          <div className="text-xl font-black text-emerald-700">$79</div>
                  ...
                  <div className="mt-6">
                    <div className="mb-3">
                      <h4 className="font-bold text-slate-800 text-sm">
                        You have {failCount} fixable issue{failCount !== 1 ? 's' : ''} — choose your
                        path:
                      </h4>
                    </div>
                      ...
                      >
                        ...
                        <div className="flex items-center justify-between mt-0.5">
                          <div className="flex-1">
                            ...
                            <p className="text-xs text-slate-500">
                              Continuous compliance monitoring with drift alerts, live dashboard,
                              and automated re-scans
                            </p>
                          ...
                          <div className="text-right ml-4 flex-shrink-0">
                            <div className="text-xl font-black text-blue-700">$79</div>
                            <div className="text-[10px] text-slate-400">/month</div>
                          </div>
                        </div>
                      </a>
                  ...
                  <div className="bg-red-50 border border-red-200 rounded-lg p-3 flex items-center gap-3 mb-3">
                    ...
                    <div>
                      ...
                      <span className="text-xs text-red-600 ml-1">
                        — {failCount} violation{failCount !== 1 ? 's' : ''} detected. Potential
                        penalty up to ${(failCount * 50000).toLocaleString()}.
                      </span>
                    </div>
                  </div>
