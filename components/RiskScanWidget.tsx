...
5. HTML example:
   <div class="practitioner-review">
     ✓ Reviewed by Dr. Jane Smith, MD (License #12345) on [Date]
   </div>
...
const RiskScanWidget = () => {
  ...
  const saveToRegistry = async (scanResults) => {
    ...
    try {
      ...
      if (response.ok || response.status === 201 || response.status === 204) {
        addLog('✓ Registry entry saved', 'success');
        ...
        if (verification && verification.length > 0) {
          addLog('✓ Registry entry verified', 'success');
          return true;
        } else {
          addLog('⚠️ Registry entry not found after save', 'warning');
          return false;
        }
      } else {
        const error = await response.text();
        addLog(`⚠️ Registry save failed`, 'warning');
        console.error('Registry save error:', error);
        return false;
      }
    } catch (error) {
      addLog(`⚠️ Registry save failed: ${error.message}`, 'warning');
  ...
  const saveViolationsToSupabase = async (registryId, findings) => {
    ...
    if (failedFindings.length === 0) {
      addLog('✓ No violations to save', 'success');
      return;
    }
    ...
    try {
      ...
      if (response.ok) {
        addLog(`✓ Saved ${violations.length} violations to database`, 'success');
        return true;
      } else {
        const error = await response.text();
        addLog(`⚠️ Database save failed: ${error}`, 'warning');
        console.error('Supabase save error:', error);
        return false;
      }
    } catch (error) {
      addLog(`⚠️ Database connection failed: ${error.message}`, 'warning');
  ...
  const stealthProbe = async (targetUrl) => {
    ...
    if (wafDetected) {
      addLog('⚠️ WAF detected - switching to evasion mode', 'warning');
      await delay(500);
      addLog('Using randomized user-agents and request timing', 'info');
    } else {
      addLog('✓ No active WAF detected', 'success');
    }
  ...
  const scanDataSovereignty = async (targetUrl) => {
    ...
    addLog(`${dr01Pass ? '✓' : '✗'} DR-01: ${ipLocation}`, dr01Pass ? 'success' : 'error');
    ...
    addLog(`${cdnCompliant ? '✓' : '✗'} DR-02: CDN compliance`, cdnCompliant ? 'success' : 'error');
    ...
    addLog(`${mxCompliant ? '✓' : '✗'} DR-03: MX pathing`, mxCompliant ? 'success' : 'error');
    ...
    addLog(`${subProcessorCompliant ? '✓' : '✗'} DR-04: Sub-processor audit`, subProcessorCompliant ? 'success' : 'error');
  ...
  const scanAITransparency = async (targetUrl) => {
    ...
    addLog(`${hasDisclosure ? '✓' : '✗'} AI-01: Disclosure text`, hasDisclosure ? 'success' : 'error');
    ...
    addLog(`${noObscuredLinks ? '✓' : '✗'} AI-02: No dark patterns`, noObscuredLinks ? 'success' : 'error');
    ...
    addLog(`${hasDiagnosticDisclaimer ? '✓' : '✗'} AI-03: Diagnostic disclaimer`, hasDiagnosticDisclaimer ? 'success' : 'error');
    ...
    addLog(`${chatbotCompliant ? '✓' : '✗'} AI-04: Chatbot notice`, chatbotCompliant ? 'success' : 'error');
  ...
  const scanEHRIntegrity = async (targetUrl) => {
    ...
    addLog(`${hasBioSexField ? '✓' : '✗'} ER-01: Biological sex field`, hasBioSexField ? 'success' : 'error');
    ...
    addLog(`${hasParentalAccess ? '✓' : '✗'} ER-02: Parental access`, hasParentalAccess ? 'success' : 'error');
    ...
    addLog(`${hasMetabolicFields ? '✓' : '✗'} ER-03: Metabolic health`, hasMetabolicFields ? 'success' : 'error');
    ...
    addLog(`${noForbiddenFields ? '✓' : '✗'} ER-04: Forbidden fields`, noForbiddenFields ? 'success' : 'error');
  ...
  const runScan = async () => {
    if (!npi || !url) {
      addLog('❌ Please provide both NPI and URL', 'error');
      return;
    }
    ...
    try {
      ...
      try {
        ...
        if (riskResponse.ok) {
          ...
          addLog('✓ Risk calculation complete', 'success');
          ...
          addLog('⚠️ Risk calculation unavailable, using local calculation', 'warning');
        }
      } catch (apiError) {
        addLog('⚠️ Edge function unavailable, using local calculation', 'warning');
      ...
      try {
        ...
        if (relayResponse.ok) {
          ...
          addLog('⚠️ Edge function unavailable, saving directly', 'warning');
        ...
        if (relayResponse.ok) {
          addLog('✓ Results saved to registry', 'success');
        } else {
          addLog('⚠️ Could not save to registry', 'warning');
        }
      } catch (apiError) {
        addLog('⚠️ Registry update unavailable', 'warning');
        console.error('NPI-relay API error:', apiError);
      }
