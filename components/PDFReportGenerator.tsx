...
const PDFReportGenerator: React.FC<PDFReportGeneratorProps> = ({
  ...
  const generatePDF = async () => {
    ...
    try {
      ...
      yPos += 7;
      doc.text(
        `Scan Date: ${registryData.lastScanDate || new Date(registryData.lastScanTimestamp).toLocaleDateString()}`,
        20,
        yPos,
      );
      ...
      doc.setFontSize(12);
      doc.text(
        `Risk Level: ${registryData.riskLevel || registryData.risk_meter_level || 'Unknown'}`,
        120,
        yPos,
      );
      ...
      doc.setFont('helvetica', 'normal');
      doc.text(
        `Status: ${registryData.complianceStatus || registryData.overall_compliance_status || 'Unknown'}`,
        30,
        yPos,
      );
      ...
      if (violations.length > 0) {
        ...
        const categories = {
          'Data Sovereignty & Residency': violations.filter((v) =>
            v.violation_id.startsWith('DR-'),
          ),
          'AI Transparency & Disclosure': violations.filter((v) =>
            v.violation_id.startsWith('AI-'),
          ),
        ...
        Object.entries(categories).forEach(([category, categoryViolations]) => {
          ...
          categoryViolations.forEach((violation, idx) => {
            ...
            // Priority badge
            const priorityColor =
              violation.fix_priority === 'Critical'
                ? ([220, 38, 38] as [number, number, number])
                : violation.fix_priority === 'High'
                  ? ([234, 88, 12] as [number, number, number])
      ...
      if (violations.length > 0) {
        ...
        sortedViolations.forEach((violation, idx) => {
          ...
          doc.setTextColor(71, 85, 105);
          doc.text(
            `Priority: ${violation.fix_priority} | Complexity: ${violation.fix_complexity}`,
            30,
            yPos,
          );
          doc.setTextColor(0, 0, 0);
      ...
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          ...
          <p className="text-sm text-slate-600">
            Generate detailed PDF reports with technical remediation guidance
          </p>
        </div>
      </div>
      ...
      {!registryData && !loading && (
        <div className="text-center py-8">
          <p className="text-slate-600 mb-4">
            No data loaded. Please provide an NPI or Registry ID.
          </p>
          <button
              ...
              <div>
                ...
                <div
                  className={`font-semibold ${
                    (registryData.riskScore || registryData.risk_score || 0) >= 75
                      ? 'text-green-600'
                      : (registryData.riskScore || registryData.risk_score || 0) >= 50
                        ? 'text-amber-600'
                        : 'text-red-600'
                  }`}
                >
