...
interface TopicInputProps {
  ...
  onBatchGenerate: (
    items: { topic: string; audience: string; intent: string; angle: string }[],
  ) => void;
...
export default function TopicInput({
  ...
  const [intent, setIntent] = useState<'thought_leadership' | 'educational' | 'product_awareness'>(
    'thought_leadership',
  );
    ...
    <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
      ...
      <div className="mb-4">
        ...
        <p className="text-[10px] text-slate-400 mt-1">
          Enter a category to get targeted AI topic suggestions, or leave blank for general
          suggestions.
        </p>
      </div>
        ...
        >
          ...
          <div className="flex-1">
            <span className="font-medium">
              {dataDriven ? 'Data-Driven Mode' : 'Data-Driven Mode'}
            </span>
          ...
          <div
            className={`w-8 h-4.5 rounded-full transition-colors relative ${dataDriven ? 'bg-indigo-500' : 'bg-slate-300'}`}
          >
            <div
              className={`absolute top-0.5 w-3.5 h-3.5 rounded-full bg-white shadow transition-all ${dataDriven ? 'left-4' : 'left-0.5'}`}
            />
          </div>
        </button>
      </div>
