...
interface PostPreviewProps {
  ...
  onSchedule: (
    ...
    scheduledAt: string,
    channels: string[],
    channelSchedules?: Record<string, string>,
  ) => void;
        ...
        {isEditing ? (
          ...
          <div>
            <div className="prose prose-sm max-w-none text-slate-700 whitespace-pre-wrap text-sm leading-relaxed max-h-[300px] overflow-y-auto">
              {currentBody || (
                <span className="text-slate-300 italic">No content for this channel</span>
              )}
            </div>
            <div className="flex items-center gap-2 mt-3 pt-3 border-t border-slate-100">
              ...
              <span className="text-[10px] text-slate-300 ml-auto">{currentBody.length} chars</span>
            </div>
          </div>
        )}
      </div>
      ...
      {showScheduler && (
        <div className="px-5 py-4 border-t border-slate-100 bg-blue-50/50">
                ...
                {channels.map((ch) => (
                  <div key={ch} className="flex items-center gap-2">
                    ...
                    <select
                      ...
                      onChange={(e) =>
                        setChannelOffsets((prev) => ({ ...prev, [ch]: Number(e.target.value) }))
                      }
          ...
          <p className="text-[10px] text-slate-400 mt-2">
            Posts are checked every 15 minutes. Your post will publish at the next check after the
            scheduled time.
          </p>
        </div>
      )}
      ...
      <div className="px-5 py-4 border-t border-slate-100 bg-slate-50 rounded-b-xl">
        {post.status === 'published' && (
          <p className="text-[10px] text-emerald-600 font-medium mb-2">
            Published. Select channels to publish to additional platforms.
          </p>
        )}
