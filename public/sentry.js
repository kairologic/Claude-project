...
(function () {
  ...
  if (!scriptTag) {
    console.error('[KairoLogic] Script tag not found');
    return;
  }
  ...
  function init() {
    fetchWidgetData(cfg.npi, function (data) {
      ...
      var status = data
        ? data.widget_status || data.status || (data.verified ? 'verified' : 'unregistered')
        : 'unregistered';
      var lastScanRaw = data && (data.last_scan_timestamp || data.last_scan || data.updated_at);
      var lastScan = lastScanRaw
        ? new Date(lastScanRaw).toLocaleDateString('en-US', {
            month: 'short',
            day: 'numeric',
            year: 'numeric',
          })
  ...
  function buildBadge(status) {
    ...
    return (
      '' +
      '<div class="kl-badge-v3 ' +
      badgeClass +
      '" id="kl-badge" role="button" tabindex="0" aria-expanded="false">' +
      '<div class="kl-badge-icon">' +
      ico.shield +
      (verified ? '<div class="kl-badge-check">' + ico.check + '</div>' : '') +
      '</div>' +
      '<div class="kl-badge-text">' +
      '<div class="kl-badge-title">' +
      (verified ? (isShield ? 'Data & AI Trust' : 'Data & AI Trust') : 'Compliance Unknown') +
      '</div>' +
      '<div class="kl-badge-sub">' +
      (verified
        ? isShield
          ? 'Live Monitoring Active'
          : 'US Data \u00b7 AI Disclosed'
        : 'Not yet verified') +
      '</div>' +
      '</div>' +
      (isShield && verified ? '<div class="kl-shield-pulse"></div>' : '') +
      '<div class="kl-badge-chevron">' +
      ico.chevron +
      '</div>' +
      '</div>'
    );
  ...
  function buildTrustPane(data, lastScan) {
    ...
    return (
      ...
      '<div class="kl-pane-header-icon">' +
      ico.shield +
      '</div>' +
      '<div class="kl-pane-header-text">' +
      '<div class="kl-pane-header-title">KairoLogic\u2122 Sentry</div>' +
      '<div class="kl-pane-header-sub">' +
      (isShield ? 'Shield \u00b7 Live Monitoring' : 'Watch \u00b7 Compliance Verified') +
      '</div>' +
      '</div>' +
      '<button class="kl-pane-close" id="kl-pane-close">' +
      ico.close +
      '</button>' +
      '</div>' +
      // Status banner
      ...
      '<span class="kl-scan-date-label">' +
      (isShield ? 'Monitoring Since' : 'Last Verified') +
      '</span>' +
      '<span class="kl-scan-date-value">' +
      lastScan +
      (isShield ? ' \u00b7 Live' : '') +
      '</span>' +
      '</div>' +
      ...
      (isShield
        ...
        : '<a class="kl-drift-alert-cta" href="https://buy.stripe.com/test_5kQfZh1IveW058j7ZO4ko00?client_reference_id=' +
          cfg.npi +
          '" style="background:#0A192F;" target="_blank" rel="noopener">Upgrade to Shield for Real-Time Alerts \u2192</a>') +
      '</div>' +
      // Actions
      '<div class="kl-pane-actions" style="padding-top:14px;">' +
      '<a class="kl-pane-btn kl-pane-btn-primary" href="' +
      API_BASE +
      '/scan/results?npi=' +
      cfg.npi +
      '&mode=verified" target="_blank" rel="noopener">View Full Compliance Report</a>' +
      (isShield
        ? '<a class="kl-pane-btn kl-pane-btn-secondary" id="kl-shield-dash-link" href="' +
          API_BASE +
          '/dashboard/' +
          cfg.npi +
          '" target="_blank" rel="noopener">\ud83d\udee1\ufe0f Open Compliance Dashboard</a>'
        : '') +
      '</div>' +
      // Footer
      ...
      '<a href="' +
      API_BASE +
      '" target="_blank" rel="noopener">Powered by KairoLogic Sentry ' +
      (isShield ? 'Shield\u2122' : 'Watch\u2122') +
      ' \u00b7 kairologic.net</a>' +
      '</div>' +
      '</div>' +
      '</div>'
  ...
  function buildUnregisteredPane() {
    return (
      ...
      '/scan?npi=' +
      cfg.npi +
      '" target="_blank" rel="noopener">Run Free Scan</a>' +
      '</div>' +
      '<div class="kl-pane-footer-v3">' +
      '<a href="' +
      API_BASE +
      '" target="_blank" rel="noopener">Powered by KairoLogic \u00b7 kairologic.net</a>' +
      '</div>' +
      '</div>' +
      '</div>'
    );
      ...
      .then(function (resp) {
        ...
        if (rpt.category_scores) {
          ...
          setBar(
            ...
            'kl-val-dr',
            cs.data_sovereignty
              ? cs.data_sovereignty.percentage
              : cs.dataResidency
                ? cs.dataResidency.percentage
                : 0,
          );
          setBar(
            'kl-bar-ai',
            'kl-val-ai',
            cs.ai_transparency
              ? cs.ai_transparency.percentage
              : cs.aiTransparency
                ? cs.aiTransparency.percentage
                : 0,
          );
        }
  ...
  function loadDriftCount(npi) {
    fetch(API_BASE + '/api/widget/drift?npi=' + npi + '&status=new&limit=100', {
        ...
        var detailText =
          detailParts.length > 0
            ...
            : isShield
              ...
              : 'Upgrade to Shield to see details and get alerts';
    ...
    privacy_policy: function () {
      ...
      for (var i = 0; i < links.length; i++) {
        ...
        if (
          ...
          text.indexOf('privacy policy') !== -1 ||
          text.indexOf('privacy notice') !== -1
        ) {
    ...
    for (var i = 0; i < categories.length; i++) {
      ...
      if (!base) {
        if (!curr.empty) {
          drifts.push({
            category: cat,
            drift_type: 'content_added',
            previous_hash: null,
            current_hash: curr.hash,
            content_after: curr.content,
          });
        }
    ...
    if (baseline) {
      ...
      for (var b = 0; b < baseCategories.length; b++) {
        if (!current[baseCategories[b]]) {
          drifts.push({
            category: baseCategories[b],
            drift_type: 'content_removed',
            previous_hash: baseline[baseCategories[b]].hash,
            current_hash: null,
            content_before: baseline[baseCategories[b]].content,
            content_after: '',
          });
        }
      }
