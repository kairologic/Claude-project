...
const TECHNICAL_FIXES: Record<string, any> = {
  ...
  'DR-03': {
    ...
    regulation: 'SB1188',
    clause: 'Sec. 183.002(b)',
    fix_priority: 'High',
  },
  'DR-04': {
    name: 'Third-Party Resource Audit',
    regulation: 'SB1188',
    clause: 'Sec. 183.002(c)',
    fix_priority: 'High',
  },
  'AI-01': {
    name: 'AI Disclosure Presence',
    regulation: 'HB149',
    clause: 'Sec. 101.001(a)',
    fix_priority: 'High',
  },
  'AI-02': {
    name: 'AI Disclosure Visibility',
    regulation: 'HB149',
    clause: 'Sec. 101.001(b)',
    fix_priority: 'Critical',
  },
  'AI-03': {
    name: 'AI Diagnostic Tool Disclaimer',
    regulation: 'HB149',
    clause: 'Sec. 101.002',
    fix_priority: 'High',
  },
...
const StatusBadge: React.FC<{ status: string; size?: 'sm' | 'md' }> = ({ status, size = 'md' }) => {
  ...
  const icons: Record<string, React.ReactNode> = {
    ...
    trial: <Clock size={11} />,
    inactive: <Pause size={11} />,
  };
  return (
    <span
      className={`inline-flex items-center gap-1 ${size === 'sm' ? 'px-1.5 py-0.5 text-[8px]' : 'px-2 py-1 text-[9px]'} rounded-full border font-bold uppercase tracking-wide ${styles[status] || styles.hidden}`}
    >
      {icons[status]} {status}
    </span>
  );
...
const Modal: React.FC<{
  ...
  return (
    ...
    >
      <div
        className={`${widths[size]} w-full bg-white rounded-xl shadow-2xl max-h-[85vh] flex flex-col`}
...
const ProviderForm: React.FC<{
  ...
  const [form, setForm] = useState<ProviderFormData>({
    ...
    zip: entry?.zip || '',
    url: entry?.url || '',
    widget_status: (entry?.widget_status as any) || 'hidden',
    subscription_status: (entry?.subscription_status as any) || 'trial',
    is_visible: entry?.is_visible ?? false,
    risk_score: entry?.risk_score || 0,
    provider_type: (entry as any)?.provider_type || 2,
  });
  ...
  return (
    ...
    >
      <div className="grid grid-cols-2 gap-3">
        <div>
          ...
          <input
            ...
            required
            className="w-full px-3 py-2 text-sm bg-slate-50 border border-slate-200 rounded-lg focus:ring-2 focus:ring-[#C5A059] focus:outline-none"
          />
        </div>
        <div>
          <label className="block text-[10px] font-bold text-slate-500 uppercase mb-1">NPI *</label>
          <input
            type="text"
            value={form.npi}
            ...
            required
            maxLength={10}
            className="w-full px-3 py-2 text-sm font-mono bg-slate-50 border border-slate-200 rounded-lg focus:ring-2 focus:ring-[#C5A059] focus:outline-none"
          />
        </div>
        <div>
          <label className="block text-[10px] font-bold text-slate-500 uppercase mb-1">
          ...
          <select
            ...
            onChange={(e) => setForm({ ...form, provider_type: parseInt(e.target.value) })}
        ...
        <div className="col-span-2">
          <label className="block text-[10px] font-bold text-slate-500 uppercase mb-1">
            Website URL (required for scanning)
          </label>
          <input
            type="url"
            value={form.url}
        ...
        <div>
          ...
          <select
            value={form.widget_status}
            onChange={(e) => setForm({ ...form, widget_status: e.target.value as any })}
            className="w-full px-3 py-2 text-sm bg-slate-50 border border-slate-200 rounded-lg"
          >
      ...
      <div className="flex justify-end gap-2 pt-3 border-t">
        ...
        <button
          type="submit"
          className="px-5 py-2 bg-[#00234E] text-white text-sm font-bold rounded-lg hover:bg-[#C5A059] flex items-center gap-1.5"
        >
          <Save size={14} /> Save
        </button>
      </div>
...
export default function AdminDashboard() {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      setProviders(data || []);
      console.log(
        `Loaded ${data?.length || 0} providers with URLs (${withUrlCount} total scannable, ${totalCount} total in registry, ${activeCount} active)`,
      );
      ...
      if (emailTemplatesData && emailTemplatesData.length > 0) {
        ...
        setTemplates([
          {
            id: 'ET-001',
            name: 'Cure Notice Warning',
            category: 'marketing',
            subject: 'URGENT: TX SB 1188 Alert',
            body: 'Your practice may be at risk.',
            event_trigger: 'risk_scan_high',
            is_active: true,
          },
          {
            id: 'ET-002',
            ...
            subject: 'Sentry Verified',
            body: 'Congratulations! Your practice is verified.',
            event_trigger: 'verification_complete',
            is_active: true,
          },
        ]);
      }
      ...
      const { data: marketingData } = await supabase.from('templates').select('*').order('id');
      if (marketingData) {
        setMarketingTemplates(
          marketingData.map((t: any) => ({
            id: t.id,
            name: t.name,
    ...
    for (let d = 0; d < 7; d++) {
      ...
      ['09:00', '10:00', '11:00', '14:00', '15:00', '16:00'].forEach((time) => {
        slots.push({
          ...
          booked_by: Math.random() > 0.8 ? { name: 'Dr. Sample' } : null,
        });
      });
    }
  ...
  const stats = useMemo(
    () => ({
      ...
      type2: providers.filter((r) => (r as any).provider_type === 2 || !(r as any).provider_type)
        .length,
    }),
  ...
  const filteredProviders = useMemo(() => {
    ...
    const sorted = [...results].sort((a, b) => {
      ...
      if (sortField === 'name') cmp = (a.name || '').localeCompare(b.name || '');
        ...
        const bTs = b.last_scan_timestamp ? new Date(b.last_scan_timestamp).getTime() : 0;
        cmp = aTs - bTs;
      } else if (sortField === 'widget_status')
        cmp = (a.widget_status || '').localeCompare(b.widget_status || '');
      return sortDir === 'asc' ? cmp : -cmp;
    });
    return sorted;
  ...
  const toggleSort = useCallback(
    (field: SortField) => {
      if (sortField === field) setSortDir((d) => (d === 'asc' ? 'desc' : 'asc'));
        ...
        setSortField(field);
        setSortDir(field === 'name' || field === 'npi' ? 'asc' : 'desc');
      }
    },
    [sortField],
  );
  ...
  const SortIcon: React.FC<{ field: SortField }> = ({ field }) => {
    if (sortField !== field)
      return <span className="ml-0.5 opacity-30 text-[8px]">{'\u2195'}</span>;
    return (
      <span className="ml-0.5 text-[#C5A059] text-[8px]">
        {sortDir === 'asc' ? '\u2191' : '\u2193'}
      </span>
    );
  };
  ...
  const calendarDates = useMemo(
    () => [...new Set(calendarSlots.map((s) => s.date))].sort(),
    [calendarSlots],
  );
  const dateSlots = useMemo(
    () => (selectedDate ? calendarSlots.filter((s) => s.date === selectedDate) : []),
    [calendarSlots, selectedDate],
  );
  ...
  const handleSaveProvider = async (data: ProviderFormData) => {
    try {
      ...
      notify(isNew ? 'Provider added!' : 'Provider updated!');
    } catch (e: any) {
        ...
        if (scanData) {
          try {
            ...
            const updatePayload: any = {
              ...
              risk_level: scanData.risk_level,
              status_label:
                scanData.status_label ||
                (scanData.risk_score >= 67
                  ? 'Verified Sovereign'
                  : scanData.risk_score >= 34
                    ? 'Drift Detected'
                    : 'Violation'),
  ...
  const handleScan = async (ids: string[]) => {
    ...
    setScanProgress(0);
    setScanStatus(
      `Scanning ${withUrl.length} provider(s)... (~${Math.ceil((withUrl.length * 4) / 60)} min)`,
    );
    ...
    try {
      ...
      for (let i = 0; i < withUrl.length; i++) {
        ...
        const updatePayload: any = {
          ...
          scan_count: (p.scan_count || 0) + 1,
          widget_status:
            scanData.risk_score >= 67 ? 'active' : scanData.risk_score >= 34 ? 'warning' : 'hidden',
          last_scan_result: scanData,
          last_scan_timestamp: nowIso,
          updated_at: nowIso,
        };
      ...
      const msg =
        failed > 0
          ...
          : `Scan complete! ${scanned} provider(s) scanned.`;
    ...
    try {
      ...
      while (hasMore) {
        ...
        if (data && data.length > 0) {
          ...
          offset += batchSize;
          setScanProgress(
            Math.min(90, Math.round((allMissing.length / totalCounts.withoutUrl) * 100)),
          );
        } else {
          hasMore = false;
        }
      ...
      const csv = [
        ...
        ...allMissing.map(
          (r) =>
            `${r.npi},"${(r.name || '').replace(/"/g, '""')}",${r.provider_type || 2},${r.city || ''},${r.email || ''},${r.phone || ''},`,
        ),
      ].join('\n');
      ...
      for (let i = 0; i < importData.length; i++) {
        ...
        if (existing) {
          ...
          await supabase.from('registry').insert({
            ...
            zip: row.zip || null,
            url: row.url || null,
            provider_type: row.provider_type || 2,
  ...
  const handleWidgetStatusChange = async (p: Registry, status: string) => {
    try {
      await getSupabase().from('registry').update({ widget_status: status }).eq('id', p.id);
      await loadData();
      notify(`Widget: ${status}`);
    } catch (e: any) {
  ...
  return (
    <div className="min-h-screen bg-slate-100">
      ...
      {notification && (
        <div
          className={`fixed bottom-4 right-4 z-50 px-4 py-2 rounded-lg shadow-lg text-sm font-bold flex items-center gap-2 ${notification.type === 'success' ? 'bg-emerald-500 text-white' : notification.type === 'error' ? 'bg-red-500 text-white' : 'bg-blue-500 text-white'}`}
        >
      ...
      {scanning && (
        <div className="fixed inset-0 z-50 bg-black/50 backdrop-blur-sm flex items-center justify-center">
          <div className="bg-white rounded-2xl p-8 shadow-2xl text-center max-w-md">
            ...
            <h3 className="text-lg font-bold text-[#00234E] mb-2">
      ...
      <header className="bg-[#00234E] text-white py-3 px-4 sticky top-0 z-40 shadow-lg">
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-base font-bold">Sentry Control Center</h1>
              <p className="text-[9px] text-slate-400 uppercase">Registry & Compliance</p>
            </div>
          </div>
      ...
      <div className="bg-white border-b py-2 px-4 shadow-sm">
        <div className="max-w-7xl mx-auto flex items-center gap-4 overflow-x-auto">
          ...
          <div className="flex items-center gap-1.5 px-3 py-1.5 bg-blue-50 rounded-lg">
            ...
            <div>
              <div className="text-sm font-bold text-blue-700">{stats.withUrl}</div>
              <div className="text-[8px] font-bold text-blue-600 uppercase">With URL</div>
            </div>
          </div>
          <div className="flex items-center gap-1.5 px-3 py-1.5 bg-red-50 rounded-lg">
            <FileWarning size={14} className="text-red-600" />
            <div>
              <div className="text-sm font-bold text-red-700">{stats.withoutUrl}</div>
              <div className="text-[8px] font-bold text-red-600 uppercase">No URL</div>
            </div>
          </div>
          <div className="flex items-center gap-1.5 px-3 py-1.5 bg-emerald-50 rounded-lg">
            <Shield size={14} className="text-emerald-600" />
            <div>
              <div className="text-sm font-bold text-emerald-700">{stats.active}</div>
              <div className="text-[8px] font-bold text-emerald-600 uppercase">Active</div>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 py-4">
        <div className="flex gap-1.5 mb-4 overflow-x-auto pb-1">
          {tabs.map((t) => (
            <button
              ...
              className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-[10px] font-bold uppercase tracking-wide transition-all whitespace-nowrap ${activeTab === t.id ? 'bg-[#00234E] text-white shadow' : 'bg-white text-slate-500 hover:bg-slate-50 border border-slate-100'}`}
            >
              {t.icon} {t.label}
            </button>
          ))}
        ...
        {loading ? (
          ...
          <>
              ...
              <div className="space-y-4">
                <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
                  <div className="bg-white rounded-xl p-4 shadow-sm border">
                    <Database size={18} className="text-slate-700 mb-2" />
                  ...
                  <div className="bg-white rounded-xl p-4 shadow-sm border">
                    <Globe size={18} className="text-blue-600 mb-2" />
                  ...
                  <div className="bg-white rounded-xl p-4 shadow-sm border">
                    <FileWarning size={18} className="text-red-600 mb-2" />
                    <div className="text-2xl font-bold text-red-600">{stats.withoutUrl}</div>
                    <div className="text-[9px] font-bold text-slate-400 uppercase">Need URLs</div>
                  </div>
                  <div className="bg-white rounded-xl p-4 shadow-sm border">
                    <Shield size={18} className="text-emerald-600 mb-2" />
                    <div className="text-2xl font-bold text-emerald-600">{stats.active}</div>
                    <div className="text-[9px] font-bold text-slate-400 uppercase">Compliant</div>
                  </div>
                </div>
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <div>
                          <h4 className="font-bold text-red-800">
                            {stats.withoutUrl} providers need URLs before scanning
                          </h4>
                          <p className="text-xs text-red-600">
                            Export → Add URLs → Re-import → Run Global Scan
                          </p>
                        </div>
                      </div>
                      <button
                        onClick={handleExportMissingUrls}
                        className="bg-red-600 text-white px-3 py-1.5 rounded-lg text-[10px] font-bold flex items-center gap-1 hover:bg-red-700"
                      >
                ...
                <div className="bg-[#00234E] rounded-xl p-4">
                  <h3 className="text-[10px] font-bold text-[#C5A059] uppercase mb-3">
                    Scan & Import Actions
                  </h3>
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
                    <button
                      onClick={handleGlobalScan}
                      disabled={stats.withUrl === 0}
                      className={`${stats.withUrl === 0 ? 'opacity-50 cursor-not-allowed' : ''} bg-white/10 hover:bg-white/20 text-white p-3 rounded-lg text-center`}
                    >
                      <Play size={18} className="mx-auto mb-1" />
                      <div className="text-[8px] font-bold uppercase">Global Scan</div>
                      <div className="text-[7px] text-slate-400">
                        Scan {stats.withUrl} providers
                      </div>
                    </button>
                    <button
                      onClick={() => fileInputRef.current?.click()}
                      className="bg-white/10 hover:bg-white/20 text-white p-3 rounded-lg text-center"
                    >
                      <Upload size={18} className="mx-auto mb-1" />
                      <div className="text-[8px] font-bold uppercase">Import CSV</div>
                    ...
                    >
                      ...
                      <div className="text-[8px] font-bold uppercase">Export All</div>
                    ...
                    <button
                      ...
                      className="bg-white/10 hover:bg-white/20 text-white p-3 rounded-lg text-center"
                    >
                      <FileWarning size={18} className="mx-auto mb-1" />
                      <div className="text-[8px] font-bold uppercase">Missing URLs</div>
                ...
                <div className="bg-blue-50 border border-blue-200 rounded-lg p-3 flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-sm text-blue-800">
                      Showing <strong>{filteredProviders.length}</strong> of {providers.length}{' '}
                      loaded providers ({stats.withUrl.toLocaleString()} scannable).
                      <span className="text-blue-600 ml-1">
                        {stats.withoutUrl.toLocaleString()} need URLs.
                      </span>
                    </span>
                  </div>
                </div>
                <div className="flex flex-wrap gap-2 items-center justify-between">
                  <div className="flex gap-2 items-center">
                    <div className="relative">
                      ...
                      <input
                        ...
                        className="pl-9 pr-3 py-1.5 text-sm bg-white border rounded-lg w-72"
                      />
                    </div>
                    {searchTerm && (
                      <button
                    ...
                    {selectedProviders.length > 0 && (
                      <button
                        onClick={() => handleScan(selectedProviders)}
                        disabled={scanning}
                        className="bg-[#C5A059] text-white px-3 py-1.5 rounded-lg text-[10px] font-bold flex items-center gap-1 disabled:opacity-50"
                      >
                        <Play size={12} /> Scan ({selectedProviders.length})
                      </button>
                    )}
                  </div>
                  <div className="flex gap-2">
                    <button
                      ...
                      className="bg-[#00234E] text-white px-3 py-1.5 rounded-lg text-[10px] font-bold flex items-center gap-1"
                    >
                      <Plus size={12} /> Add
                    </button>
                    <button
                      onClick={() => fileInputRef.current?.click()}
                      className="bg-white border px-3 py-1.5 rounded-lg text-[10px] font-bold flex items-center gap-1"
                    >
                ...
                <div className="bg-white rounded-lg shadow-sm border overflow-hidden">
                  <div className="overflow-x-auto">
                    <table className="w-full text-left text-sm">
                      <thead>
                        <tr className="bg-[#00234E] text-white text-[9px] font-bold uppercase">
                          <th className="px-3 py-2 w-8">
                            <input
                              type="checkbox"
                              onChange={(e) =>
                                setSelectedProviders(
                                  e.target.checked ? filteredProviders.map((p) => p.id) : [],
                                )
                              }
                              checked={
                          ...
                          <th
                            className="px-3 py-2 cursor-pointer hover:text-[#C5A059] select-none"
                            onClick={() => toggleSort('name')}
                          >
                            Name / City <SortIcon field="name" />
                          </th>
                          <th
                            className="px-3 py-2 cursor-pointer hover:text-[#C5A059] select-none"
                            onClick={() => toggleSort('npi')}
                          >
                            NPI <SortIcon field="npi" />
                          </th>
                          <th
                            className="px-3 py-2 cursor-pointer hover:text-[#C5A059] select-none"
                          ...
                          <th
                            className="px-3 py-2 cursor-pointer hover:text-[#C5A059] select-none"
                            onClick={() => toggleSort('last_scan_timestamp')}
                          >
                            Last Scan <SortIcon field="last_scan_timestamp" />
                          </th>
                          <th
                            className="px-3 py-2 cursor-pointer hover:text-[#C5A059] select-none"
                            onClick={() => toggleSort('report_status')}
                          >
                            Report <SortIcon field="report_status" />
                          </th>
                          <th
                            className="px-3 py-2 cursor-pointer hover:text-[#C5A059] select-none"
                            onClick={() => toggleSort('widget_status')}
                          >
                            Status <SortIcon field="widget_status" />
                          </th>
                      ...
                      <tbody className="divide-y">
                        {filteredProviders.map((r) => {
                          ...
                          const scanDate = r.last_scan_timestamp
                            ? new Date(r.last_scan_timestamp)
                            : null;
                          return (
                            <tr
                              key={r.id}
                              className={`hover:bg-slate-50 group ${!r.url ? 'bg-red-50/50' : ''}`}
                            >
                              <td className="px-3 py-2">
                                <input
                                  type="checkbox"
                                  checked={selectedProviders.includes(r.id)}
                                  onChange={() =>
                                    setSelectedProviders((p) =>
                                      p.includes(r.id) ? p.filter((i) => i !== r.id) : [...p, r.id],
                                    )
                                  }
                                />
                              </td>
                              ...
                              <td className="px-3 py-2">
                                <code className="text-xs font-mono bg-slate-50 px-1 rounded">
                                  {r.npi}
                                </code>
                              </td>
                              <td className="px-3 py-2">
                                <span className="text-xs text-slate-500">{r.zip || '-'}</span>
                              </td>
                              <td className="px-3 py-2">
                                {r.url ? (
                                  <a
                              ...
                              <td className="px-3 py-2">
                                <div className="flex items-center gap-2">
                                  <div className="w-10 h-1.5 bg-slate-100 rounded-full overflow-hidden">
                                    <div
                                      className={`h-full ${(r.risk_score || 0) >= 67 ? 'bg-emerald-500' : (r.risk_score || 0) >= 34 ? 'bg-amber-500' : 'bg-red-500'}`}
                                      style={{ width: `${r.risk_score || 0}%` }}
                                    />
                                  </div>
                                  <span className="text-xs font-bold">{r.risk_score || 0}</span>
                                </div>
                              </td>
                              <td className="px-3 py-2">
                                {scanDate ? (
                                  ...
                                  <span className="text-[10px] text-slate-300">Never</span>
                                )}
                              </td>
                              <td className="px-3 py-2">
                                {hasReport ? (
                                  <a
                                    href={r.latest_report_url || '#'}
                                    target="_blank"
                                    rel="noopener noreferrer"
                                    className="inline-flex items-center gap-1 px-1.5 py-0.5 bg-emerald-50 border border-emerald-200 text-emerald-700 rounded text-[9px] font-bold hover:bg-emerald-100 transition-colors"
                                  >
                              ...
                              <td className="px-3 py-2">
                                <StatusBadge status={r.widget_status || 'hidden'} size="sm" />
                              </td>
                              <td className="px-3 py-2">
                                <div className="flex items-center justify-end gap-0.5 opacity-0 group-hover:opacity-100 transition-opacity">
                                  ...
                                  <button
                                    onClick={() =>
                                      r.url ? handleScan([r.id]) : notify('Add URL first', 'error')
                                    }
                                    title="Scan"
                                    disabled={scanning}
                                    className={`p-1 hover:bg-amber-100 rounded ${!r.url ? 'opacity-30' : ''}`}
                                  >
                                    <Play size={13} className="text-[#C5A059]" />
                                  </button>
                                  <button
                                    onClick={() => handleDeleteProvider(r.id)}
                                    title="Delete"
              ...
              <div className="space-y-3">
                <div className="grid grid-cols-3 gap-2">
                  ...
                  <div className="bg-amber-50 border border-amber-200 rounded-lg p-3 text-center">
                    ...
                    <div className="text-[8px] font-bold text-amber-600 uppercase">Warning</div>
                  </div>
                  <div className="bg-slate-100 border border-slate-200 rounded-lg p-3 text-center">
                    <div className="text-xl font-bold text-slate-500">{stats.hidden}</div>
                    <div className="text-[8px] font-bold text-slate-400 uppercase">Hidden</div>
                  </div>
                </div>
                <div className="bg-white rounded-lg shadow-sm border overflow-hidden">
                  <table className="w-full text-left text-sm">
                    <thead>
                      <tr className="bg-[#00234E] text-white text-[9px] font-bold uppercase">
                        ...
                        <th className="px-3 py-2">Widget ID</th>
                      ...
                      {providers.map((r) => (
                        <tr key={r.id} className="hover:bg-slate-50">
                          <td className="px-3 py-2">
                            ...
                            <div className="text-[9px] text-slate-400 font-mono">{r.npi}</div>
                          </td>
                          <td className="px-3 py-2">
                            <code className="text-xs font-mono bg-slate-50 px-1 rounded">
                              {r.widget_id || `WGT-${r.npi}`}
                            </code>
                          </td>
                          <td className="px-3 py-2">
                            <StatusBadge status={r.widget_status || 'hidden'} size="sm" />
                          ...
                          <td className="px-3 py-2">
                            <div className="flex items-center gap-1.5">
                              <select
                                value={r.widget_status || 'hidden'}
                                onChange={(e) => handleWidgetStatusChange(r, e.target.value)}
                                className="px-2 py-1 text-xs bg-slate-50 border rounded"
                              >
                                <option value="active">Active</option>
                                <option value="warning">Warning</option>
                                <option value="hidden">Hidden</option>
                              ...
                              <button
                                onClick={() => setWidgetModal(r)}
                                className="p-1 hover:bg-amber-100 rounded"
                              >
                                <FileCode size={13} className="text-[#C5A059]" />
                              </button>
                            </div>
                          </td>
                        </tr>
            ...
            {activeTab === 'templates' && <EmailTemplatesTab showNotification={notify} />}
                    ...
                    {calendarDates.map((d) => {
                      ...
                      return (
                        <button
                          ...
                          className={`flex-shrink-0 w-14 py-2 rounded-lg border ${selectedDate === d ? 'bg-[#00234E] text-white' : 'bg-slate-50 hover:border-[#C5A059]'}`}
                        >
                  ...
                  <div className="bg-white rounded-lg p-3 shadow-sm border">
                    <div className="grid grid-cols-3 sm:grid-cols-6 gap-1">
                      {dateSlots.map((s) => (
                        <div
                          key={s.id}
                          className={`p-1.5 rounded-lg border text-center text-[10px] ${s.is_booked ? 'bg-slate-100' : 'bg-emerald-50 border-emerald-200'}`}
                        >
                          <div
                            className={`font-bold ${s.is_booked ? 'text-slate-700' : 'text-emerald-700'}`}
                          >
                            {s.time}
                          </div>
                        </div>
      ...
      {viewingProvider && (
        <ProviderDetailModal entry={viewingProvider} onClose={() => setViewingProvider(null)} />
      )}
        ...
        {editingTemplate && (
          <form
            onSubmit={(e) => {
              e.preventDefault();
              setTemplates((p) => {
                const ex = p.find((x) => x.id === editingTemplate.id);
                return ex
                  ? p.map((x) => (x.id === editingTemplate.id ? editingTemplate : x))
                  : [...p, editingTemplate];
              });
          ...
          >
            ...
            <div>
              ...
              <textarea
                ...
                rows={4}
                className="w-full px-3 py-2 text-sm font-mono bg-slate-50 border rounded-lg"
              />
            </div>
            <div className="flex justify-end gap-2 pt-3 border-t">
              <button
              ...
              <button
                type="submit"
                className="px-5 py-2 bg-[#00234E] text-white text-sm font-bold rounded-lg hover:bg-[#C5A059]"
              >
                Save
              </button>
            </div>
                ...
                <button
                  onClick={() => {
                    navigator.clipboard.writeText(
                      `<script src="https://widget.kairologic.net/sentry.js" data-npi="${widgetModal.npi}"></script>`,
                    );
                    notify('Copied!');
                  }}
                  className="text-[9px] font-bold text-[#C5A059] flex items-center gap-0.5"
                >
                  <Copy size={10} /> Copy
                </button>
              </div>
      ...
      {/* Scan Report Modal */}
      <Modal
        isOpen={showScanReport && !!scanResult}
        onClose={() => setShowScanReport(false)}
        title="Global Scan Report - Type 2 Providers"
        size="xl"
      >
        {scanResult && (
          <div className="space-y-4">
            <div className="grid grid-cols-4 gap-3">
              <div className="bg-slate-50 rounded-lg p-3 text-center">
                ...
                <div className="text-[8px] font-bold text-slate-400 uppercase">Total Type 2</div>
              </div>
              <div className="bg-blue-50 rounded-lg p-3 text-center">
                <div className="text-2xl font-bold text-blue-700">
                  {scanResult.scanned.toLocaleString()}
                </div>
                <div className="text-[8px] font-bold text-blue-600 uppercase">Scanned</div>
              </div>
              <div className="bg-emerald-50 rounded-lg p-3 text-center">
                <div className="text-2xl font-bold text-emerald-700">
                  {scanResult.withUrl.toLocaleString()}
                </div>
                <div className="text-[8px] font-bold text-emerald-600 uppercase">With URLs</div>
              </div>
              <div className="bg-red-50 rounded-lg p-3 text-center">
                <div className="text-2xl font-bold text-red-700">
                  {scanResult.withoutUrlCount.toLocaleString()}
                </div>
                <div className="text-[8px] font-bold text-red-600 uppercase">Need URLs</div>
              </div>
            </div>
            ...
            {scanResult.scanResults.length > 0 && (
              <div className="bg-white border rounded-lg p-3">
                <h4 className="text-sm font-bold mb-2">
                  Scan Results Summary ({scanResult.scanned} providers scanned)
                </h4>
                  ...
                  <table className="w-full text-xs">
                    <thead>
                      <tr className="text-left text-slate-500 border-b">
                        ...
                        <th className="py-1">SB1188</th>
                        <th className="py-1">HB149</th>
                    ...
                    <tbody>
                      {scanResult.scanResults.map((r, i) => (
                        <tr key={i} className="border-b border-slate-100">
                          ...
                          <td className="py-1">
                            <span className="text-emerald-600">{r.sb1188_pass_count}✓</span>{' '}
                            <span className="text-red-600">{r.sb1188_fail_count}✗</span>
                          </td>
                          <td className="py-1">
                            <span className="text-emerald-600">{r.hb149_pass_count}✓</span>{' '}
                            <span className="text-red-600">{r.hb149_fail_count}✗</span>
                          </td>
                          <td className="py-1">
                            <span
                              className={`px-1.5 py-0.5 rounded text-[8px] font-bold ${r.risk_level === 'low' ? 'bg-emerald-100 text-emerald-700' : r.risk_level === 'medium' ? 'bg-amber-100 text-amber-700' : 'bg-red-100 text-red-700'}`}
                            >
                              {r.risk_level}
                            </span>
                          </td>
                ...
                <div className="flex items-center justify-between">
                  <div>
                    <h4 className="text-sm font-bold text-red-800">
                      {scanResult.withoutUrlCount.toLocaleString()} Type 2 Providers Need URLs
                    </h4>
                    <p className="text-xs text-red-600 mt-1">
                      Export the list, add website URLs, then re-import to enable scanning
                    </p>
                  </div>
                  <button
                    onClick={handleExportMissingUrls}
          ...
          {importData.length > 0 ? (
            <>
              <div className="bg-emerald-50 border border-emerald-200 rounded-lg p-3">
                <h4 className="text-sm font-bold text-emerald-800">
                  Ready to import {importData.length} providers
                </h4>
                <p className="text-[10px] text-emerald-600 mt-0.5">
                  {importData.filter((r: any) => r.url).length} have URLs (scannable),{' '}
                  {importData.filter((r: any) => !r.url).length} without URLs
                </p>
              </div>
                ...
                <table className="w-full text-xs">
                  <thead>
                    <tr className="text-left font-bold text-slate-600">
                      <th className="py-1 px-2">NPI</th>
                      <th className="py-1 px-2">Name</th>
                      <th className="py-1 px-2">City</th>
                      <th className="py-1 px-2">Zip</th>
                  ...
                  <tbody>
                    ...
                    {importData.length > 20 && (
                      <tr>
                        <td colSpan={5} className="py-1 px-2 text-center text-slate-400">
              ...
              <div className="flex justify-end gap-2">
                ...
                <button
                  ...
                  disabled={scanning}
                  className="px-5 py-2 bg-[#00234E] text-white text-sm font-bold rounded-lg hover:bg-[#C5A059] flex items-center gap-1.5 disabled:opacity-50"
                >
            ...
            <div className="text-center py-8">
              ...
              <p className="text-[10px] text-slate-400 mt-1">
                CSV columns: NPI, Name, URL, City, Zip, Email, Phone
              </p>
              <button
                onClick={() => fileInputRef.current?.click()}
                className="mt-3 px-4 py-2 bg-[#00234E] text-white text-sm font-bold rounded-lg"
              >
                Choose File
              </button>
            </div>
      ...
      <Modal
        isOpen={showImportScanPrompt}
        onClose={() => {
          setShowImportScanPrompt(false);
          setImportedIds([]);
        }}
        title="Scan Imported Providers?"
        size="sm"
      >
        <div className="space-y-4">
          <div className="bg-amber-50 border border-amber-200 rounded-lg p-4 text-center">
            ...
            <h4 className="text-sm font-bold text-slate-800">
              {importedIds.length} imported provider(s) have URLs
            </h4>
            <p className="text-xs text-slate-500 mt-1">
              Run compliance scans now to generate reports?
            </p>
          </div>
          <div className="flex justify-center gap-3">
            <button
              ...
              className="px-4 py-2 text-sm text-slate-500 border rounded-lg"
            >
              Skip
            </button>
            <button
              onClick={() => {
                setShowImportScanPrompt(false);
                handleScan(importedIds);
                setImportedIds([]);
              }}
              className="px-5 py-2 bg-[#C5A059] text-white text-sm font-bold rounded-lg hover:brightness-110 flex items-center gap-1.5"
            >
