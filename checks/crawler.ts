...
function extractAddress(html: string): {
  ...
  // or "123 Main St\nAustin, TX 78701"
  const addrRegex =
    /(\d{1,5}\s+[A-Za-z0-9\s.,#\-]+?)[\s,]+([A-Z][a-z]+(?:\s[A-Z][a-z]+)?)\s*,\s*([A-Z]{2})\s+(\d{5}(?:-\d{4})?)/g;
  const match = addrRegex.exec(text);

  if (match) {
    const streetParts = match[1]
      .trim()
      .split(/[,\n]+/)
      .map((s) => s.trim());
    return {
...
function extractSchemaAddress(html: string): {
  line1: string;
  line2: string;
  city: string;
  state: string;
...
function extractProviders(html: string): string[] {
  ...
  const credentialPattern =
    /(?:Dr\.?\s+)?([A-Z][a-z]+(?:\s+[A-Z]\.?)?\s+[A-Z][a-z]{2,})(?:\s*,\s*(?:MD|DO|NP|PA|PA-C|APRN|ARNP|DNP|PhD|DPM|DC|DDS|DMD|OD|RN|LPC|LCSW|LMFT))/g;
