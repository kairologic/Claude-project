...
export async function runScan(
  ...
  const riskLevel =
    compositeScore >= 75 ? 'Sovereign' : compositeScore >= 50 ? 'Drift' : 'Violation';
  ...
  const npiResults = results.filter(
    (r) => r.category === 'npi-integrity' && (r.status === 'fail' || r.status === 'warn'),
  );
    ...
    if (existing?.length > 0) {
      ...
      await supabaseFetch(`mismatch_alerts?id=eq.${existing[0].id}`, {
        ...
        body: JSON.stringify({
          ...
          npi_value:
            evidence.npi_address || evidence.npi_phone || evidence.npi_classification || '',
          site_value:
            evidence.site_address ||
            evidence.site_phone ||
            evidence.site_specialties?.join(', ') ||
            '',
  ...
  const passingNpiChecks = results.filter(
    (r) => r.category === 'npi-integrity' && r.status === 'pass',
  );
  for (const passing of passingNpiChecks) {
    await supabaseFetch(`mismatch_alerts?npi=eq.${npi}&check_id=eq.${passing.id}&status=eq.open`, {
      method: 'PATCH',
      body: JSON.stringify({
        status: 'resolved',
  ...
  console.log(
    `[Engine] Scan complete for NPI ${npi}: ${compositeScore}/100 (${riskLevel}) — ${passed}✓ ${failed}✗ ${warned}⚠`,
  );
