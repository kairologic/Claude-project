  ...
  async run(ctx) {
    ...
    if (!site || site.provider_count === 0) {
      return {
        ...
        detail:
          'Could not determine provider count from the website. Ensure your "Our Providers" or team page lists providers individually.',
      };
    }
...
export const rosterNameCheck: CheckModule = {
  ...
  async run(ctx) {
    ...
    const onSiteNotInNpi = siteNames.filter(
      (sn) => !npiNames.some((nn) => fuzzyNameMatch(sn.normalized, nn.normalized)),
    );

    // Find providers in NPI but NOT on website
    const inNpiNotOnSite = npiNames.filter(
      (nn) => !siteNames.some((sn) => fuzzyNameMatch(sn.normalized, nn.normalized)),
    );
