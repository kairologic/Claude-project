'use strict';
(() => {
  function detectNpiFromNPPES() {
    const params = new URLSearchParams(window.location.search);
    return params.get('npi') || null;
  }
  function detectNpiFromPECOS() {
    const npiInput = document.querySelector('input[name*="npi"], input[id*="npi"]');
    if (npiInput?.value) {
  ...
  function detectNpiFromPayerPortal() {
    const selectors = [
      'input[name*="npi"]',
      'input[id*="npi"]',
      '[data-field="npi"]',
  ...
  function injectSidebar(corrections) {
  ...
  async function markCorrectionAsComplete(workflowId) {
    try {
