...
test.describe('UC-WF2: Payer Directory Workflows', () => {
  ...
  test('UC-WF2-01: Payer directory page loads with grid view', async ({
    page,
  }) => {
  ...
  test('UC-WF2-03: Mismatch details show source comparison', async ({
    page,
  }) => {
    ...
    if (mismatchCount > 0) {
      ...
      const comparisonTable = detailPanel.locator(
        '[data-testid="mismatch-comparison"]'
      );
      await expect(comparisonTable).toBeVisible();

      // Verify source columns exist
      const sourceColumn = detailPanel.locator(
