...
test.describe('UC-WF3: Onboarding Workflow', () => {
  ...
  test('UC-WF3-01: Onboarding workflow type visible in filters', async ({
    page,
  }) => {
  ...
  test('UC-WF3-02: Onboarding workflow detail shows credentialing checklist', async ({
    page,
  }) => {
    // Filter for onboarding workflows
    const onboardingFilter = page.locator(
      '[data-testid="filter-workflow-type-onboarding"]'
    );
    await onboardingFilter.click();
    await page.waitForTimeout(500);
    ...
    if (cardCount > 0) {
      ...
      const checklist = detailPanel.locator(
        '[data-testid="credentialing-checklist"]'
      );
  ...
  test('UC-WF3-03: Task progression follows sequential order', async ({
    page,
  }) => {
    // Filter for onboarding workflows
    const onboardingFilter = page.locator(
    ...
    if (cardCount > 0) {
      ...
      const progressionSection = detailPanel.locator(
        '[data-testid="task-progression"]'
      );
