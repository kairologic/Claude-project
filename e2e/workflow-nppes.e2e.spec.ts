...
test.describe('UC-WF1: NPPES Update Workflow', () => {
  ...
  test('UC-WF1-02: Click workflow card opens detail panel', async ({
    page,
  }) => {
  ...
  test('UC-WF1-03: Detail panel shows provider name, status, type badges', async ({
    page,
  }) => {
    // Open first workflow
    const workflowCard = page
    ...
    const providerName = detailPanel.locator(
      '[data-testid="workflow-provider-name"]'
    );
    await expect(providerName).toBeVisible();

    // Verify status badge is displayed
    const statusBadge = detailPanel.locator(
    ...
    const typeBadge = detailPanel.locator(
      '[data-testid="workflow-type-badge"]'
    );
    await expect(typeBadge).toBeVisible();
  });

  test('UC-WF1-04: Task checklist displayed with status icons', async ({
    page,
  }) => {
    // Open first workflow
    const workflowCard = page
    ...
    const taskChecklist = detailPanel.locator(
      '[data-testid="workflow-task-checklist"]'
    );
    ...
    const statusIcon = firstTask.locator(
  ...
  test('UC-WF1-05: Filter pills toggle workflow list', async ({ page }) => {
    ...
    if (pillCount > 0) {
      ...
      expect(
        filteredCount !== initialCount || hasEmptyState
      ).toBeTruthy();
    }
  });
  ...
  test('UC-WF1-07: Empty state shown when no results match filter', async ({
    page,
  }) => {
  ...
  test('UC-WF1-08: Workflow detail panel slides in from right', async ({
    page,
  }) => {
  ...
  test('UC-WF1-09: Close button dismisses detail panel', async ({ page }) => {
    ...
    const closeBtn = detailPanel.locator(
      '[data-testid="workflow-detail-close-btn"]'
    );
  ...
  test('UC-WF1-10: Workflow results count updates with filters', async ({
    page,
  }) => {
    // Get initial result count
    const resultCount = page.locator(
      '[data-testid="workflow-result-count"]'
    );
