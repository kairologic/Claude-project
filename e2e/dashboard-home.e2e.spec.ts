...
test.describe('UC-HOME: Dashboard Home', () => {
  ...
  test('UC-HOME-03: Priority providers list displays provider data', async ({
    page,
  }) => {
    // Verify priority providers section is present
    const prioritySection = page.locator(
      '[data-testid="priority-providers-section"]'
    );
    ...
    if (hasProviders) {
      const providerCards = page.locator(
        '[data-testid="priority-provider-card"]'
      );
      ...
      if (count > 0) {
        ...
        await expect(
          firstCard.locator('[data-testid="provider-name"]')
        ).toBeVisible();
      }
    }
  });

  test('UC-HOME-04: Payer sync status panel is visible', async ({ page }) => {
    // Locate payer sync status panel
    const payerSyncPanel = page.locator(
      '[data-testid="payer-sync-status-panel"]'
    );
    await expect(payerSyncPanel).toBeVisible();

    // Verify it has status information
    const statusText = payerSyncPanel.locator(
      '[data-testid="payer-sync-status-text"]'
    );
    await expect(statusText).toBeVisible();

    // Verify timestamp is displayed
    const timestamp = payerSyncPanel.locator(
      '[data-testid="payer-sync-timestamp"]'
    );
    await expect(timestamp).toBeVisible();
  });

  test('UC-HOME-05: Welcome banner shows user name and can be dismissed', async ({
    page,
  }) => {
    ...
    if (await welcomeBanner.isVisible()) {
      ...
      const welcomeText = welcomeBanner.locator(
      ...
      const dismissBtn = welcomeBanner.locator(
  ...
  test('UC-HOME-06: Practice compliance section is visible', async ({
    ...
    const statusIndicator = firstItem.locator(
