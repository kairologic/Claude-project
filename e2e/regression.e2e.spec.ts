...
test.describe('UC-REG: Regression Tests', () => {
  ...
  test('UC-REG-02: Error boundaries catch rendering errors', async ({ page }) => {
    // Navigate to a page
    await navigateToDashboard(page);
  ...
  test('UC-REG-03: Long provider names truncate properly', async ({ page }) => {
    // Navigate to roster
    ...
    if (rowCount > 0) {
      ...
      expect(hasOverflow || styles.whiteSpace?.includes('nowrap')).toBeTruthy();
    }
  });

  test('UC-REG-04: Keyboard navigation works (Tab, Enter, Escape)', async ({ page }) => {
    await navigateToDashboard(page);
