...
test.describe('UC-ALRT: Alerts', () => {
  ...
  test('UC-ALRT-01: Alerts page loads with summary count', async ({ page }) => {
  ...
  test('UC-ALRT-02: New alerts section shows with label', async ({ page }) => {
    ...
    if (hasNewAlerts) {
      ...
      const sectionLabel = newAlertsSection.locator('[data-testid="new-alerts-label"]');
      ...
      const alertCards = newAlertsSection.locator('[data-testid="alert-card"]');
      const count = await alertCards.count();
      expect(count).toBeGreaterThan(0);
    }
  });

  test('UC-ALRT-03: Alert cards display severity, title, description', async ({ page }) => {
    // Get alert cards
    ...
    const severityIndicator = firstAlert.locator('[data-testid="alert-severity-icon"]');
    const hasSeverityIcon = await severityIndicator.isVisible().catch(() => false);
  ...
  test('UC-ALRT-04: Earlier alerts section separated by divider', async ({ page }) => {
    // Check for earlier alerts section
    const earlierAlertsSection = page.locator('[data-testid="earlier-alerts-section"]');
    const hasEarlierAlerts = await earlierAlertsSection.isVisible().catch(() => false);
    ...
    if (hasEarlierAlerts) {
      ...
      const sectionLabel = earlierAlertsSection.locator('[data-testid="earlier-alerts-label"]');
      ...
      const alertCards = earlierAlertsSection.locator('[data-testid="alert-card"]');
      const count = await alertCards.count();
      expect(count).toBeGreaterThan(0);
    }
  });
