    ...
    test(`all images load on ${pagePath}`, async ({ page, baseURL }) => {
      ...
      for (const src of [...new Set(srcs)]) {
        ...
        try {
          ...
          expect
            .soft(response.status(), `Broken image: ${src} (found on ${pagePath})`)
            .toBeLessThan(400);
        } catch {
  ...
  test('key external links return valid status', async ({ page, baseURL }) => {
    ...
    const externalLinks = [
      ...new Set(
        allHrefs.filter(
          (href) =>
            href.startsWith('http') && !href.includes('localhost') && !href.includes('127.0.0.1'),
        ),
      ),
    ];
    ...
    for (const url of sample) {
      try {
        ...
        expect.soft(response.status(), `External link returned error: ${url}`).toBeLessThan(400);
      } catch {
...
test.describe('Downloadable assets', () => {
  const expectedAssets = ['/sample-report.pdf', '/logo.svg'];
