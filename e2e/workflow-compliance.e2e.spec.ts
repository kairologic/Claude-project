...
test.describe('UC-WF5: Compliance Workflow', () => {
  ...
  test('UC-WF5-01: Compliance workflow type visible in filters', async ({ page }) => {
  ...
  test('UC-WF5-02: Compliance workflow shows finding details', async ({ page }) => {
    // Filter for compliance workflows
    const complianceFilter = page.locator('[data-testid="filter-workflow-type-compliance"]');
    await complianceFilter.click();
    await page.waitForTimeout(500);

    // Get compliance workflow cards
    const workflowCards = page.locator('[data-testid="workflow-card"][data-type="compliance"]');
    ...
    if (cardCount > 0) {
      ...
      const findingsSection = detailPanel.locator('[data-testid="findings-section"]');
      await expect(findingsSection).toBeVisible();

      // Verify findings have details
      const findingItems = findingsSection.locator('[data-testid="finding-item"]');
      ...
      const findingTitle = firstFinding.locator('[data-testid="finding-title"]');
      const findingDescription = firstFinding.locator('[data-testid="finding-description"]');
      const findingSeverity = firstFinding.locator('[data-testid="finding-severity"]');
  ...
  test('UC-WF5-03: Compliance remediation template available', async ({ page }) => {
    // Filter for compliance workflows
    const complianceFilter = page.locator('[data-testid="filter-workflow-type-compliance"]');
    await complianceFilter.click();
    await page.waitForTimeout(500);
    ...
    if (cardCount > 0) {
      ...
      const remediationSection = detailPanel.locator('[data-testid="remediation-section"]');

      if (await remediationSection.isVisible()) {
        // Verify template button is available
        const templateBtn = remediationSection.locator('[data-testid="use-template-button"]');
        ...
        const templateModal = page.locator('[data-testid="remediation-template-modal"]');
        await expect(templateModal).toBeVisible({ timeout: 5000 });

        // Verify template has content
        const templateContent = templateModal.locator('[data-testid="template-content"]');
