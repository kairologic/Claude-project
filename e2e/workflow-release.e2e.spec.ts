...
test.describe('UC-WF4: Provider Release Workflow', () => {
  ...
  test('UC-WF4-01: Release workflow type visible in filters', async ({
    page,
  }) => {
    ...
    const releaseFilter = filterSection.locator(
      '[data-testid="filter-workflow-type-release"]'
    );
    ...
    const releaseWorkflows = page.locator(
      '[data-testid="workflow-card"][data-type="release"]'
    );
  ...
  test('UC-WF4-02: Release workflow shows departure checklist', async ({
    page,
  }) => {
    // Filter for release workflows
    const releaseFilter = page.locator(
      '[data-testid="filter-workflow-type-release"]'
    );
    await releaseFilter.click();
    await page.waitForTimeout(500);
    ...
    if (cardCount > 0) {
      ...
      const checklist = detailPanel.locator(
        '[data-testid="departure-checklist"]'
      );
      await expect(checklist).toBeVisible();

      // Verify checklist has items (e.g., terminations, obligations, etc.)
      const checklistItems = checklist.locator(
  ...
  test('UC-WF4-03: Mark provider as departing creates release workflow', async ({
    page,
  }) => {
    ...
    if (rowCount > 0) {
      ...
      const departingOption = page.locator(
        '[data-testid="provider-action-departing"]'
      );
