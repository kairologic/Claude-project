...
const SUPABASE_ANON_KEY =
  'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Im14cnRsdGV6aGt4aHFpenZ4dnN6Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3Njg3NzI1ODAsImV4cCI6MjA4NDM0ODU4MH0.pkPlFyHsMOKJKcxuw_eoV5EKkrXG09Vx_0MIDgHn7aw';
...
const RiskScanWidget: React.FC<RiskScanWidgetProps> = ({
  ...
  const saveToRegistry = async (scanResults: any) => {
    ...
    try {
      ...
      if (patchedRows === 0) {
        ...
        response = await fetch(`${SUPABASE_URL}/rest/v1/registry`, {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
            apikey: SUPABASE_ANON_KEY,
  ...
  const runScan = async () => {
    ...
    try {
      ...
      try {
        await fetch('https://mxrtltezhkxhqizvxvsz.supabase.co/functions/v1/calculate-risk', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
    ...
    <div className="w-full max-w-4xl mx-auto p-6 bg-gradient-to-br from-slate-50 to-slate-100 rounded-xl shadow-lg">
      ...
      <div className="bg-white p-6 rounded-lg shadow-sm mb-6">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
          <div>
            <label className="block text-sm font-medium text-slate-700 mb-2">Provider NPI</label>
            <input
              type="text"
              value={npi}
              ...
              <div className="mt-4 pt-4 border-t border-blue-200">
                ...
                {results.topIssues.map((issue: any, idx: number) => (
                  <div key={idx} className="flex items-center gap-2 text-sm text-slate-600 mb-1.5">
                    ...
                    {issue.severity === 'critical' && (
                      <span className="ml-1 px-1.5 py-0.5 bg-red-50 text-red-600 rounded text-xs">
                        Fine Risk
                      </span>
                    )}
                  </div>
            ...
            <div className="mb-6 p-5 border border-slate-200 rounded-lg">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                {results.dataBorderMap.map((node: any, idx: number) => {
                  ...
                  const riskBg =
                    node.phiRisk === 'direct' && !isUS
                      ? 'bg-red-50 border-red-200'
                      : node.phiRisk === 'indirect' && !isUS
                  ...
                  return (
                    <div key={idx} className={`p-3 rounded-lg border ${riskBg} text-sm`}>
                      <div className="flex items-center justify-between mb-1">
                        <div className="flex items-center gap-1.5">
                          ...
                          <span
                            ...
                            title={node.domain}
                          >
                            {node.domain.length > 28
                              ? node.domain.substring(0, 28) + '...'
                              : node.domain}
                          </span>
                        </div>
                        <span
                      ...
                      <div className="flex items-center gap-1 mt-1">
                        ...
                        {node.purpose && (
                          <span className="text-xs text-slate-400 ml-1">{node.purpose}</span>
                        )}
                      </div>
                    </div>
                  );
              ...
              {(() => {
                ...
                return (
                  <div className="mt-3 pt-3 border-t border-slate-200 text-xs text-slate-500 flex items-center gap-3">
                    <span>
                      {total} endpoint{total !== 1 ? 's' : ''} mapped
                    </span>
                    <span className="text-green-600">🇺🇸 {total - foreign.length} US</span>
                    {foreign.length > 0 && (
                      <span className="text-amber-600">🌍 {foreign.length} foreign</span>
                    )}
                    {foreignPHI.length > 0 && (
                      <span className="text-red-600 font-medium">
                        ⚠ {foreignPHI.length} foreign PHI handler{foreignPHI.length > 1 ? 's' : ''}
                      </span>
                    )}
                  </div>
                );
                      ...
                      {catScore && (
                        <span
                          className={`text-sm font-bold px-2 py-0.5 rounded ${
                            catScore.percentage >= 67
                              ? 'bg-green-100 text-green-700'
                              : catScore.percentage >= 34
                                ? 'bg-amber-100 text-amber-700'
                                : 'bg-red-100 text-red-700'
                          }`}
                        >
                          ...
                          <div className="flex-1">
                            <div className="flex items-center gap-2 flex-wrap">
                              ...
                              {finding.phiRisk === 'indirect' && (
                                <span className="text-xs px-1.5 py-0.5 bg-amber-50 text-amber-600 rounded border border-amber-200">
                                  PHI Indirect
                                </span>
                              )}
                              {/* Page Context Badge */}
