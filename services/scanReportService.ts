...
export async function getReportsForProvider(npi: string): Promise<ReportListItem[]> {
  try {
    ...
    return data.map((r) => ({
      ...
      violations_count: Array.isArray(r.findings)
        ? r.findings.filter((f: unknown) => (f as Record<string, unknown>).status === 'fail').length
        : 0,
 ...
 */
export async function attachPdfToReport(
  reportId: string,
  pdfBase64: string,
  sizeBytes: number,
): Promise<boolean> {
  ...
  try {
    ...
    const [totalRes, monthRes, emailRes, paidRes] = await Promise.all([
      ...
      (() => {
        ...
        return fetch(
          `${SUPABASE_URL}/rest/v1/scan_reports?select=id&report_date=gte.${startOfMonth.toISOString()}&limit=1`,
          { headers: countHeader },
        );
      })(),
      fetch(`${SUPABASE_URL}/rest/v1/scan_reports?select=id&emailed_at=not.is.null&limit=1`, {
        headers: countHeader,
      }),
      fetch(`${SUPABASE_URL}/rest/v1/scan_reports?select=id&payment_confirmed=eq.true&limit=1`, {
        headers: countHeader,
      }),
    ]);
